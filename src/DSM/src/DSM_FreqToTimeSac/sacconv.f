...
c variables for input/output
        use spectotime_par,only:ismooth,station_net,station_name,
     &                           myrank,sac_output_dir
        implicit none
	integer n_frequency,n_station,ncomp,ista_start
	real*8 time_series_length,omega_imag
	real*8 source_depth,source_lat,source_lon
	real*8 station_lat(n_station),station_lon(n_station)
c WENBO
        complex*16 iMulOmega

        complex*16 station_displacement(n_station,ncomp,n_frequency)
        real work_time(4*ismooth*n_frequency)
        real(kind(0d0)) work_in(4*ismooth*n_frequency)
        real(kind(0d0)) work_out(4*ismooth*n_frequency)
        real(kind(0d0)) dt_tmp,f1,f2
        complex*16 work_spc(2*ismooth*n_frequency)


c other variables


	integer i_station,icomp,i_frequency,n1,m1,nn,isign,i
	integer lnblnk
	real*8 t
	character*4 cext(3)
        character*3 cext_stress(6)
        character*5 cext_pressure
	character*256 outfile
c
c	data ismooth / 32 /
	data cext / '.bhz','.bhr','.bht' /
        data cext_stress / '.zz','.rr','.tt','.zr','.zt','.rt' /
        data cext_pressure / '.pres'/
...
c
	do 1000 i_station=1,n_station
          ...
          if(myrank.eq.0) 
     &       print *,'   work on ',i_station,' of',n_station,' stations'

	  do 900 icomp=1,ncomp
c	    work_spc(1) = dcmplx(0.d0)
	    do 200 i_frequency=1,n_frequency
c              if(i_frequency.eq.10.and.i_station.eq.1) then
c                print *,"check_pressure1",
c     c               station_displacement(1,icomp,9:11 )
c              end if
c              if(icomp.eq.1.and.i_station.eq.1) then
c                print *,"check_disp_sta1_comp1",i_frequency,
c     c               station_displacement(1,1,i_frequency )
c              end if
	      work_spc(i_frequency)
     &	        = station_displacement(i_station,icomp,i_frequency)

!             iMulOmega=dcmplx(0,2.d0*3.141502*
...
!     &            iMulOmega
  ...
  200	    continue
	    do 210 i_frequency=n_frequency+1,ismooth*n_frequency+1
  ...
  220	    continue
	    ...
	    isign = 1
c            if(i_station.eq.21)
c     &           print *,"check_spc",work_spc(500:502)
	    call four1( work_spc,nn,isign )
c            if(i_station.eq.21)
c     &           print *,"check_spc1",work_spc(500:502),nn
	    do 230 i=-nn+1,nn
...
c --- km -> m
c	      work_time(i+nn) = work_time(i+nn) * 1.e3
  230	    continue
c	    outfile
c     &	      = sac_file(i_station)(1:lnblnk(sac_file(i_station)))
c     &	        //cext(icomp)
c            write(outfile,"('./OUTPUT_FILES/sta_',i6.6)")i_station
c     &               +ista_start-1
            write(outfile, "(A,'/',A,'_',A)") 
     &           trim(sac_output_dir),trim(station_net(i_station)), 
     &           trim(station_name(i_station))
            if(ncomp.eq.3) then
              outfile
     &          =trim(outfile)//cext(icomp)
            else if(ncomp.eq.6) then
              outfile
     &          =trim(outfile)//cext_stress(icomp)
            else if(ncomp.eq.1) then
              outfile
     &          =trim(outfile)//cext_pressure
            else
              stop 'ncomp should be 3(displacement) or 6(stress)'
            end if
            work_in(:)=work_time(:)
            dt_tmp=time_series_length/nn
c            f1=0.03
c            f2=0.09
c            call bwfilt(work_in,work_out,dt_tmp,
c     &                  2*nn,1,4,f1,f2)
c            work_time(:)=work_out(:)
...
c
	if ( icmpt.eq.1.or.icmpt.eq.4 ) then
	  cmpaz = 0.0
	  cmpinc = 0.0
	else
	if ( icmpt.eq.2 .or.icmpt.eq.5) then
	  cmpaz = baz + 180.0
	  if ( cmpaz.ge.360.0 ) cmpaz = cmpaz - 360.0
	  cmpinc = 90.0
	else
	if ( icmpt.eq.3.or.icmpt.eq.6 ) then
	  cmpaz = baz + 90.0
	  if ( cmpaz.ge.360.0 ) cmpaz = cmpaz - 360.0
	  cmpinc = 90.0
	else
