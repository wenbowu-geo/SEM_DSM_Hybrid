cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	subroutine param_input
     ...
     &	    max_ndep,max_ntheta,dist_ambig,
     &      ngrid_r,lmin,lmax,
...
c        integer ndep,ndep_solid,ndep_fluid,idep_forcut
        ...
        real*8  depth_tolerence,ddepth_for_inpterpo
...
c other variables
	...
	integer i,igll,inode,idep,idep_recount,idist,itheta
...
c WENBO
	source_r = rmax_structure_zone(n_structure_zone)
     &	            - source_depth
c where exp(-25) comes from? Answer: dyn*cm = 10**(-7) N*m = 
c 10**(-7) Pa*m**3=10**(-25) Gpa*km**3. This code uses units 
c of Gpa and km.
...
c where exp(-20) comes from? Answer: 1 dyn=10**(-5) N = 10**(-5)
c Pa*m**2=10**(-20) Gpa*km**2. This code uses units of Gpa and km.
        fr=fr*( 10.d0**(nexp-20) )
...
c        call distaz(0.0,0.0,0.0,90.0,1,dist,az,baz,xdeg,nerr)

        open( unit=12, file=dep_solid_file, status='unknown' )

c ddepth_for_inpterpo - the pamarter used to find and/or add nodes for stress 
c calculation. For each Green's funciton depth, three nodes or depths
c are set to compute the stress.
c depth_tole_tmp - the parameter used to check wether get two depths merged. 
c If node to be added is so close to the existing node (difference < 
c depth_tole_tmp), just merge them and no need to add a new one.
c Therefore, depth_tole_tmp must be smaller than ddepth_for_inpterpo
        read(12,*) ddepth_for_inpterpo, depth_tole_tmp
c        read(12,*)nwgll
c        if(nwgll.lt.2.or.nwgll.gt.10)
c     &         stop 'nwgll should be between 2 and 10'

        depth_tolerence=depth_tole_tmp
        if(ddepth_for_inpterpo.lt.depth_tolerence) 
     &     stop 'Error,ddepth_for_inpterpo smaller than depth_tolerence'

        read(12,*) ndep_solid
        if(ndep_solid.gt.max_ndep) stop 'Error,ndep_solid>max_ndep'
c        if(ndep_solid.le.2) stop 'ndep_solid should be larger than 2'
        idep_recount=0
...
c           igll=mod(idep_recount,nwgll-1)
c           if(igll.eq.0) igll=nwgll-1

           if(ndep_solid.eq.1.and.
     &        (r_freesurf-depth_solid(idep))
     &        -rmin_structure_zone(izone_idep_solid(idep)).lt.
     &        2.0*ddepth_for_inpterpo+depth_tolerence) then
              stop 'single solid depth too close to bottom boundary'
           end if

           if(idep.eq.1) then
c Very close to the discontinuity, so set up three depths as below 
             if(rmax_structure_zone(izone_idep_solid(idep)) -
     &          (r_freesurf-depth_solid(idep)) .lt.
     &          2.0*ddepth_for_inpterpo+depth_tolerence) then
     ...
     &            2.0*ddepth_for_inpterpo  !down
             else
c Setting target depth at the middle gives more accuate interpolated results
               depth_for_stress(3,idep)=depth_solid(idep) -
     &            ddepth_for_inpterpo  !up
               depth_for_stress(2,idep)=depth_solid(idep) !mid
               depth_for_stress(1,idep)=depth_solid(idep) +
     &            ddepth_for_inpterpo  !down
c               print *,'normal_stress_node_top'
             end if
c discontinuity
           else if(izone_idep_solid(idep).ne.
     &             izone_idep_solid(idep-1)) then
c Update the depths (between idep and idep-1) set in the previous step  
             depth_for_stress(3,idep)=depth_solid(idep) !up
             depth_for_stress(2,idep)=depth_solid(idep)+
     &               ddepth_for_inpterpo    !mid
             depth_for_stress(1,idep)=depth_solid(idep)+
     &               2.0*ddepth_for_inpterpo    !down

c Depth between idep and idep+1
             depth_for_stress(3,idep-1)=depth_solid(idep-1) -
     &               2.0*ddepth_for_inpterpo    !up
...
! Very close to the bottom discontinuity. Set up depths as below
             if(  (r_freesurf-depth_solid(idep))
     &           -rmin_structure_zone(izone_idep_solid(idep)).lt. 
     &          2.0*ddepth_for_inpterpo+depth_tolerence) then
               depth_for_stress(3,idep)=depth_solid(idep) -
     &            2.0*ddepth_for_inpterpo !up
...
c               print *,'discon_stress_nodes_bot'
             else
c Sufficient space to set the target at the middle
c This setting up gives more accuate interpolated results
               depth_for_stress(3,idep)=depth_solid(idep) -
     &            ddepth_for_inpterpo  !up
               depth_for_stress(2,idep)=depth_solid(idep) !mid
               depth_for_stress(1,idep)=depth_solid(idep) +
...
c             if(igll.ne.1) stop 'Error of the depth table!'
           else 
              depth_for_stress(3,idep)= depth_solid(idep) -
     &               ddepth_for_inpterpo    !up
              depth_for_stress(2,idep)= depth_solid(idep) !mid
...
c***************************Eariler version************************
c       used in the earlier version of SEM-DSM. The distances to be
c       calculated are output from the SEM output.
c        do idep=1,ndep_solid
c           read(13,*) ndist_ela_tmp
c           if(idep.eq.2) ndist_ela=ndist_ela_tmp
c           itheta=0
c           do idist=1,ndist_ela_tmp
c             itheta=itheta+1
c             read(13,*) dist_elastic_tmp
c             if(idep.eq.2) dist_elastic(itheta)=dist_elastic_tmp*pi/180.0
c           end do
c        end do
c*********new version of genrating a generally used database*******
        read(13,*) ndist_ela_tmp
        ndist_ela=ndist_ela_tmp
        itheta=0
        do idist=1,ndist_ela_tmp
           itheta=itheta+1
           read(13,*) dist_elastic_tmp
	           if(dabs(dist_elastic_tmp).le.dist_ambig.or.
     &          dabs(180.d0-dist_elastic_tmp).le.
     &          dist_ambig) then
	              write(*,*) 'ERROR: ambiguous elastic distance.'
	              write(*,*) 'Distance (deg): ',dist_elastic_tmp
	              write(*,*) 'Threshold (deg): ',
     &             dist_ambig
	              write(*,*) 'Distances must be farther from 0 and ',
     &             '180 degrees.'
	              stop
	           end if
           dist_elastic(itheta)=dist_elastic_tmp*pi/180.0
        end do
        close(13)


        open( unit=14, file=dep_fluid_file, status='unknown' )
        read(14,*) ddepth_for_inpterpo,depth_tole_tmp
        depth_tolerence=depth_tole_tmp
...
!           if(igll.eq.0) igll=nwgll-1
           ...
           if(idep.eq.1) then
             if(rmax_structure_zone(izone_idep_fluid(idep))-
c Very close to the top discontinuity
     &          (r_freesurf-depth_fluid(idep)).le.
     &          2.0*ddepth_for_inpterpo+depth_tolerence) then
     ...
     &           2.0*ddepth_for_inpterpo  !down
c               print *,'discon_pressure_node'
C Sufficient space to set up the target depth at the middle
c This setting up gives more accuate interpolated results
             else
...
c Very close to the bottom discontinuity 
             if(dabs((r_freesurf-depth_fluid(idep))-
     &               rmin_structure_zone(izone_idep_fluid(idep)))
     &          .lt.2.0*ddepth_for_inpterpo+depth_tolerence) then
...
c This setting up gives more accuate interpolated results
     ...
     &               ddepth_for_inpterpo    !up
               ...
               depth_for_pressure(1,idep)=depth_fluid(idep) +
     &               ddepth_for_inpterpo  !down
c               print *,'normal_pressure_node_bot'
             end if
c            if(igll.ne.1) stop 'Error of the depth table (fluid)!'
           else
              depth_for_pressure(3,idep)=depth_fluid(idep)  -
     &               ddepth_for_inpterpo    !up
              depth_for_pressure(2,idep)=depth_fluid(idep) !mid
...
c Check the validaty
     ...
     &    stop 'Either ndep_fluid or ndep_solid should be positive'


        open( unit=15, file=dist_fluid_file, status='unknown' )
c***************************Eariler version************************
c       used in the earlier version of SEM-DSM. The distances to be
c       calculated come from the SEM output.
c        do idep=1,ndep_fluid
c           read(15,*) ndist_acou_tmp
c           if(idep.eq.2) then
c               ndist_acou=ndist_acou_tmp
c               if(ndist_acou.ne.ndist_ela.and.ndep_solid.gt.0) 
c     &           stop 'ndist_acou should be equal to ndist_ela'
c           end if
c           itheta=0
c           do idist=1,ndist_acou_tmp
c             itheta=itheta+1
c             read(15,*) dist_acoustic_tmp
c             if(idep.eq.2) then
c               dist_acoustic(itheta)=dist_acoustic_tmp*pi/180.0
c               if(dabs(dist_acoustic(itheta)-dist_elastic(itheta)).gt.
c     &            1.e-10.and.ndep_solid.gt.0) then
c                stop 'distance table of elastic and acoustic 
c     &                 should be  exactly the same'
c               end if
c             end if
c           end do
c        end do

c*********new version of generating generic Green's function database*********
        ...
        do idist=1,ndist_acou_tmp
          ...
          read(15,*) dist_acoustic_tmp
	          if(dabs(dist_acoustic_tmp).le.dist_ambig.or.
     &         dabs(180.d0-dist_acoustic_tmp).le.
     &         dist_ambig) then
	             write(*,*) 'ERROR: ambiguous acoustic distance.'
	             write(*,*) 'Distance (deg): ',dist_acoustic_tmp
	             write(*,*) 'Threshold (deg): ',
     &            dist_ambig
	             write(*,*) 'Distances must be farther from 0 and ',
     &            '180 degrees.'
...
c In this new version, the distance tables in the fluid and solid  
c media must be the same and see the reasons in the readme file.
          if(dabs(dist_acoustic(itheta)-dist_elastic(itheta)).gt.
     &            1.e-10.and.ndep_solid.gt.0) then
                stop 'distance table of elastic and acoustic media 
     &                 should be the same'
          end if
...
c     Thus, we set threshold depth at 20km and use average over the degree 'l' 
c     to reduce the oscillation errors for grids near source depth.
        if(Ddepth.lt.25.0) then
          print *,'WARNING!!!!!!!! The depth_cut_off is close to 
     &      depth_source (dif_depth<25km). The convergence 
     &      becomes slow (too many modes (l) get involved)
     ...
     &            that is not allowed in current version!'
        end if
...
c                  reduce the osillation around the source depth.
...
ccccc O      Even if the coefficient amplitude at station grids are
...
cccc         Even if the coefficient amplitude at station grids are
cccc         under threshold, we still solve it.
              if(itmp.gt.min_ir_dep) itmp=min_ir_dep
