...
c computing excitation vector for the given frequency.
...
c WENBO START
c The currently implemented source generates displacement seismograms 
c for a Delta function of moment release or velocity seismograms for 
c a Delta function of moment rate. To obtain displacement seismograms 
c for a Delta function of moment rate (which is the way an earthquake 
c releases moment), the source term must be integrated. 
c This integration can be performed by dividing `b1` and `b2` by i*omega 

c Why use a Delta function for moment release rather than the more
c reasonblae Delta function for moment rate, as the source time function? 
c A: This approach originates from the original DSM code.

c Just comment/uncomment lines 162-164/167-170 or vice versa
c to switch between these two kinds of sources
c	real*8 b1,b2,lsq2,lsq
	complex*16 b1,b2
        real*8 lsq2,lsq
c WENBO EDN
...
c **********************************************************************
c WENBO START
c Choice I - a Delta function of moment release
	b1 = dsqrt( dble(2*l+1) / ( 16.d0 * pi ) ) * dcmplx(1.0,0.0)
	b2 = dsqrt( dble(2*l+1)*dble(l-1)*dble(l+2) / ( 64.d0 * pi ) )*
     &        dcmplx(1.0,0.0)
c Choice II - a Delta function of moment rate (STF of an earthquake)
c b1 and b2 divided with i*omega
c	b1 = dsqrt( dble(2*l+1) / ( 16.d0 * pi ) )
c     &               /(dcmplx(0.0,1.0)*dble(omega))
...
c --- spheroidal excitations due to the traction diccontinuities
	...
	whole_vector_sph(idim_rs_sph+1,2)
     ...
     &	                  2.d0 * source_mt(2,3) )
     &	        / source_r
c --- toroidal excitations due to the traction discontinuities
...
c Just comment/uncomment lines 532/533-534 or vice versa
c to switch between these two kinds of sources
c       real*8 b1,b2,lsq2
        complex*16 b1,b2
        real*8 lsq2
...
c	endif
c Solving for all the station grids, even the ones whose amplitudes are below 
c the threshold (parameter eps in subroutine check_amp_significance)
c         nq=max0(end_grid-idim_station_sph+1, end_grid-init_grid+1+2)

c Wenbo Wu found a bug in the below line and corrected it on 2021/06/29
...
c	endif
c       Solving for all the station grids, even the ones whose amplitudes are 
c       below the threshold (parameter eps in subroutine check_amp_significance)
c         nq=max0(end_grid - init_grid + 1,end_grid - init_grid + 1+2)
...
c other variables
        ...
        do 100 itheta=1,ntheta 
         ...
         phi=0.0
c        To remove the oscillation errors for grids around the source 
c        depth, we take the average values within M periods.
c        The oscillation period of sum(c_lm*Ylm) (when l>>kr and k is the wavenumber) 
c        equals dl_onecycle=2*PI/theta (see Wu et al., 2018, GJI). Thus, we take
...
!        Too large dl_onecycle (small distance) or too small
!        nl_check_amp results in m_cycles=0
!        and nl_for_average=0. Change it to 1.   
         if(m_cycles.eq.0) nl_for_average=1
         if(nl_for_average.lt.0.or.nl_for_average.gt.nl_check_amp) then
           write(6,*) 'Error, nl_for_average <0 or >nl_check_amp'
           stop
         end if
         
         if(nl_for_average.lt.100.and.l.eq.1.and.itheta.eq.1.and.
     &      ifreq.eq.1) then
          write(6,*) "WARNING:nl_for_average<100!!!!!!! 
     &         Truncation error during removing oscillation errors 
     &          might be as large as 1/100. Possible reasons include:
...
c        Some variables, such as Ylm, only need to be computed one time
c        (i.e., when idep=1; see sub-subfuction comp_disp_strain), 
c        Thus, solid and fluid media are packaged in one loop instead of two loops.
...
c              depth=min_dep_solid+(idep_new-1)*ddep
     ...
     &           nl_for_average)
c          Convegence test
c           if(ifreq.eq.50.and.m.eq.0.and.idep_new.eq.1.and.itheta.eq.1) print
c          if(ifreq.eq.10.and.idep.eq.1.and.itheta.eq.1)
c     &     print *,'check_disp_freq10',
c     &          l,m,ifreq,coef_c_lm(1),coef_c_lm(2),
c     &          coef_c_lm(3),ur,theta
...
c The variables declared at lines 2557-22558 are only computed one time
        if(idep.eq.1) then
         factor(m) = 1.d0
...
!        using the three depths' results to get interpolated c1 at the target depth
         c_input=dcmplx(0.d0,0.d0)
!        Note that only c1 is used in fluid media, so c_put(i),
!        where i=2,3,5,6,8,9, are zeros or not used here.
         c_input(1)=whole_vector_sph(idim_up_pressure,m)
         r_input(1)=grid_r(ir_up_pressure)
         c_input(4)=whole_vector_sph(idim_mid_pressure,m)
         r_input(2)=grid_r(ir_mid_pressure)
         c_input(7)=whole_vector_sph(idim_down_pressure,m)
         r_input(3)=grid_r(ir_down_pressure)
...
c***********************************************************************
         ...
         do i=1,3
          do icomp=1,ncomp
c The most up grid point between ir,ir_up,ir_down
            if(i.eq.1) then
...
c Calculate the coefficients for stresses of the top and/or bottom boundary
        end if
...
c For convergence test
c Print out for testing (r/r_source)**l decay
c        if(idep.eq.1.and.(l.eq.12000.or.l.eq.4000.or.l.eq.8000
c     &     .or.l.eq.16000.or.l.eq.1000).and.m.eq.0.and.ifreq.eq.50) then
c          do i=ngrid_r-500,ngrid_r,2
c          do i=54227,54527,3
c           print *,'coefficient=',
c     & abs(whole_vector_sph(idim_ir_sph(i),0)),l,grid_r(i),
c     &    idim_ir_sph(i)
c          end do
c        end if
...
c         if(ir_up_pressure.eq.ir_CMB) then
c           rho_up=grid_rho(2,ir_up_pressure-1)
c         else
c           rho_up=grid_rho(1,ir_up_pressure)
c         end if

c         if(ir_mid_pressure.eq.ir_CMB) then
c           rho_mid=grid_rho(2,ir_mid_pressure-1)
c         else
c           rho_mid=grid_rho(1,ir_mid_pressure)
c         end if


c         if(ir_down_pressure.eq.ir_CMB) then
c           rho_down=grid_rho(2,ir_down_pressure-1)
c         else
c           rho_down=grid_rho(1,ir_down_pressure)
c         end if

c        use the three depths' results to get interpolated c1 at the
c        target depth
         c_input=dcmplx(0.d0,0.d0)
c         Note that only c1 is used in fluid media, so c_put(i),
c         where i=2,3,5,6,8,9, are zeros or not used here.
         c_input(1)=whole_vector_sph(idim_up_pressure,m)
         r_input(1)=grid_r(ir_up_pressure)
         c_input(4)=whole_vector_sph(idim_mid_pressure,m)
         r_input(2)=grid_r(ir_mid_pressure)
...
c            print *,'ir_down_stress',ir_down_stress,idep_new
         ...
         c_input=dcmplx(0.d0,0.d0)
         do i=1,3
c         The upper-most grid point between ir,ir_up,ir_down
...
c ***********************Top or bottome boundary***********
c Calculate the coefficients for stresses of the top and/or bottom
c boundary
        coef_c_lm(1)=c1
