...
c       --- Initial tests shows that nl_check_amp>400 works well
	...
	parameter ( maxnfreq = 200 )
		real*8 pi,dist_ambiguity_tole_deg
		parameter ( pi=3.1415926535897932d0 )
		parameter ( dist_ambiguity_tole_deg=1.d-2 )
...
c variables for numerical grids
	...
	integer ngrid_r_estimated,ngrid_r,lmin,lmax,l
        integer lmax_computed_ifreq,lmax_allfreq_iproc,lmax_allfreq,ierr
...
c WENBO
	if ( rank.eq.0 ) then
     ...
     &	    max_ndep,max_ntheta,dist_ambiguity_tole_deg,
...
!WENBO
     ...
     &            min_ir_dep)
                  ...
                  lmax_computed_ifreq=l
                  if(lmax_computed_ifreq>lmax_allfreq_iproc)
     &                 lmax_allfreq_iproc=lmax_computed_ifreq
c                 if(mod(l,2000).eq.0) print *,'l=',l,'ifreq=',i_frequency
	        endif
	      endif
  110	    continue
	   endif
...
c **********************************************************************
     ...
     &     source_type,top_fluid,bot_fluid,lmax_computed_ifreq,
...
c	endif
        ...
        call mpi_reduce(lmax_allfreq_iproc,lmax_allfreq,1,mpi_integer,
