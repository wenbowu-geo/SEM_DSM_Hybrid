...
c     & depth_for_stress(1,idep),izone_idep_solid(idep),
c     & depth_for_stress(2,idep),depth_for_stress(3,idep)
                 stop 'Error, r_for_stress is not in the same 
     &                  izone as the input depth(solid)'
           end if
...
c                 print *,'ir_dep_stress_find',idep,grid_r(ir_add_node)
                 stop 'Error, r_for_stress is not in the same 
     &                  izone as the input depth(fluid)'
           end if
     ...
     &             ir_bot_stress,ir_top_pressure,ir_bot_pressure)
        ...
        if(bot_fluid.eq.0) then
          if(ndep_solid.eq.1.and.top_fluid.eq.0) then
            do i=1,3
              r_bot_stress(i)=r_top_stress(i)
              ir_bot_stress(i)=ir_top_stress(i)
            end do
          else
