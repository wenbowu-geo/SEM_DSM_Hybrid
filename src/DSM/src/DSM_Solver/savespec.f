...
! Save displacement
        if(save_velo.eq.1) then
          write(outfile_disp,
     &     "('./OUTPUT_FILES/disp_solid/freq_',i5.5)")
     &                  i_frequency
c          outfile_disp=outfile_disp//"_disp"
...
! Save velocity
          ...
          write(outfile_velo,
     &     "('./OUTPUT_FILES/velo_solid/freq_',i5.5)")
     &                  i_frequency
 ...
 1001        continue
  ...
  901     continue
        ...
        write(outfile_disp,
     &     "('./OUTPUT_FILES/disp_fluid/freq_',i5.5)")
     &                  i_frequency
c        outfile_disp=outfile_disp//"_disp"
        open(unit=16,file=trim(outfile_disp),status='unknown',
     &       form='unformatted',iostat=ios)
 ...
 1110   continue
     ...
     &      "('./OUTPUT_FILES/stress/freq_',i5.5)")
     &                    i_frequency
c          outfile_stress=outfile_stress//"_stress"
...
c               print *,'izone',izone_used,izone1,izone2
...
!Gpa to pa
     ...
     &    "('./OUTPUT_FILES/pressure/freq_',i5.5)")
     &                  i_frequency
...
c       avoid to devide zero
     ...
     &    "('./OUTPUT_FILES/potential/freq_',i5.5)")
     &                  i_frequency
...
!save the coefficients c and dcdr
     ...
     &    "('./OUTPUT_FILES/coef_cAnddcdr/freq_',i5.5)")
     &                  i_frequency
...
c other variables
        ...
        real*8 pi,km,gcm3_kgm3
        parameter ( pi=3.1415926535897932d0 )
        parameter ( km=1000.0 )
        parameter ( gcm3_kgm3=1000.0 )
        logical output_more
        parameter (output_more=.FALSE.)
...
c        integer nl_eachpack
          ...
          outfile_par="./OUTPUT_FILES/DSM_Par_file"
          open(unit=18,file=trim(outfile_par),status='unknown',
     &       form='formatted',iostat=ios)
          if(ios / = 0) stop 'Error in openning file DSM_Par_file'
          write(18,*)n_frequency
          write(18,*)ndep_fluid,ndep_solid
...
c******************************Start*******************************
c***save more parameters for earlier version of DSM-SEM coupling***
c******************************************************************
c The whole below chunk was used in the earlier version of 
c receiver-side and mid-way couplings. In that version, the 
c injection elements are classified into two types of 
c top/bottom and middle depths. 
          if(output_more) then
            outfile_par="./OUTPUT_FILES/topbot_forResave"
...
c Including l=0, so it is lmax_allfreq+1
            ...
            outfile_par="./OUTPUT_FILES/Partop_forSpecCoefToTime"
     ...
     &       form='formatted',iostat=ios)
              ...
              if(source_type.eq.1) then
                  ...
                  write(20,*) lmax_allfreq,'nl_eachpack',1
              end if
            ...
            outfile_par="./OUTPUT_FILES/matetop_forSpecCoefToTime"
            open(unit=21,file=trim(outfile_par),status='unknown',
     &       form='formatted',iostat=ios)
            if(top_fluid.eq.1) then
              write(21,*) grid_r(ir_dep_fluid(1))*km
              write(21,*) grid_rho(1,ir_dep_fluid(1))*gcm3_kgm3
            else
              write(21,*) grid_r(ir_dep_solid(1))*km
              write(21,*) Atop
              write(21,*) Ctop
              write(21,*) Ftop
              write(21,*) Ltop
            ...
            outfile_par="./OUTPUT_FILES/Parbot_forSpecCoefToTime"
     ...
     &       form='formatted',iostat=ios)
              ...
              if(source_type.eq.1) then
                  ...
                  write(22,*) lmax_allfreq,'nl_eachpack',1
              end if
            ...
            outfile_par="./OUTPUT_FILES/matebot_forSpecCoefToTime"
            open(unit=23,file=trim(outfile_par),status='unknown',
     &       form='formatted',iostat=ios)
            if(bot_fluid.eq.1) then
              write(23,*) grid_r(ir_dep_fluid(ndep_fluid))*km
              write(23,*) grid_rho(1,ir_dep_fluid(ndep_fluid))*gcm3_kgm3
            else
              write(23,*) grid_r(ir_dep_solid(ndep_solid))*km
              write(23,*) Abot
              write(23,*) Cbot
              write(23,*) Fbot
              write(23,*) Lbot
           ...
           if(ndep_fluid.gt.0) then
            ...
            outfile_par="./OUTPUT_FILES/ParChidot_forSpectotimeEdge"
     ...
     &       form='formatted',iostat=ios)
            ...
            outfile_par="./OUTPUT_FILES/ParDispfluid_forSpectotimeEdge"
     ...
     &       form='formatted',iostat=ios)
           ...
           if(ndep_solid.gt.0) then
            ...
            outfile_par="./OUTPUT_FILES/ParStress_forSpectotimeEdge"
     ...
     &       form='formatted',iostat=ios)
            ...
            outfile_par="./OUTPUT_FILES/ParVelosolid_forSpectotimeEdge"
