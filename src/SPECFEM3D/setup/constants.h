...
!WENBO
!for normalized dimension
!  double precision, parameter :: SMALLVAL_TOL = 1.d-10
! for real earth radius
  double precision, parameter :: SMALLVAL_TOL = 1.d-5
...
!WENBO_S
!DDEPTH_FOR_INTERPO_DSM (km) and DEPTH_TOLERENCE_DSM (km) are used in the DSM running, so you can
!manually change the below two parameters in the files 'depth_table_elastic' or 
!'depth_table_acoustic' after the generate database step and run DSM, if you are sure what 
!you are doing is reasonable (i.e. work on very high frequency). The depth spacing used to 
!add radial nodes in DSM if any injected depth is located at the discontinuity.
double precision, parameter :: DDEPTH_FOR_INTERPO_DSM = 0.04
!The depths collapse to one depth if their difference is smaller than the below torerence.
double precision, parameter :: DEPTH_TOLERENCE_DSM = 0.01

!radius of the earth
double precision, parameter :: R_EARTH_SURF=6371000.0

  integer, parameter ::MAX_NDEPTH_TABLE=10000
! points used to do interpolation
...
!Maybe there is better way to deal with it.
  integer, parameter:: NDECIMAL_REMAINED=2

!debuging mode for DSM-SEM coupling
...
!WENBO-S
  integer, parameter :: IMODEL_DSM1D            = 13
  integer, parameter :: IMODEL_DSM3D            = 14
!WENBO-E
