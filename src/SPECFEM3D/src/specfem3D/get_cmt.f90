...
! read source number isource
  do isource = 1,NSOURCES
    ...
    ! explosion check for SINGLE_FORCE_ENZ = -1
    if (SINGLE_FORCE_ENZ == -1) then
      if (abs(moment_tensor(1,isource) - moment_tensor(2,isource)) > 1.d-6 * abs(moment_tensor(1,isource)) .or. &
          abs(moment_tensor(2,isource) - moment_tensor(3,isource)) > 1.d-6 * abs(moment_tensor(2,isource)) .or. &
          abs(moment_tensor(1,isource)) < 1.d-20 .or. &
          abs(moment_tensor(4,isource)) > 1.d-20 .or. &
          abs(moment_tensor(5,isource)) > 1.d-20 .or. &
          abs(moment_tensor(6,isource)) > 1.d-20) then
        print *, "Error: SINGLE_FORCE_ENZ = -1 requires an explosion source."
        print *, "In CMTSOLUTION, Mrr, Mtt, Mpp must be non-zero and equal,"
        print *, "and Mrt, Mrp, Mtp must be zero."
        print *, "Values read:", moment_tensor(:,isource)
        stop "CMTSOLUTION mismatch for explosion source"
      endif
    endif
