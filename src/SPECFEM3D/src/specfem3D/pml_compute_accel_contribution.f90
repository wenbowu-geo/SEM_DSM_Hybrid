...
!WENBO_S
  use specfem_par, only: NGLOB_AB,deltat,wgll_cube,jacobian,ibool,rhostore,CUBED_SPHERE_PROJECTION
!WENBO_E
  use pml_par, only: CPML_regions,d_store_x,d_store_y,d_store_z,K_store_x,K_store_y,K_store_z, &
                     alpha_store_x, alpha_store_y, alpha_store_z, &
                     NSPEC_CPML,accel_elastic_CPML,PML_displ_old,PML_displ_new,pml_rotation_matrix
...
!WENBO_S
  integer ::icomp,jcomp
  real(kind=CUSTOM_REAL),dimension(NDIM,NDIM) ::rotation_matrix
  real(kind=CUSTOM_REAL),dimension(NDIM) :: veloc_cubedsph,displ_cubedsph,&
                           accel_elastic_CPML_cubedsph
  real(kind=CUSTOM_REAL),dimension(NDIM) :: PML_displ_new_cubedsph,PML_displ_old_cubedsph
!WENBO_E
...
!WENBO_S
!All the above parameters for PML are based on cubed_sphere coordinates, 
!so the displacement in global coordinates need to be transformed to 
!cubed_sphere coordinates.
        if(CUBED_SPHERE_PROJECTION) then
         ! updates memory variables
         do icomp=1,NDIM
             PML_displ_new_cubedsph(icomp)=0._CUSTOM_REAL
             PML_displ_old_cubedsph(icomp)=0._CUSTOM_REAL
             veloc_cubedsph(icomp)=0._CUSTOM_REAL
             displ_cubedsph(icomp)=0._CUSTOM_REAL
           do jcomp=1,NDIM
!From global coordinates to cubed_sphere coordinates.
!It is a_cubedsph(i)=a_cubedsph(i)+a_globalcoord(j)*rotation_matrix(j,i), rather
!than  a_cubedsph(i)=a_cubedsph(i)+a_globalcoord(j)*rotation_matrix(i,j).
              PML_displ_new_cubedsph(icomp)=PML_displ_new_cubedsph(icomp)+PML_displ_new(jcomp,i,j,k,ispec_CPML)*&
                            rotation_matrix(jcomp,icomp)
              PML_displ_old_cubedsph(icomp)=PML_displ_old_cubedsph(icomp)+PML_displ_new(jcomp,i,j,k,ispec_CPML)*&
                            rotation_matrix(jcomp,icomp)
              veloc_cubedsph(icomp)=veloc_cubedsph(icomp)+veloc(jcomp,iglob)*&
                            rotation_matrix(jcomp,icomp)
              displ_cubedsph(icomp)=displ_cubedsph(icomp)+displ(jcomp,iglob)*&
                            rotation_matrix(jcomp,icomp)
           end do
         end do
         rmemory_displ_elastic(1,i,j,k,ispec_CPML,1) = coef0_x * rmemory_displ_elastic(1,i,j,k,ispec_CPML,1) &
                + PML_displ_new_cubedsph(1) * coef1_x + PML_displ_old_cubedsph(1) * coef2_x
         rmemory_displ_elastic(2,i,j,k,ispec_CPML,1) = coef0_x * rmemory_displ_elastic(2,i,j,k,ispec_CPML,1) &
                + PML_displ_new_cubedsph(2) * coef1_x + PML_displ_old_cubedsph(2) * coef2_x
         rmemory_displ_elastic(3,i,j,k,ispec_CPML,1) = coef0_x * rmemory_displ_elastic(3,i,j,k,ispec_CPML,1) &
                + PML_displ_new_cubedsph(3) * coef1_x + PML_displ_old_cubedsph(3) * coef2_x

         rmemory_displ_elastic(1,i,j,k,ispec_CPML,2) = coef0_y * rmemory_displ_elastic(1,i,j,k,ispec_CPML,2) &
                + PML_displ_new_cubedsph(1) * coef1_y + PML_displ_old_cubedsph(1) * coef2_y 
         rmemory_displ_elastic(2,i,j,k,ispec_CPML,2) = coef0_y * rmemory_displ_elastic(2,i,j,k,ispec_CPML,2) &
                + PML_displ_new_cubedsph(2) * coef1_y + PML_displ_old_cubedsph(2) * coef2_y 
         rmemory_displ_elastic(3,i,j,k,ispec_CPML,2) = coef0_y * rmemory_displ_elastic(3,i,j,k,ispec_CPML,2) &
                + PML_displ_new_cubedsph(3) * coef1_y + PML_displ_old_cubedsph(3) * coef2_y 

         rmemory_displ_elastic(1,i,j,k,ispec_CPML,3) = coef0_z * rmemory_displ_elastic(1,i,j,k,ispec_CPML,3) &
                + PML_displ_new_cubedsph(1) * coef1_z + PML_displ_old_cubedsph(1) * coef2_z 
         rmemory_displ_elastic(2,i,j,k,ispec_CPML,3) = coef0_z * rmemory_displ_elastic(2,i,j,k,ispec_CPML,3) &
                + PML_displ_new_cubedsph(2) * coef1_z + PML_displ_old_cubedsph(2) * coef2_z 
         rmemory_displ_elastic(3,i,j,k,ispec_CPML,3) = coef0_z * rmemory_displ_elastic(3,i,j,k,ispec_CPML,3) &
                + PML_displ_new_cubedsph(3) * coef1_z + PML_displ_old_cubedsph(3) * coef2_z 

         ! updates PML acceleration
         accel_elastic_CPML_cubedsph(1) =  wgllcube * rhol * jacobianl * &
             ( A_1 * veloc_cubedsph(1) + A_2 * displ_cubedsph(1) + &
         ...
         accel_elastic_CPML_cubedsph(2) =  wgllcube * rhol * jacobianl * &
             ( A_1 * veloc_cubedsph(2) + A_2 * displ_cubedsph(2) + &
         ...
         accel_elastic_CPML_cubedsph(3) =  wgllcube * rhol * jacobianl * &
             ( A_1 * veloc_cubedsph(3) + A_2 * displ_cubedsph(3) + &
         ...
         do icomp=1,NDIM
           accel_elastic_CPML(icomp,i,j,k) =0._CUSTOM_REAL
           do jcomp=1,NDIM
!From global coordinates to cubed_sphere coordinates.
!It is a_globalcoord(i)=a_globalcoord(i)+a_cubedsph(j)*rotation_matrix(i,j), rather
!than  a_globalcoord(i)=a_globalcoord(i)+a_cubedsph(j)*rotation_matrix(j,i).
              accel_elastic_CPML(icomp,i,j,k) = accel_elastic_CPML(icomp,i,j,k)+&
                     accel_elastic_CPML_cubedsph(jcomp)*rotation_matrix(icomp,jcomp)
           end do
         end do

        else
!WENBO_E
!not CUBED_SPHERE_PROJECTION
         ! updates memory variables
         rmemory_displ_elastic(1,i,j,k,ispec_CPML,1) = coef0_x * rmemory_displ_elastic(1,i,j,k,ispec_CPML,1) &
