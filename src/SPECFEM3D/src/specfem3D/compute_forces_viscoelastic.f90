...
subroutine compute_forces_viscoelastic(iphase, &
                        ...
                        phase_ispec_inner_elastic,backward_simulation,iele_strain_saved_ispec,&
                        strain_saved,save_strain,LOW_RESOLUTION,NSTEP_BETWEEN_OUTPUTBOUND,&
                        npoints_strain_saved,COUPLING_TYPE,SINGLE_FORCE_ENZ,it,DECIMATE_COUPLING)

  use constants, only: CUSTOM_REAL,NGLLX,NGLLY,NGLLZ,NDIM,N_SLS,ONE_THIRD,FOUR_THIRDS

  use fault_solver_dynamic, only: Kelvin_Voigt_eta

  use specfem_par, only: SAVE_MOHO_MESH,USE_LDDRK,myrank
  ...
  !WENBO added these variables
  integer ::NSTEP_BETWEEN_OUTPUTBOUND,npoints_strain_saved
  logical, dimension(NGLOB_AB) ::save_strain
  integer, dimension(NGLOB_AB):: iele_strain_saved_ispec
  real(kind=CUSTOM_REAL), dimension(NSTEP_BETWEEN_OUTPUTBOUND,6,npoints_strain_saved) ::strain_saved
  logical ::LOW_RESOLUTION
  integer ::COUPLING_TYPE,SINGLE_FORCE_ENZ,it,DECIMATE_COUPLING
...
!WENBO
  ! locatal saving strain parameters
  integer istrain_saved,it_resampled,it_strain_saved
  logical save_strain_this_it
!WENBO_S
  logical ::logical_tmp
!The below line is used to avoid possible error reports during compling
!the code. PML_CONDITIONS is not used now, that occationally causes error 
!reports. But it might be usful in the future, so we keep them here.
  logical_tmp=PML_CONDITIONS


  if(COUPLING_TYPE.eq.2.and.SINGLE_FORCE_ENZ.gt.0.and. &
     (mod(it,DECIMATE_COUPLING).eq.1.or. DECIMATE_COUPLING.eq.1) ) then
!     print *,'check-DECIMATE_COUPLING',DECIMATE_COUPLING
     save_strain_this_it=.true.
     it_resampled=(it-1)/DECIMATE_COUPLING+1
     it_strain_saved=mod(it_resampled,NSTEP_BETWEEN_OUTPUTBOUND)
     if(it_strain_saved.eq.0) it_strain_saved=NSTEP_BETWEEN_OUTPUTBOUND
  else
     save_strain_this_it=.false.
  end if
...
!WENBO_B
!WENBO added the below lines for receiver-side coupling
          if(save_strain_this_it.and.save_strain(ispec)) then
             if(LOW_RESOLUTION.and.i.eq.(NGLLX+1)/2.and.j.eq.(NGLLY+1)/2.and.k.eq.(NGLLZ+1)/2) then
                istrain_saved=iele_strain_saved_ispec(ispec)

                strain_saved(it_strain_saved,1,istrain_saved)=duxdxl
                strain_saved(it_strain_saved,2,istrain_saved)=duydyl
                strain_saved(it_strain_saved,3,istrain_saved)=duzdzl
                strain_saved(it_strain_saved,4,istrain_saved)=0.5*(duxdyl+duydxl)
                strain_saved(it_strain_saved,5,istrain_saved)=0.5*(duxdzl+duzdxl)
                strain_saved(it_strain_saved,6,istrain_saved)=0.5*(duzdyl+duydzl)
                if(myrank.eq.0) then
                   print *,'strain_saved_rank0',ispec,istrain_saved,it,strain_saved(it_strain_saved,1,istrain_saved)
                end if

             end if
             if(.not.LOW_RESOLUTION.and.i.ne.1.and.j.ne.1.and.k.ne.1) then
                istrain_saved=(iele_strain_saved_ispec(ispec)-1)*(NGLLX-1)*(NGLLY-1)*(NGLLZ-1)+&
                         (k-2)*(NGLLX-1)*(NGLLY-1)+(j-2)*(NGLLX-1)+(i-1)
                if(istrain_saved.gt.npoints_strain_saved) &
                       print *,'Error_istrain_saved',myrank,ispec,iele_strain_saved_ispec(ispec)
                strain_saved(it_strain_saved,1,istrain_saved)=duxdxl
                strain_saved(it_strain_saved,2,istrain_saved)=duydyl
                strain_saved(it_strain_saved,3,istrain_saved)=duzdzl
                strain_saved(it_strain_saved,4,istrain_saved)=0.5*(duxdyl+duydxl)
