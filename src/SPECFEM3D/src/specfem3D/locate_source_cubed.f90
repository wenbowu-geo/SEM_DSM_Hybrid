...
!WENBO - Sometimes, especially when a large number of CPU processors are used, 
!segmentation fault is raised when run this subroutine.  
!Use a specified number, instead of NRPOC, to explicitly define the array dimention can solve this problem, 
!although I still do not know why.
...
!   integer, dimension(NGATHER_SOURCES,0:899) :: ispec_selected_source_all
!   double precision, dimension(NGATHER_SOURCES,0:899) :: xi_source_all,eta_source_all,gamma_source_all, &
!      final_distance_source_all,x_found_source_all,y_found_source_all,z_found_source_all
!   double precision, dimension(3,3,NGATHER_SOURCES,0:899) :: nu_source_all
...
!    endif
    ...
    if( ispec_selected_source(isource) == 0 ) then
      idomain(isource) = 0
    else if( ispec_is_acoustic( ispec_selected_source(isource) ) ) then
