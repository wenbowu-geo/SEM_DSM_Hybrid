...
!! DK DK put this because I do not know how to fix the rules.mk dependencies
  ...
  use constants, only: CUSTOM_REAL,MAX_STRING_LEN,NGNOD2D_FOUR_CORNERS_AVS_DX, &
    NGLLX,NGLLY,NGLLSQUARE,IIN,IOUT,OUTPUT_FILES,TINYVAL
...
!WENBO change the utm to cubed-sphere projection
...
!              write(11,*) sngl(long),sngl(lat),sngl(field_display(ilocnum+ieoff))
                  !     UTM_PROJECTION_ZONE,IUTM2LONGLAT,SUPPRESS_UTM_PROJECTION)
                   call xyz2latlon(xp_save(ilocnum+ieoff),yp_save(ilocnum+ieoff),zp_save(ilocnum+ieoff),lat,long)
                  write(11,'(4f16.6)') long,lat,field_display(ilocnum+ieoff),&
                               dsqrt(xp_save(ilocnum+ieoff)**2+yp_save(ilocnum+ieoff)**2+zp_save(ilocnum+ieoff)**2)
            endif
