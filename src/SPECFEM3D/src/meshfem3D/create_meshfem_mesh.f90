...
!WENBO_S
  ! create different regions of the mesh
  use constants, only: MF_IN_DATA_FILES,MAX_STRING_LEN,NGNOD_EIGHT_CORNERS,IMAIN,IOVTK, &
                       CUSTOM_REAL,R_EARTH_SURF,SMALLVAL_TOL
!WENBO_E
...
!WENBO_S
  use coupling_mesh_par, only: R_TOP_BOUND,NXI_NO_TOPOGRAPHY,NETA_NO_TOPOGRAPHY,&
   coupling_nspec_xlow,coupling_nspec_xhigh,coupling_nspec_ylow,coupling_nspec_yhigh,coupling_nspec_rtop,coupling_nspec_rbottom,&
   coupling_ispec_xlow,coupling_isubregion_xlow,coupling_ispec_xhigh,coupling_isubregion_xhigh,&
   coupling_ispec_ylow,coupling_isubregion_ylow,coupling_ispec_yhigh,coupling_isubregion_yhigh,&
   coupling_ispec_rtop,coupling_isubregion_rtop,coupling_ispec_rbottom,coupling_isubregion_rbottom,Imain_coupling,&
   rotation_matrix,ANGULAR_WIDTH_XI_IN_DEGREES,ANGULAR_WIDTH_ETA_IN_DEGREES,&
   COUPLING_IXI_LOW,COUPLING_IXI_HIGH,COUPLING_IETA_LOW,COUPLING_IETA_HIGH,COUPLING_IR_TOP,COUPLING_IR_BOTTOM,&
   xstore_cubedsph,ystore_cubedsph,zstore_cubedsph,nodes_coords_cubedsph,&
   nodes_coords_cubedsph_old
!WENBO_E
...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION) then
    allocate(xstore_cubedsph(NGLLX_M,NGLLY_M,NGLLZ_M,nspec),stat=ier)
    if (ier /= 0) stop 'Error allocating array xstore_cubedsph'
    allocate(ystore_cubedsph(NGLLX_M,NGLLY_M,NGLLZ_M,nspec),stat=ier)
    if (ier /= 0) stop 'Error allocating array ystore_cubedsph'
    allocate(zstore_cubedsph(NGLLX_M,NGLLY_M,NGLLZ_M,nspec),stat=ier)
    ! exit if there is not enough memory to allocate all the arrays
    if (ier /= 0) call exit_MPI(myrank,'not enough memory to allocate arrays')
  end if
!WENBO_E
...
!WENBO_S
    coupling_ixlow=0;coupling_ixhigh=0;coupling_iylow=0;coupling_iyhigh=0;coupling_irtop=0;coupling_irbottom=0
!WENBO_E
...
!WENBO_S
            if(CUBED_SPHERE_PROJECTION) then
                !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ! Reclassify the element as either fluid or elastic type based on its position 
                ! relative to the local topography or bathymetry.
                ! Note: The ocean layer is present only when the SEM box includes the top free
                ! surface, i.e., when R_TOP_BOUND = R_EARTH_SURF.
                if(abs(R_TOP_BOUND-R_EARTH_SURF)<SMALLVAL_TOL) then
                  call determine_ocean_land(xelm,yelm,zelm,imaterial_number,&
                       isubregion,nmeshregions,ir,ir1,ir2,iy,iy1,iy2,ix,ix1,ix2,&
                       iproc_xi_current,NPROC_XI,iproc_eta_current,NPROC_ETA,&
                       NXI_NO_TOPOGRAPHY,NETA_NO_TOPOGRAPHY)
                  ispec_material_id(ispec) =imaterial_number
                end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

                ! detect mesh boundaries
                call get_flags_boundaries_cubed(nspec,iproc_xi_current,iproc_eta_current,ispec,doubling_index, &
                ...
                call store_coords(xstore_cubedsph,ystore_cubedsph,zstore_cubedsph,xelm,yelm,zelm,ispec,nspec)
                call cubed_geo(xelm,yelm,zelm,R_TOP_BOUND,ANGULAR_WIDTH_XI_IN_DEGREES,&
                       ANGULAR_WIDTH_ETA_IN_DEGREES,rotation_matrix)
                  ! store coordinates
                call store_coords(xstore,ystore,zstore,xelm,yelm,zelm,ispec,nspec)
                call find_coupling_elements(COUPLING_IXI_LOW,COUPLING_IXI_HIGH,COUPLING_IETA_LOW, &
                       COUPLING_IETA_HIGH,COUPLING_IR_TOP,COUPLING_IR_BOTTOM,coupling_nspec_xlow,&
                       coupling_ixlow,coupling_ispec_xlow,coupling_isubregion_xlow,&
                       coupling_nspec_xhigh,coupling_ixhigh,coupling_ispec_xhigh,coupling_isubregion_xhigh,coupling_nspec_ylow,&
                       coupling_iylow,coupling_ispec_ylow,coupling_isubregion_ylow,&
                       coupling_nspec_yhigh,coupling_iyhigh,coupling_ispec_yhigh,&
                       coupling_isubregion_yhigh,coupling_nspec_rtop,coupling_irtop,coupling_ispec_rtop,coupling_isubregion_rtop,&
                       coupling_nspec_rbottom,coupling_irbottom,coupling_ispec_rbottom,coupling_isubregion_rbottom,ixele,&
                       iyele,irele,isubregion,ispec_superbrick,ispec,iproc_xi_current,iproc_eta_current,&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                   ! Reclassify the element as either fluid or elastic type based
                   ! on its position 
                   ! relative to the local topography or bathymetry.
                   ! Note: The ocean layer is present only when the SEM box
                   ! includes the top free
                   ! surface, i.e., when R_TOP_BOUND = R_EARTH_SURF.
                   if(abs(R_TOP_BOUND-R_EARTH_SURF)<SMALLVAL_TOL) then
                       call determine_ocean_land(xelm,yelm,zelm,imaterial_number,&
                         isubregion,nmeshregions,ir,ir1,ir2,iy,iy1,iy2,ix,ix1,ix2,&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    call find_coupling_elements(COUPLING_IXI_LOW,COUPLING_IXI_HIGH,COUPLING_IETA_LOW,&
                           ...
                           coupling_isubregion_xhigh,coupling_nspec_ylow,&
                           coupling_iylow,coupling_ispec_ylow,coupling_isubregion_ylow,coupling_nspec_yhigh,&
                           coupling_iyhigh,coupling_ispec_yhigh,coupling_isubregion_yhigh,coupling_nspec_rtop,&
                           coupling_irtop,coupling_ispec_rtop,coupling_isubregion_rtop,&
                           coupling_nspec_rbottom,coupling_irbottom,coupling_ispec_rbottom,coupling_isubregion_rbottom,ixele,&
                           iyele,irele,isubregion,ispec_superbrick,ispec,iproc_xi_current,iproc_eta_current,&
                           NEX_PER_PROC_XI,NEX_PER_PROC_ETA,NER,USE_REGULAR_MESH,NDOUBLINGS,myrank)
                else 
...
!WENBO_S
  if(coupling_ixlow.ne.coupling_nspec_xlow.or.coupling_ixhigh.ne.coupling_nspec_xhigh.or. &
     coupling_iylow.ne.coupling_nspec_ylow.or.coupling_iyhigh.ne.coupling_nspec_yhigh.or.&
     coupling_irtop.ne.coupling_nspec_rtop.or.coupling_irbottom.ne.coupling_nspec_rbottom) then
         print *,'TeleEle',coupling_ixlow,coupling_nspec_xlow,coupling_ixhigh,coupling_nspec_xhigh,&
                 coupling_iylow,coupling_nspec_ylow &
                ,coupling_iyhigh,coupling_nspec_yhigh,coupling_irtop,coupling_irbottom,&
                 coupling_nspec_rtop,coupling_nspec_rbottom,&
                'nrele',COUPLING_IXI_LOW,COUPLING_IXI_HIGH,COUPLING_IETA_LOW,&
                COUPLING_IETA_HIGH,COUPLING_IR_TOP,COUPLING_IR_BOTTOM,myrank
         call exit_MPI(myrank,'TeleEle_i(x,y,r) must equal TeleEle_n(x,y,r)')
  end if
!WENBO_E
  ...
  if(CUBED_SPHERE_PROJECTION) then
      if(myrank.eq.0)   write (Imain_coupling,*) 'save the boundary elements information'
      call save_coupling_element(coupling_nspec_xlow,coupling_ispec_xlow,coupling_isubregion_xlow,&
                ...
                coupling_nspec_rbottom,coupling_ispec_rbottom,coupling_isubregion_rbottom,LOCAL_PATH,myrank)

      if(myrank.eq.0)  close(Imain_coupling)
  end if
...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION) then
   allocate(nodes_coords_cubedsph(nglob,3),stat=ier)
   if (ier /= 0) stop 'Error allocating array nodes_coords_cubedsph'
   nodes_coords_cubedsph(:,:) = 0.0d0
  end if
!WENBO_S
...
!WENBO_S
          if(CUBED_SPHERE_PROJECTION) then
            nodes_coords_cubedsph(iglob(ilocnum+ieoff),1) = xstore_cubedsph(i,j,k,ispec)
            nodes_coords_cubedsph(iglob(ilocnum+ieoff),2) = ystore_cubedsph(i,j,k,ispec)
            nodes_coords_cubedsph(iglob(ilocnum+ieoff),3) = zstore_cubedsph(i,j,k,ispec)
          end if
!WENBO_E
        enddo
      enddo
...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION) then
     deallocate(xstore_cubedsph,ystore_cubedsph,zstore_cubedsph)
  end if
!WENBO_E
...
!WENBO_S
    if(CUBED_SPHERE_PROJECTION) then
      allocate(nodes_coords_cubedsph_old(nglob_old,3))
      nodes_coords_cubedsph_old=nodes_coords_cubedsph
      deallocate(nodes_coords_cubedsph)
      allocate(nodes_coords_cubedsph(nglob,3))
    end if
!WENBO_E
...
!WENBO_S
        if(CUBED_SPHERE_PROJECTION) then
           nodes_coords_cubedsph(inode_new(i_node),:)=nodes_coords_cubedsph_old(i_node,:)
        end if
!WENBO_E
      endif
    enddo
...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION) then
     ! CPML initialization
     call create_CPML_regions(nspec,nglob,nodes_coords_cubedsph)
     !deallocate(nodes_coords_cubedsph)
     if(ncavity == 1) deallocate(nodes_coords_cubedsph_old)
  else
     ! CPML initialization
...
!WENBO_S  added nodes_coords_cubedsph,CUBED_SPHERE_PROJECTION for CPML, cubed
!sphere enabled
...
!WENBO_E
  ...
  deallocate(nodes_coords)
!WENBO_S
  if(CUBED_SPHERE_PROJECTION)    deallocate(nodes_coords_cubedsph)
!WENBO_E
