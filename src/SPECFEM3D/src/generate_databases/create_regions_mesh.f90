...
!build depth list of solid media
  if(myrank==0) write(IMAIN,*) 'save the depth table for DSM-SEM coupling'
  if(myrank==0) write(IMAIN,*) 'save depths in elastic media'
!WENBO
  if(COUPLING_TYPE.eq.2.or.COUPLING_TYPE.eq.3) then
     call save_depth_id(COUPLING_TYPE,myrank,NPROC,num_abs_boundary_faces,&
                     ...
                     ystore_dummy,zstore_dummy,1,IMAIN,&
                     prname,LOCAL_PATH)

  if(myrank==0) write(IMAIN,*) 'save depths in acoustic media'
!acoustic media
     call save_depth_id(COUPLING_TYPE,myrank,NPROC,num_abs_boundary_faces,&
                     abs_boundary_ispec,abs_boundary_ijk,ispec_is_elastic,&
                     ispec_is_acoustic,nspec,ibool,nglob,xstore_dummy,&
                     ystore_dummy,zstore_dummy,2,IMAIN,&
                     prname,LOCAL_PATH)
   end if


   if(COUPLING_TYPE.eq.1.or.COUPLING_TYPE.eq.3) then
      call teleseismic_coupling(myrank,kappastore,mustore,&
...
! creates global indexing array ibool
  ...
  use generate_databases_par, only: NGLLX,NGLLY,NGLLZ,NDIM,COUPLING_TYPE,&
           PML_CONDITIONS
  use create_regions_mesh_ext_par
  use tele_coupling_par,only : rotation_matrix_back_cubedsph,&
