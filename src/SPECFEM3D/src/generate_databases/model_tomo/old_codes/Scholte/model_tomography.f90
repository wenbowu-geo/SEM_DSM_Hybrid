...
!
  ...
  real(kind=CUSTOM_REAL), dimension (:), allocatable :: vp_tomography,vs_tomography,&
                        rho_tomography,qmu_tomography,qkappa_tomography,&
                        x_tomography,y_tomography,z_tomography
  ...
  !Using the interface topography to help assign 1-D DSM structure. 
  !Interface could be 410-km, 660-km, CMB or ICB.
  module topography_model_interface
  use constants
    double precision ::ORIG_LON_interface,ORIG_LAT_interface,END_LON_interface,END_LAT_interface
    double precision ::SPACING_LON_interface, SPACING_LAT_interface
    integer :: NLON_interface, NLAT_interface
    real(kind=CUSTOM_REAL), dimension (:,:), allocatable :: topography_interface
    double precision ::ref_depth_interface
  end module topography_model_interface
...
!----------------------------read 3-D tomography model----------------------
  ...
  ! local parameters
  real(kind=CUSTOM_REAL) :: x_tomo,y_tomo,z_tomo,vp_tomo,vs_tomo,rho_tomo,qmu_tomo,qkappa_tomo
  integer :: irecord
  integer ::ier
  character(len=256):: filename
  ...
  allocate(vp_tomography(1:nrecord), &
          vs_tomography(1:nrecord), &
          rho_tomography(1:nrecord), &
          qmu_tomography(1:nrecord), &
          qkappa_tomography(1:nrecord), &
  ...
  do irecord = 1,nrecord
    read(27,*) x_tomo,y_tomo,z_tomo,vp_tomo,vs_tomo,rho_tomo,qmu_tomo,qkappa_tomo
    ...
    qmu_tomography(irecord) = qmu_tomo
    qkappa_tomography(irecord) = qkappa_tomo
  enddo
  close(27)
...
!***********************INCORPORATE INTERFACE TOPOGRAPY**************************************
!*******implement topogrpahy of discontinuity, such as 410-km, 660-km, CMB or ICB**********

  !*******read the interface topography************
  subroutine read_topography_interface(myrank)
  use topography_model_interface
...
!  real(kind=CUSTOM_REAL) ::temp
  ...
  filename = MF_IN_DATA_FILES(1:len_trim(MF_IN_DATA_FILES))//'latlon_interface_topo.txt'
  open(unit=27,file=trim(filename),status='old',action='read',iostat=ier)
  if( ier /= 0 ) call exit_MPI(myrank,'error reading interface topography file')

  ! reads in model dimensions
  read(27,*) NLON_interface, NLAT_interface
  read(27,*) ORIG_LON_interface,ORIG_LAT_interface, END_LON_interface, END_LAT_interface
  read(27,*) SPACING_LON_interface, SPACING_LAT_interface
  allocate(topography_interface(NLAT_interface,NLON_interface))
  do ilat=1,NLAT_interface
     read(27,*)topography_interface(ilat,:)
  end do
  ref_depth_interface=-topography_interface(1,1)
  close(27)
  end subroutine read_topography_interface

  !*******refer to the interface topography(lat,lon) for updating the structure
  !at relevant GLL points
  subroutine adjust_with_topography(imaterial_id,myrank,x_eval,y_eval,z_eval,&
                                r_middle,x_eval_adjusted,y_eval_adjusted,z_eval_adjusted,&
                                r_middle_adjusted,topo_adjust_structure)

  use topography_model_interface
  use constants, only:PI,TINYVAL
  use tele_coupling_par, only :R_TOP_BOUND,COUPLING_DEPTH_TOLERENCE

  implicit none
  integer, intent(in) :: imaterial_id
  double precision, intent(in) :: x_eval,y_eval,z_eval,r_middle
  integer, intent(in) ::myrank
  double precision, intent(out):: x_eval_adjusted,y_eval_adjusted,z_eval_adjusted,r_middle_adjusted
  logical, intent(out) ::topo_adjust_structure

  !local variables
  real(kind=CUSTOM_REAL) ::radius,theta_surf,phi_surf
  real(kind=CUSTOM_REAL) ::real_depth_interface,depth,depth_middle
  integer:: ilat,ilon
  double precision:: r_adjusted
  double precision:: adjust_scale
  integer ::tmp_int

  !imaterial_id not used now, but may be useful for future modifying.
  tmp_int=imaterial_id

  radius=dsqrt(x_eval**2+y_eval**2+z_eval**2)
  theta_surf=dacos(z_eval/radius)
  phi_surf=dacos(dble(x_eval)/(dble(radius)*dsin(theta_surf))*(1-TINYVAL))
  if(y_eval<-TINYVAL) then
        phi_surf=-phi_surf
  end if
  theta_surf=theta_surf*180.0/PI
  phi_surf=phi_surf*180.0/PI
  ilat=(90.0-theta_surf-ORIG_LAT_interface)/SPACING_LAT_interface
  ilon=(phi_surf-ORIG_LON_interface)/SPACING_LON_interface
  real_depth_interface=-topography_interface(ilat,ilon)

  depth=R_TOP_BOUND-radius
  depth_middle=R_TOP_BOUND-r_middle
  !zero topography at this (lon,lat)
  if(abs(ref_depth_interface-real_depth_interface) < TINYVAL) then
      r_adjusted=radius
      topo_adjust_structure=.false.
  !bump, real interface above the 1-D reference interface depth
  else if (real_depth_interface < ref_depth_interface+COUPLING_DEPTH_TOLERENCE) then
     !Either this GLL point or element centroid within the bump.
     !We need to consider both GLL depth and centroid depth, because of
     !the possible interpolation error of topography during meshing.
     if( (depth > real_depth_interface .and. depth < ref_depth_interface) .or. &
         (depth_middle > real_depth_interface .and. depth_middle < ref_depth_interface) ) then
         topo_adjust_structure=.true.
         !centroid above the real interface depth, shift to the upper layer
         if(depth_middle < real_depth_interface) then
             r_adjusted=R_TOP_BOUND - ref_depth_interface + 2*COUPLING_DEPTH_TOLERENCE
         !other situations, shift to the lower layer
         else
             r_adjusted=R_TOP_BOUND - ref_depth_interface - 2*COUPLING_DEPTH_TOLERENCE
             !if(ilat>1000 .and. ilat <1002 .and. ilon >1000 .and. ilon < 1002) then
             !   write(*,*) "shifted to lower layer",depth,depth_middle,r_adjusted
             !end if
         end if
     else
         topo_adjust_structure=.false.
  ...
  !depression, real interface below the 1-D reference interface depth
  else if (real_depth_interface > ref_depth_interface-COUPLING_DEPTH_TOLERENCE) then
     !Either this GLL point or element centroid within the bump.
     if( (depth < real_depth_interface .and. depth > ref_depth_interface) .or. &
         (depth_middle < real_depth_interface .and. depth_middle > ref_depth_interface) ) then
         topo_adjust_structure=.true.
         !centroid below the interface, shift to the lower layer
         if(depth_middle > real_depth_interface) then
             r_adjusted=R_TOP_BOUND - ref_depth_interface - 2*COUPLING_DEPTH_TOLERENCE
         !other situations, shift to the lower layer
         else
     ...
     call exit_MPI(myrank,'Error in finding the DSM layer of GLL point with interface topography')
  end if

  !Both real interface depth and reference depth are between depth and
  !real_depth_interface (in the elements).
  !This situation would be misidentified in the above steps, so fix it here.  
  if((depth-real_depth_interface)*(depth_middle-real_depth_interface)<0.0) then
     topo_adjust_structure=.true.
     if(depth_middle>real_depth_interface) then
        r_adjusted=R_TOP_BOUND-ref_depth_interface-2*COUPLING_DEPTH_TOLERENCE
     else
  ...
  if(topo_adjust_structure) then
     adjust_scale=r_adjusted/radius
     x_eval_adjusted=x_eval*adjust_scale
     y_eval_adjusted=y_eval*adjust_scale
     z_eval_adjusted=z_eval*adjust_scale
     r_middle_adjusted=r_middle*adjust_scale
  else
     x_eval_adjusted=x_eval
     y_eval_adjusted=y_eval
     z_eval_adjusted=z_eval
     r_middle_adjusted=r_middle
  end if
!     call model_DSM1D(imaterial_id,x_eval_adjusted,y_eval_adjusted,z_eval_adjusted, &
!                        r_middle_adjusted,rho_final,vp_final,vs_final,&
!                        qkappa_atten_final,qmu_atten_final,myrank)
!  end if
  end subroutine adjust_with_topography
...
!**********add Ultra-Low Velocity Zone at CMB 
subroutine add_ULVZ(myrank,x_eval,y_eval,z_eval,xeval_cubedsph,yeval_cubedsph, &
                 vp_final,vs_final,rho_final)
USE sediment
USE constants, ONLY:PI,TINYVAL,R_EARTH_SURF
IMPLICIT NONE
...
DOUBLE PRECISION, PARAMETER ::ULVZ_thickness = 40000.0 !unit of meter
DOUBLE PRECISION, PARAMETER ::r_CMB = 3482000.0 !unit of meter
DOUBLE PRECISION, PARAMETER ::ULVZ_width_x = 1.0
DOUBLE PRECISION, PARAMETER ::ULVZ_width_y = 1.0
DOUBLE PRECISION, PARAMETER ::degtorad=PI/180.0
DOUBLE PRECISION, PARAMETER ::dvp_vp = -0.15 
DOUBLE PRECISION, PARAMETER ::dvs_vs = -0.3
DOUBLE PRECISION, PARAMETER ::drho_rho = 0.2
integer ::tmp_int

!myrank is not used now, but may be used in the future.
tmp_int=myrank

radius = DSQRT(x_eval**2 + y_eval**2 + z_eval**2)
!ULVZ above the CMB
if(vs_final.gt.TINYVAL.and.abs(radius-r_CMB)<ULVZ_thickness.and. &
   abs(xeval_cubedsph).lt.ULVZ_width_x*degtorad/2.0.and. &
   abs(yeval_cubedsph).lt.ULVZ_width_y*degtorad/2.0) then
 vp_final=vp_final*(1+dvp_vp)
 vs_final=vs_final*(1+dvs_vs)
 rho_final=rho_final*(1+drho_rho)   
end if

END SUBROUTINE add_ULVZ
...
if(fluid_thispoint.eq.1.and.r.gt.6357000.0) then
!water
   vs_final=0.d0
   vp_final=1500.0
   rho_final=1000.0
...
! invalid number
  ...
  if(izone2_r.eq.0.and.izone1_r.eq.0) then
       ...
       stop 'Error, the GLL point does not match any layer of dsm_model_input. &        
             ...
             &but the DSM layer at this depth is solid. Or vice sersa. If &
             &interface topography implemented, curvature may be too large (use & 
             &smaller mesh size or smooth the topography)'
  ...
  !if(myrank.eq.19.and.r_middle>5711000.0-3000.0.and.r_middle<5711000.0+3000.0) &
  !   print *,"debug_velo",r,r_middle,vp_final
     !print *,"debug_velo",x_eval,y_eval,z_eval,r,r_middle,vp_final
  end subroutine model_DSM1D

  subroutine model_DSM3D(imaterial_id,r_middle,x_eval,y_eval,z_eval,xeval_cubedsph,&
                         yeval_cubedsph,zeval_cubedsph, &
                         rho_final,vp_final,vs_final,qkappa_atten_final,qmu_atten_final,myrank)
  use tomography

  implicit none

  integer, intent(in) :: imaterial_id
  double precision, intent(in) :: r_middle,x_eval,y_eval,z_eval,xeval_cubedsph,&
                                  yeval_cubedsph,zeval_cubedsph
  real(kind=CUSTOM_REAL), intent(out) ::vp_final,vs_final,rho_final, &
                                  qkappa_atten_final,qmu_atten_final
  integer,intent(in) ::myrank
  ...
  logical, parameter ::ULVZ_incorporated=.false.
  logical, parameter ::interface_topography = .false.

  double precision ::x_eval_adjusted,y_eval_adjusted,z_eval_adjusted,r_middle_adjusted
  logical :: topo_adjust_structure

!********************************************************************

  if(interface_topography) then
    call adjust_with_topography(imaterial_id,myrank,x_eval,y_eval,z_eval,&
                                r_middle,x_eval_adjusted,y_eval_adjusted,z_eval_adjusted,&
                                r_middle_adjusted,topo_adjust_structure)
    call model_DSM1D(imaterial_id,x_eval_adjusted,y_eval_adjusted,z_eval_adjusted, &
                        r_middle_adjusted,rho_final,vp_final,vs_final,&
    ...
    call model_DSM1D(imaterial_id,x_eval,y_eval,z_eval, &
                        r_middle,rho_final,vp_final,vs_final,&
                        qkappa_atten_final,qmu_atten_final,myrank)
  end if


  if(imaterial_id.eq.2) then !Implement below structures only for solid media
     ...
     if(tomography_3D_incorporated) then
      ...
      !                          rho_final,vp_final,vs_final)
      call tomography3D_vpvsrho_latlon(x_eval,y_eval,z_eval,xeval_cubedsph,yeval_cubedsph, &
                                rho_final,vp_final,vs_final,qmu_atten_final,qkappa_atten_final)
     end if
     ...
     if(ULVZ_incorporated) then
      call add_ULVZ(myrank,x_eval,y_eval,z_eval,xeval_cubedsph,yeval_cubedsph, &
                    vp_final,vs_final,rho_final)
     end if
  end if
...
!***********************find the perturbation***********************
  subroutine tomography3D_perturbation(x_eval,y_eval,z_eval, &
                             rho_final,vp_final,vs_final)
  use tomography
  ...
  double precision ::r,depth,Rearth
...
!                      +0.000106*(vp_final / 1000.0)**5 )*1000.0
  ...
  ! impose minimum and maximum velocity and density if needed
     ...
     perturbation= &
       ...
       vp7*gamma_interp_x*gamma_interp_y*gamma_interp_z3 + &
       vp8*(1.-gamma_interp_x)*gamma_interp_y*gamma_interp_z4
       vp_final=vp_final*(1+3.0*perturbation)
       vs_final=vs_final*(1+6.0*perturbation)
       rho_final=rho_final*(1+3.0*perturbation)
...
!***********************find the perturbation***********************
  subroutine tomography3D_vpvsrho_latlon(x_eval,y_eval,z_eval,xeval_cubedsph,yeval_cubedsph, &
                             rho_final,vp_final,vs_final,qmu_final,qkappa_final)
  use tomography
  use tele_coupling_par, only :R_TOP_BOUND
  ...
  double precision, intent(in) :: x_eval,y_eval,z_eval, xeval_cubedsph, yeval_cubedsph
  real(kind=CUSTOM_REAL), intent(out) :: vp_final,vs_final,rho_final,qmu_final,qkappa_final
  ...
  real :: dist,t_total
  real :: new_vp,new_vs,new_rho,new_qmu,new_qkappa
  ...
  double precision, parameter ::top_taper_depth_start = -100000.0
  double precision, parameter ::top_taper_depth_end   = -100000.0
  double precision, parameter ::bot_taper_depth_start = 80000.0
  double precision, parameter ::bot_taper_depth_end =  100000.0
  
  !specify the below two widths for making the local-dsm model transitions at the four
  !edges of SEM box.
  !This width gives the gradual tapering (in unit of degree and 0.1 degree is roughly
  !11.2 km at the surface)
  double precision, parameter ::taper_width_edges = 0.4
  !DSM model is used in this outermost width (in unit of degree and 0.1 degree is
  !roughly 11.2 km at the surface)
  ...
  ! Decide longitude convention from tomography grid definition
  if (ORIG_X < 0.d0)  then
     ! grid uses -180 to 180
  ...
  if(ix.gt.0.and.ix.le.NX.and.iy.gt.0.and.iy.le.NY.and.iz.gt.0.and.iz.le.NZ) then
     do i=0,1
        do j=0,1
           do k=0,1
              ...
              new_qmu = new_qmu + qmu_tomography(l)/dist
              new_qkappa = new_qkappa + qkappa_tomography(l)/dist
           enddo
        enddo
     enddo
     ...
     new_qmu = new_qmu/t_total
     new_qkappa = new_qkappa/t_total

     !make a gradual transition to the dsm model at the bottom and edges of the SEM box.
     call taper_to_dsm_model(taper,depth,top_taper_depth_start,top_taper_depth_end, &
                bot_taper_depth_start,bot_taper_depth_end,xeval_cubedsph,yeval_cubedsph,&
                taper_width_edges,dsm_width_edges)
   
     if(new_vp.lt.0.1.or.new_vs.lt.0.1.or.new_rho.lt.0.1) then
        stop 'Error in vp, vs or rho of 3-D tomography model'
     else
       vp_final=new_vp*taper+vp_final*(1.0-taper)
       vs_final=new_vs*taper+vs_final*(1.0-taper)
       rho_final=new_rho*taper+rho_final*(1.0-taper)
       qmu_final=new_qmu*taper+qmu_final*(1.0-taper)
       qkappa_final=new_qkappa*taper+qkappa_final*(1.0-taper)
     endif
  else
     write(*,*)lat,lon,depth,ORIG_X,ORIG_Y,ORIG_Z,ix,iy,iz
  ...
  subroutine deallocate_tomography_files()
    ...
    deallocate(qmu_tomography)
    deallocate(qkappa_tomography)
