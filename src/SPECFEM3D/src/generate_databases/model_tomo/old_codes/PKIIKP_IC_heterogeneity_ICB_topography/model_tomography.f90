...
!
  ...
  use constants
    ...
    real(kind=CUSTOM_REAL) :: ref_depth_interface
...
!=====================================================================
! Read tiled external tomography models (tomography_model_box?.xyz)
! Crops automatically based on mesh bounds and EXTEND
! Uses parameters from module tomography
!=====================================================================
  subroutine read_model_tomography(myrank)
    ...
    ! local variables
    integer :: ier, ix, iy, iz, itile
    integer :: ix_g, iy_g, iz_g, irec
    ...
    real(kind=CUSTOM_REAL) :: xmin_c, xmax_c, ymin_c, ymax_c, zmin_c, zmax_c
    real(kind=CUSTOM_REAL) :: xval, yval, zval
  
    real(kind=CUSTOM_REAL), parameter :: EXTEND = 3000.0
  
    real(kind=CUSTOM_REAL), allocatable :: row_tmp(:)
    ...
    !-------------------------------------------------
    ! 2. Read spacing from first existing tile
    !-------------------------------------------------
    itile = 1
    do
       write(filename,'(A,"/",A,I0,".xyz")') trim(TOMOGRAPHY_PATH), 'tomography_model_box', itile
       inquire(file=trim(filename), exist=exists)
       if (exists) exit
       itile = itile + 1
       if (itile > 9999) call exit_MPI(myrank,'No tomography_model_box?.xyz found')
    end do
  
    open(27,file=trim(filename),status='old',action='read',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening first tomography tile')
    ...
    !-------------------------------------------------
    ! 3. Define cropped global domain (grid-aligned)
    !-------------------------------------------------
    ORIG_X = floor(xmin_c / SPACING_X) * SPACING_X
    ORIG_Y = floor(ymin_c / SPACING_Y) * SPACING_Y
    ORIG_Z = floor(zmin_c / SPACING_Z) * SPACING_Z
  
    NX = int((xmax_c - ORIG_X) / SPACING_X) + 1
    NY = int((ymax_c - ORIG_Y) / SPACING_Y) + 1
    NZ = int((zmax_c - ORIG_Z) / SPACING_Z) + 1
    ...
    allocate(vp_tomography(nrecord), vs_tomography(nrecord), &
             rho_tomography(nrecord), x_tomography(nrecord), &
             y_tomography(nrecord), z_tomography(nrecord), stat=ier)
  
    if (ier /= 0) call exit_MPI(myrank,'Not enough memory for tomography arrays')
  
    vp_tomography = 0.0; vs_tomography = 0.0; rho_tomography = 0.0
  
    !-------------------------------------------------
    ! 4. Loop over tiles and ingest data
    !-------------------------------------------------
    itile = 1
    do
       write(filename,'(A,"/",A,I0,".xyz")') trim(TOMOGRAPHY_PATH), 'tomography_model_box', itile
       ...
       open(27,file=trim(filename),status='old',action='read',iostat=ier)
       if (ier /= 0) call exit_MPI(myrank,'Error opening tomography tile')
       ...
       allocate(row_tmp(NX_f))
  
       do iz = 1, NZ_f
          zval = ORIG_Z_f + (iz-1)*SPACING_Z
          if (zval < zmin_c .or. zval > zmax_c) then
             do iy = 1, NY_f
                read(27,*) row_tmp(:)
             end do
             cycle
          ...
          do iy = 1, NY_f
             yval = ORIG_Y_f + (iy-1)*SPACING_Y
             read(27,*) row_tmp(:)
             if (yval < ymin_c .or. yval > ymax_c) cycle
  
             do ix = 1, NX_f
                xval = ORIG_X_f + (ix-1)*SPACING_X
                if (xval < xmin_c .or. xval > xmax_c) cycle
  
                ix_g = int((xval - ORIG_X) / SPACING_X) + 1
                iy_g = int((yval - ORIG_Y) / SPACING_Y) + 1
                iz_g = int((zval - ORIG_Z) / SPACING_Z) + 1
  
                irec = (iz_g-1)*NY*NX + (iy_g-1)*NX + ix_g
                vp_tomography(irec) = row_tmp(ix)
             end do
          end do
       end do
    ...
    if (myrank == 0) then
       write(*,*) 'Tiled tomography loaded:'
       write(*,*) ' NX, NY, NZ = ', NX, NY, NZ
       write(*,*) ' ORIG = ', ORIG_X, ORIG_Y, ORIG_Z
       write(*,*) ' END  = ', END_X, END_Y, END_Z
    end if
...
!  real(kind=CUSTOM_REAL) ::temp
  ...
  lon_lookup=phi_surf
  do while(lon_lookup < ORIG_LON_interface)
     lon_lookup=lon_lookup+360.d0
  end do
  do while(lon_lookup > END_LON_interface)
     lon_lookup=lon_lookup-360.d0
  end do

  ilat=nint((90.0d0-dble(theta_surf)-ORIG_LAT_interface)/SPACING_LAT_interface)+1
  ilon=nint((lon_lookup-ORIG_LON_interface)/SPACING_LON_interface)+1
  ilat=max(1,min(NLAT_interface,ilat))
  ilon=max(1,min(NLON_interface,ilon))
...
! invalid number
  ...
  logical, parameter ::interface_topography = .true.
...
!***********************find the perturbation***********************
  ...
  !no top taperin. Set them as negative numbers.
  double precision, parameter ::top_taper_depth_start = 30000.0
  double precision, parameter ::top_taper_depth_end =   50000.0
  !bottom tapering. Starts at 520 km below the top boundary of the SEM box.
  double precision, parameter ::bot_taper_depth_start = 1000000.0
  double precision, parameter ::bot_taper_depth_end =   1100000.0
