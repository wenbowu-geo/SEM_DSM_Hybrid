...
!specify these two depths (meters), from which local/tomography model grapdually transition to
!the dsm model.
...
!print *,"spacing_x_bathymetry",spacing_x_bathymetry,spacing_y_bathymetry
...
! invalid number
  ...
  if(izone2_Radi.eq.0.and.izone1_Radi.eq.0) then
      if(DEBUG_COUPLING) print *,'izone2',izone2_Radi,r,&
     ...
     if(r_middle.lt.r) then
       call cal_PREM_structure(r,rho_structure_zone(1,izone1_Radi),temp)
       rho_final=temp
       call cal_PREM_structure(r,vpv_structure_zone(1,izone1_Radi),temp)
       vp_final=temp
       call cal_PREM_structure(r,vsv_structure_zone(1,izone1_Radi),temp)
       vs_final=temp
       qmu_atten=qmu_structure_zone(izone1_Radi)
       qkappa_atten=qkappa_structure_zone(izone1_Radi)
     else 
       call cal_PREM_structure(r,rho_structure_zone(1,izone2_Radi),temp)
       rho_final=temp
       call cal_PREM_structure(r,vpv_structure_zone(1,izone2_Radi),temp)
       vp_final=temp
       call cal_PREM_structure(r,vsv_structure_zone(1,izone2_Radi),temp)
       vs_final=temp
       ...
       call cal_PREM_structure(r,vsv_structure_zone(1,izone2_Radi),temp)
       vs_final=temp
       call cal_PREM_structure(r,qmu_structure_zone(izone2_Radi),temp)
       qmu_atten=qmu_structure_zone(izone2_Radi)
       qkappa_atten=qkappa_structure_zone(izone2_Radi)
   else
  ...
  logical, parameter ::oceanic_lithosphere_incorporated = .true.
...
!***********************find the perturbation***********************
  subroutine get_tomography_2D(x,y,z,x_cubedsph,y_cubedsph, &
    ...
    !specify these two depths (meters), from which local/tomography model 
    !grapdually transition to the dsm model.
...
!100 km thick of 660km coupling
    ...
    !add taper at the top,bottom and edges of simulated model to make it smoothly 
