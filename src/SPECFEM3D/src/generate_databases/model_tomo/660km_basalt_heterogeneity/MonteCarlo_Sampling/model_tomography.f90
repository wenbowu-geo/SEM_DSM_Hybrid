...
contains
  ...
  ! Matches Python's hash_u64 exactly
  pure real(8) function hash_u64(ik, seed)
    integer, intent(in) :: ik, seed
    integer(8) :: x
    ! Wrap BOZ literals in INT(..., 8) for standard compliance
    integer(8), parameter :: K1 = int(z'5851F42D4C957F2D', 8)
    integer(8), parameter :: K2 = int(z'BF58476D1CE4E5B9', 8)
    integer(8), parameter :: K3 = int(z'94D049BB133111EB', 8)

    ! Python matching logic: x = (ik + K1 * seed)
    x = int(ik, 8) + K1 * int(seed, 8)
    ...
    ! Mask to 53 bits (2.0d0**53 = 9007199254740992.0d0)
    hash_u64 = dble(iand(ishft(x, -11), int(z'001FFFFFFFFFFFFF', 8))) / 9007199254740992.0d0
  end function hash_u64

  subroutine stochastic_perturbation(x, y, z, perturbation, L, sigma, dx_eff, Nk)
    real(8), intent(in)  :: x, y, z, L, sigma, dx_eff
    integer, intent(in)  :: Nk
    real(8), intent(out) :: perturbation
    
    integer :: ik
    real(8) :: k, kmax, theta, phi, kx, ky, kz, Pk, pdf_k, A, phase, sumval

    kmax = pi / dx_eff
    sumval = 0.0d0

    do ik = 1, Nk
       ! Matches Python: u1, u2, u3 logic
       k     = kmax * max(hash_u64(ik, 1), 1.d-15)**(1.0d0/3.0d0)
       theta = acos(2.0d0 * hash_u64(ik, 2) - 1.0d0)
       phi   = 2.0d0 * pi * hash_u64(ik, 3)
       ...
       ! Power Spectrum
       Pk = (sigma**2 * L**3) / (pi**2 * (1.0d0 + (k*L)**2)**2)
       pdf_k = 3.0d0 * k**2 / kmax**3

       ! Amplitude (Matches your Python structure)
       A = sqrt( 2.0d0 * Pk * 4.0d0 * pi * k**2 / (pdf_k * Nk) )
       phase = 2.0d0 * pi * hash_u64(ik, 999)

       sumval = sumval + A * cos(kx*x + ky*y + kz*z + phase)
    end do
    perturbation = sumval
  end subroutine stochastic_perturbation

end module stochastic_model
...
!***********************find the perturbation***********************
  ...
  !no top tapering. Set both depths as R_Earth-R_ICB, in unit of km.
    double precision, parameter ::top_taper_depth_start =  49999.0
  ...
  !degree.
  double precision, parameter ::taper_width_edges = 0.3
  double precision, parameter ::dsm_width_edges = 0.3

  !Heterogeneities with exponential ACF
  double precision, parameter :: L = 4000.0       ! correlation length (m)
  double precision, parameter :: sigma = 0.04d0   ! RMS amplitude
  ! Use largest GLL distance from mesh for dx_eff

  ! Note:
  ! Decreasing dx_eff (i.e., using a finer effective mesh) increases kmax.
  ! If Nk is not increased accordingly, fewer Monte Carlo samples fall in the
  ! physically important low-k range, reducing the number of useful modes.
  ! To maintain the same sampling density in 3-D k-space, Nk should scale
  ! approximately with kmax³ (i.e., Nk ∝ dx_eff⁻³). 
  ! For scattering problems, the "useful" modes are those whose wavelengths
  ! are comparable to the seismic wavelength. Because incident waves have
  ! specific propagation directions, only modes whose wavevector projections
  ! along the propagation direction match the seismic wavelength contribute
  ! efficiently to scattering.
  double precision, parameter :: dx_eff = 633.0  ! effective grid spacing (m)
  ! Number of plane waves (Fourier modes) used to construct the stochastic field.
  ! Each mode contributes a cosine wave with deterministic k, theta, phi, and phase.
  integer, parameter :: Nk = 20000
...
!  z_eval_proj=0.0
  ...
  call stochastic_perturbation(x_eval_proj, y_eval_proj, z_eval_proj, perturbation,L,sigma,dx_eff,Nk)
  ...
  if(myrank.eq.80) write(*,*) "perturbations",x_eval_proj, y_eval_proj, z_eval_proj, perturbation

  if(perturbation.gt.0.2.or.perturbation.lt.-0.2) write(*,*)perturbation !stop 'Error perturbation'
