...
!WENBO_S --- add the parameter PML_CONDITIONS
  use generate_databases_par, only: IMODEL, &
    ...
    ANISOTROPY,NGLLX,NGLLY,NGLLZ,FOUR_THIRDS,TWO,IMAIN,CUBED_SPHERE_PROJECTION,&
    PML_CONDITIONS,COUPLING_TYPE
  use constants, only :IMODEL_DSM3D,IMODEL_DSM1D
  use create_regions_mesh_ext_par
  use tele_coupling_par,only :xstore_dummy_cubedsph,ystore_dummy_cubedsph,zstore_dummy_cubedsph,&
     rotation_matrix_back_cubedsph,ANGULAR_WIDTH_XI_IN_DEGREES,&
...
!WENBO_S
  double precision ::r_middle,xmesh_cubedsph,ymesh_cubedsph,zmesh_cubedsph
  integer::igllx_mid,iglly_mid,igllz_mid
!WENBO_E
  integer :: iglob
...
!WENBO_S
!Comment the below line. If you want to read external tomography model
!"tomography_model.xyz", just uncomment the below line and the last line in this 
!subroutine for deallocating arrays.
...
!WENBO_S
if((COUPLING_TYPE.lt.4.and.IMODEL.eq.IMODEL_DSM3D) .or. IMODEL.eq.IMODEL_TOMO) then
!    call model_tomography_broadcast(myrank)
end if
!read 2D tomography model (now used for PKP660PKP)
!call model_tomography_broadcast_2D(myrank)
!add slab
!    call model_slabdepth_broadcast(myrank)
!add sediment
!  call read_topography(myrank)
!  call read_trench(myrank)
!add oceanic lithosphere
!  call bathymetry_broadcast(myrank)
!add interface topography (interface could be 410-km, 660-km, CMB or ICB)
  call read_topography_interface(myrank)
!WENBO_E
...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION)   call read_DSM_model(myrank)
!WENBO_E
...
!WENBO_S
          if(mod(NGLLX,2).eq.1) then
             igllx_mid=(NGLLX+1)/2
          else
             igllx_mid=NGLLX/2
          end if
          if(mod(NGLLY,2).eq.1) then
             iglly_mid=(NGLLY+1)/2
          else
             iglly_mid=NGLLY/2
          end if
          if(mod(NGLLZ,2).eq.1) then
             igllz_mid=(NGLLZ+1)/2
          else
             igllz_mid=NGLLZ/2
          end if

          r_middle=sqrt( xstore_dummy(ibool(igllx_mid,iglly_mid,igllz_mid,ispec))**2 &
                         +ystore_dummy(ibool(igllx_mid,iglly_mid,igllz_mid,ispec))**2 &
                         + zstore_dummy(ibool(igllx_mid,iglly_mid,igllz_mid,ispec))**2)

! The below three parameters help set up the tomography model and taper it to
! 1-D DSM model.
          ! already computed in the PML setting up process.
          if(CUBED_SPHERE_PROJECTION) then
            ! already computed in the PML setting up process.
            if(PML_CONDITIONS) then
              xmesh_cubedsph=xstore_dummy_cubedsph(iglob)
              ymesh_cubedsph=ystore_dummy_cubedsph(iglob)
              zmesh_cubedsph=zstore_dummy_cubedsph(iglob)
            else 
              ! calculate the cubed-sphere coordinates
              ! xmesh_cubedsph,ymesh_cubedsph and zmesh_cubedsph.
              call xyz_backto_cubedsph(xmesh,ymesh,zmesh,&
                         xmesh_cubedsph,ymesh_cubedsph,zmesh_cubedsph,&
                         ANGULAR_WIDTH_XI_IN_DEGREES,&
                         ANGULAR_WIDTH_ETA_IN_DEGREES,rotation_matrix_back_cubedsph)
            end if

          !not exist when local Cartesian coordinate system used
          !(CUBED_SPHERE_PROJECTION=.false.)
          else
            xmesh_cubedsph=0.0
            ymesh_cubedsph=0.0
            zmesh_cubedsph=0.0
...
!WENBO_E
          call get_model_values(materials_ext_mesh,nmat_ext_mesh, &
                               ...
                               xmesh,ymesh,zmesh,xmesh_cubedsph,&
                               ymesh_cubedsph,zmesh_cubedsph,r_middle, &
...
!WENBO_S
! Uncomment the below line to read external tomography model "tomography_model.xyz"
  ! deallocates tomographic arrays
#ifndef DEBUG_COUPLED
!  if ( (nundefMat_ext_mesh > 0 .or. IMODEL == IMODEL_TOMO) ) call deallocate_tomography_files()
#endif
!  Release the memories for 2D tomography model arrays (now used for PKP660PKP
!  scattering, Mao et al., 2022, G3)
!  call deallocate_tomography_2D_files()
if((COUPLING_TYPE.lt.4.and.IMODEL.eq.IMODEL_DSM3D) .or. IMODEL.eq.IMODEL_TOMO) then 
...
!WENBO_S
  double precision, intent(in) :: r_middle,xmesh_cubedsph,&
                                  ymesh_cubedsph,zmesh_cubedsph
  integer ::myrank
!WENBO_E
  ...
  iflag_aniso = 0
!WENBO_S
!  assign idomain_id as imaterial_id, which is defined in Mesh_Par_file.
!  imaterial_id=1 for acoustic and imaterial_id=2 for elastic
!  idomain_id = IDOMAIN_ELASTIC
  idomain_id = imaterial_id

! Set the solid walls at the four edges of SEM box
!  if(imaterial_id.eq.3) idomain_id=2
!WENBO_E
...
!WENBO_S
  case (IMODEL_TOMO)
    ! gets model values from tomography file
     call model_tomo3D(xmesh,ymesh,zmesh,r_middle, &
                             rho,vp,vs,qkappa_atten,qmu_atten,imaterial_id)
  case (IMODEL_DSM1D)
     !Implement 1-D DSM model
     call model_DSM1D(imaterial_id,xmesh,ymesh,zmesh,r_middle,&
                                  rho,vp,vs,qkappa_atten,qmu_atten,myrank)
  case (IMODEL_DSM3D)
     !Implement 1-D DSM model
     call model_DSM1D(imaterial_id,xmesh,ymesh,zmesh,r_middle,&
                                  rho,vp,vs,qkappa_atten,qmu_atten,myrank)
     !410-km, 660-km, CMB or ICB topography
     call model_DSM3D(imaterial_id,r_middle,xmesh,ymesh,&
                                  zmesh,xmesh_cubedsph,ymesh_cubedsph,zmesh_cubedsph, &
                                  rho,vp,vs,qkappa_atten,qmu_atten,myrank)
     has_tomo_value=.true.
