...
!  real(kind=CUSTOM_REAL) ::temp
  ...
  subroutine adjust_with_topography(imaterial_id,myrank,x_eval,y_eval,z_eval,&
                                r_middle,vp_final,vs_final,&
                                rho_final,qkappa_atten_final,qmu_atten_final)
  use topography_model_interface
  ...
  double precision, intent(out):: vp_final,vs_final,rho_final,qkappa_atten_final,qmu_atten_final

  !local variables
  real(kind=CUSTOM_REAL) ::radius,theta_surf,phi_surf
  real(kind=CUSTOM_REAL) ::real_depth_interface,depth,depth_middle
  integer:: ilat,ilon
  double precision:: x_eval_adjusted,y_eval_adjusted,z_eval_adjusted
  double precision:: r_adjusted,r_middle_adjusted
  double precision:: adjust_scale
  logical ::adjust_structure
...
!  end if
  ...
  ! impose minimum and maximum velocity and density if needed
     ...
     perturbation= &
       ...
       if(depth_below_top<50000.0 .and. perturbation <-0.01) then
               perturbation=-0.006
       end if
       !weight_tomo=1.0
