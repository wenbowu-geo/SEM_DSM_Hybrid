subroutine get_tele_coupling_surfaces(myrank,xstore_dummy,ystore_dummy,zstore_dummy,ibool,nspec,&
...
integer ::ipoint_iface,face_coupling,face_coupling_elastic
...
!                      coord_file_solid
character(len=256) ::coupling_file_solid
...
! the code. wxgll,wygll,wzgll are not used now, that occationally causes
! error reports. But they might be usful in the future, so we keep them here.
real_tmp=wxgll(1)
real_tmp=wygll(1)
real_tmp=wzgll(1)
...
ipackage=0
face_coupling_elastic=0
do face_coupling=1,nele_tele_coupling
    face_type=coupling_ele_property(face_coupling)%face_type
   ...
   case(6)
     ...
     iface=6
   case default
     stop 'Error coupling boundary type'
   end select

   ispec=coupling_ele_property(face_coupling)%ispec_coupling
   if(ispec_is_elastic(ispec)) face_coupling_elastic=face_coupling_elastic+1
   ...
   if(LOW_RESOLUTION) then
     ...
     do i=1,NGLLX
       do j=1,NGLLY
           ...
           if(myrank.eq.0.and.face_coupling.eq.1.and.DEBUG_COUPLING) print *,'jacob_whole',i,j,k,&
                 jacobian2Dw_face(i,j),jacobian_whole_face 
       end do
     end do
   end if
   imin=coupling_ele_property(face_coupling)%istart;imax=coupling_ele_property(face_coupling)%iend
   jmin=coupling_ele_property(face_coupling)%jstart;jmax=coupling_ele_property(face_coupling)%jend
   kmin=coupling_ele_property(face_coupling)%kstart;kmax=coupling_ele_property(face_coupling)%kend
   di=1;dj=1;dk=1
     ...
     do j=jmin,jmax,dj
       do i=imin,imax,di
         if(ispec_is_elastic(ispec)) then
           ...
           if(LOW_RESOLUTION) then
               ipoint_elas_acous=face_coupling
               ipoint_elastic=face_coupling_elastic
           else
               ipoint_iface=ipoint_iface+1
               ipoint_elas_acous=(face_coupling-1)*NGLLX*NGLLY+ipoint_iface
               ipoint_elastic=(face_coupling_elastic-1)*NGLLX*NGLLY+ipoint_iface
           end if
...
!          box.
           ...
           if(mod(ipoint_elas_acous,NPOINTS_PER_PACK).eq.1) then
               ipackage=ipackage+1
               write(coupling_file_solid,"(a,'boundary_info_pack',i6.6)")trim(dir_this_proc),ipackage
               open(unit=115,file=trim(coupling_file_solid),status='unknown',&
                   action='write',form='formatted',iostat=ier)
               if( ier /= 0 ) then
                 print*,'error: could not open  file'
                 print*,'path:',coupling_file_solid(1:len_trim(coupling_file_solid))
                 call exit_mpi(myrank,'error opening coupling_file_solid file')
               endif

           end if

!           write(115,*)id_depth_coupling(ipoint),id_dist_coupling(ipoint),ispec_is_elastic(ispec),ispec_is_acoustic(ispec)
           write(115,*)id_depth_coupling(ipoint_elas_acous),ispec_is_elastic(ispec),ispec_is_acoustic(ispec)
