subroutine save_depth_id(COUPLING_TYPE,myrank,NPROC,num_abs_boundary_faces,&
  ...
  use constants
  use tele_coupling_par, only :COUPLING_DEPTH_TOLERENCE,COUPLING_DIST_TOLERENCE
  implicit none

!  include "constants.h"

  integer ::COUPLING_TYPE,myrank,NPROC,media_type
...
!other parameters
  integer ::npoints_coupling,ipoint,ipoint_temp,max_ndep_global,ndepth_table,&
...
!  double precision ::theta_source,phi_source
  double precision,dimension(3) ::coord_source,coord_coupling
...
! source
  double precision :: lat_sour,long_sour,depth_sour
...
!the code. IMAIN_OUTPUT is not used now, that occationally causes error 
!reports. But it might be usful in the future, so we keep it here.
 int_tmp=IMAIN_OUTPUT
  ...
  if(num_faces_this_media_type.gt.0) then
     allocate(iface_this_mediatype(num_faces_this_media_type))
  else 
  ...
  if(DEBUG_COUPLING) print *,'depth_list begin',myrank,npoints_coupling
  ...
  call read_tele_source(COUPLING_TYPE,lat_sour,long_sour,depth_sour)

  theta_sour = PI/2.0d0 - lat_sour*PI/180.0d0
  phi_sour = long_sour*PI/180.0d0
  coord_source(1)=dsin(theta_sour)*dcos(phi_sour)
  coord_source(2)=dsin(theta_sour)*dsin(phi_sour)
  coord_source(3)=dcos(theta_sour)

  ipoint=0
  do iface=1,num_faces_this_media_type
     ...
     do igll = 1,NGLLSQUARE
       ...
       dist_rad=dacos(dot_product(coord_coupling,coord_source)*(1.d0-TINYVAL))
       ...
       if(depth_coupling(ipoint)<5104900.0.and.myrank.eq.0.and.DEBUG_COUPLING) &
...
if(ndepth_table.gt.0) then
 ...
 if(myrank.eq.0) &
  ...
  if(npoints_coupling.gt.0.and.ipoint_temp.ne.npoints_coupling) then
        print *,myrank,"Error in counting coupoing points",ipoint_temp,npoints_coupling
...
! take the conscontinuity into account
 ...
 if(myrank.eq.0) then
  ...
  if(izone_dep_table(1)-izone_dep_table(ndepth_table).ne.ndiscon_found_deptable) then
      ...
      stop 'Error in discontinuity setting or depth table'
  end if
...
!      write(113,*) dist_table_global(idist)/r*(180.d0/PI)
 ...
 if(myrank.eq.0) close(113)
  ...
  do iface=1,num_faces_this_media_type
    do igll=1,NGLLSQUARE
     ipoint=ipoint+1
     write(117,*) iface_this_mediatype(iface),igll,depth_id(ipoint),dist_id(ipoint),dist_coupling(ipoint)
    end do
  end do
...
!     write(17,*) DDEPTH_FOR_INTERPO_DSM,ndepth_table+ndiscon_found_deptable
!     write(17,*) DEPTH_TOLERENCE_DSM
     write(17,*) DDEPTH_FOR_INTERPO_DSM,DEPTH_TOLERENCE_DSM
...
! save depth and distance id
  ...
  if(num_faces_this_media_type.gt.0) deallocate(iface_this_mediatype)
...
!             depth_table_local(idepth_test)-depth_coupling(ipoint),COUPLING_DEPTH_TOLERENCE
      if(itable_test.gt.n_table) then
           n_table=itable_test
           table_local(n_table)=input_array(ipoint)
      end if
      id_inlocal(ipoint)=itable_test
!     if(DEBUG_COUPLING) print *,'id_inlocal',id_inlocal(ipoint),depth_coupling(ipoint),myrank,ipoint
  end do
...
! receive other ranks' local tables
    do iproc=1,NPROC-1
...
if(myrank.eq.0.and.DEBUG_COUPLING) then
   print *,'pt99_depth',npoints_array,n_table,map_localtoglobal(:)
end if
if(npoints_array.gt.0) then
...
!*************************read teleseismic source - way 1 *****************************
!Use the file 'CMTSOLUTION' to describe teleseismic source for deep-Earth
!coupling and receiver-side coupling. If receiver-side coupling chosen, only
!the two parameters, latitude and longitude, are functional. 
subroutine read_tele_source(COUPLINT_TYPE,lat_tele,lon_tele,dep_tele)
use constants
implicit none

integer, intent(in) :: COUPLINT_TYPE
double precision, intent(out) :: lat_tele,lon_tele,dep_tele

!local parameters
character(len=256) :: string
character(len=MAX_STRING_LEN) :: CMTSOLUTION

integer ::ier

if(COUPLINT_TYPE.ne.2 .and.COUPLINT_TYPE.ne.3) then
  stop 'Error, teleseismic source only allowed for deep-Earth / receiver-side coupling'
end if

CMTSOLUTION = IN_DATA_FILES(1:len_trim(IN_DATA_FILES))//'CMTSOLUTION'
! Note that only one station allowed in CMTSOLUTION for SEM-DSM coupling mode
...
! skips empty lines and ignore the header line
read(IIN,"(a)",iostat=ier) string
do while (len_trim(string) == 0)
  read(IIN,"(a)",iostat=ier) string
  if (ier /= 0) then
    stop 'Error reading header line in CMTSOLUTION file'
  endif
enddo

!ignore the event name line
read(IIN,"(a)",iostat=ier) string
!ignore the time shift line
read(IIN,"(a)",iostat=ier) string
!ignore the half-duration line
read(IIN,"(a)",iostat=ier) string

!read the latitude
read(IIN,"(a)",iostat=ier) string
read(string(10:len_trim(string)),*) lat_tele
!read the longitude
read(IIN,"(a)",iostat=ier) string
read(string(11:len_trim(string)),*) lon_tele
!read the depth
read(IIN,"(a)",iostat=ier) string
read(string(7:len_trim(string)),*) dep_tele
close(IIN)

end subroutine read_tele_source



!*************************read teleseismic source- way 2*****************************
!The below subroutine was used to read a teleseismic source.
!It reads either CMTSOLUTION (deep-Earth coupling) or TELESEISMIC_STATION (receiver-side).
!It is helpful to understand how this code works for diffent types of coupling, 
!but makes the next step of solver more complex (additonal soubroutine to read "TELESEISMIC_STATION"). 
!Thus, Wenbo Wu chose to simply use one file 'CMTSOLUTION' for both scenarios. 

subroutine read_tele_source_dif_files(COUPLINT_TYPE,lat_tele,lon_tele,dep_tele)
use constants
implicit none
!
integer, intent(in) :: COUPLINT_TYPE
...
!local parameters
character(len=256) :: string
character(len=MAX_STRING_LEN) :: CMTSOLUTION,tele_station_file
character(len=MAX_LENGTH_STATION_NAME) :: station_name
character(len=MAX_LENGTH_NETWORK_NAME) :: network_name
double precision ::stlat,stlon,stbur

integer ::ier


!*****************receiver-side coupling*******************
if(COUPLINT_TYPE.eq.2) then
  tele_station_file=IN_DATA_FILES(1:len_trim(IN_DATA_FILES))//'TELESEISMIC_STATION'
  open(unit=IIN,file=trim(tele_station_file),status='old',action='read',iostat=ier)
  !Note that only one station allowed for receiver-side coupling
  read(IIN,*,iostat=ier) station_name,network_name,stlat,stlon,stbur
  if (ier /= 0) then
      print *,'Error opening file: ',trim(tele_station_file)
      stop 'Error reading TELESEISMIC_STATION file'
  endif
  lat_tele=stlat; lon_tele=stlon; dep_tele=stbur
  close(IIN)


!*****************source-side or deep-Earth coupling*******************
else if(COUPLINT_TYPE.eq.1 .or. COUPLINT_TYPE.eq.3) then
  CMTSOLUTION = IN_DATA_FILES(1:len_trim(IN_DATA_FILES))//'CMTSOLUTION'
  !Note that only one station allowed in CMTSOLUTION for SEM-DSM coupling mode
  open(unit = IIN,file=trim(CMTSOLUTION),status='old',action='read',iostat=ier)
  if (ier /= 0) then
  ...
  !read the longitude
  read(IIN,"(a)",iostat=ier) string
  read(string(10:len_trim(string)),*) lon_tele
  !read the depth
  read(IIN,"(a)",iostat=ier) string
  read(string(10:len_trim(string)),*) dep_tele
  close(IIN)
end if

end subroutine read_tele_source_dif_files
