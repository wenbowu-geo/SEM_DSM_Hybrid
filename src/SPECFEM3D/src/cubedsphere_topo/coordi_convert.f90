subroutine cubedspheretoxyz(x_cubed,y_cubed,x,y,z,rotation_matrix,R_TOP_BOUND,&
                            ANGULAR_WIDTH_XI_IN_DEGREES,ANGULAR_WIDTH_ETA_IN_DEGREES)
  implicit none
  include 'constants.h'


  double precision, intent(in):: R_TOP_BOUND,ANGULAR_WIDTH_XI_IN_DEGREES,ANGULAR_WIDTH_ETA_IN_DEGREES
...
subroutine xyztolatlon(x,y,z,R_TOP_BOUND,lat,lon)
  implicit none
  include 'constants.h'

  double precision, intent(in) ::x,y,z,R_TOP_BOUND
...
!auxiliary variables
  ...
  phi=acos(x/(R_TOP_BOUND*dsin(theta))*(1-TINYVAL))
  theta=theta*raddeg
  phi=phi*raddeg
!0-360deg
  if(y<TINYVAL) phi=-phi+360.0
