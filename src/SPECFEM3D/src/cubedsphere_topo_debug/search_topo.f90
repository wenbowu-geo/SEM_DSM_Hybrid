...
! Thus, converted to the range (-180,180) deg or (0,360) deg.
  ...
  if(icornerlat<1.or.icornerlat>nlat_interface-1.or.icornerlon<1.or. &
     ...
     write(*,*)lon,lat,lon_adjusted,orig_lon_interface
     write(*,*)icornerlat,nlat_interface,icornerlon,icornerlon
     stop 'the lat or lon_adjusted is out of the range covered by the topography file!'
  end if
  dtopo_dlat=0.0
  dtopo_dlon=0.0
  dlat=lat-(orig_lat_interface+(icornerlat-1)*spacing_lat_interface)
  dlon=lon_adjusted-(orig_lon_interface+(icornerlon-1)*spacing_lon_interface)
