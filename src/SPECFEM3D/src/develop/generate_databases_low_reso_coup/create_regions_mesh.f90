...
!WENBO_S
    if(COUPLING_TYPE.eq.1) then
      call pml_set_local_dampingcoeff(myrank,xstore_dummy_cubedsph,&
                       ystore_dummy_cubedsph,zstore_dummy_cubedsph)
!      call pml_set_local_dampingcoeff(myrank,xstore_dummy,ystore_dummy,zstore_dummy)
    else if(COUPLING_TYPE.eq.4) then
      call pml_set_local_dampingcoeff(myrank,xstore_dummy,ystore_dummy,zstore_dummy)
    else 
      stop 'C-PML is not installed for COUPLING_TYPE=2 or 3 yet!'
    end if
!WENBO_E
  endif
...
!!!###################save depth and disance id
!WENBO_S
!build depth list of solid media
  if(myrank==0) write(IMAIN,*) 'saving the depth table for DSM-SEM coupling'
  if(myrank==0) write(IMAIN,*) 'saving depths in elastic media'
!WENBO
  if(COUPLING_TYPE.eq.2.or.COUPLING_TYPE.eq.3) then
     call save_depth_id(COUPLING_TYPE,myrank,NPROC,num_abs_boundary_faces,&
                     abs_boundary_ispec,abs_boundary_ijk,ispec_is_elastic,&
                     ispec_is_acoustic,nspec,ibool,nglob,xstore_dummy,&
                     ystore_dummy,zstore_dummy,1,wzgll,IMAIN,&
                     prname,LOCAL_PATH)

  if(myrank==0) write(IMAIN,*) 'saving depths in acoustic media'
!acoustic media
     call save_depth_id(COUPLING_TYPE,myrank,NPROC,num_abs_boundary_faces,&
                     abs_boundary_ispec,abs_boundary_ijk,ispec_is_elastic,&
                     ispec_is_acoustic,nspec,ibool,nglob,xstore_dummy,&
                     ystore_dummy,zstore_dummy,2,wzgll,IMAIN,&
                     prname,LOCAL_PATH)
   end if


   if(COUPLING_TYPE.eq.1.or.COUPLING_TYPE.eq.3) then
      call SEMtoteleseis_savebound(myrank,kappastore,mustore,&
            ibool,nspec,nglob,NPROC,ispec_is_elastic,ispec_is_acoustic,&
            IMAIN,prname,LOCAL_PATH,&
...
! creates global indexing array ibool
  ...
  use generate_databases_par, only: NGLLX,NGLLY,NGLLZ,NDIM,COUPLING_TYPE,&
           PML_CONDITIONS
  use create_regions_mesh_ext_par
  use SEMtoTele_par,only : rotation_matrix_back_cubedsph,&
           ANGULAR_WIDTH_XI_IN_DEGREES,ANGULAR_WIDTH_ETA_IN_DEGREES,&
           xstore_dummy_cubedsph,ystore_dummy_cubedsph,zstore_dummy_cubedsph
  implicit none
...
!WENBO_S
  if(COUPLING_TYPE.eq.1.and.PML_CONDITIONS) then
    allocate(xstore_dummy_cubedsph(nglob_dummy), &
             ystore_dummy_cubedsph(nglob_dummy), &
             zstore_dummy_cubedsph(nglob_dummy),stat=ier)
    if (ier /= 0) stop 'error in allocate'
  end if
!WENBO_E
  do ispec = 1, nspec
...
!WENBO_S
              if(COUPLING_TYPE.eq.1.and.PML_CONDITIONS) then
                 call xyz_backto_cubedsph(xstore_dummy(iglobnum),&
                         ystore_dummy(iglobnum),zstore_dummy(iglobnum),&
                         xstore_dummy_cubedsph(iglobnum),ystore_dummy_cubedsph(iglobnum),&
                         zstore_dummy_cubedsph(iglobnum),ANGULAR_WIDTH_XI_IN_DEGREES, &
                         ANGULAR_WIDTH_ETA_IN_DEGREES,rotation_matrix_back_cubedsph)
              end if
!debuging. To be removed
!              if(myrank.eq.81) print *,"xyztocubed0",xstore_dummy(iglobnum),&
!                ystore_dummy(iglobnum),zstore_dummy(iglobnum),rotation_matrix_back_cubedsph
!WENBO_E
           enddo
        enddo
     enddo
