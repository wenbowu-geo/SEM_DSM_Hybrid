subroutine save_depth_id(COUPLING_TYPE,myrank,NPROC,num_abs_boundary_faces,&
  ...
  use SEMtoTele_par, only:LOW_RESOLUTION
  use constants
  implicit none
...
! start and end gll points with injected waves
  integer ::igll_start_inject,igll_end_inject
...
!reports. But it might be usful in the future, so we keep it here.
 ...
 int_tmp=IMAIN_OUTPUT
  ...
  if(LOW_RESOLUTION) then
     !injected waves only imposed on the central GLL points
     igll_start_inject=(NGLLSQUARE+1)/2
     igll_end_inject=igll_start_inject
  else
     !injected waves imposed on all the NGLLSQUARE points
     igll_start_inject=1
     igll_end_inject=NGLLSQUARE
  end if
...
!      write(113,*) dist_table_global(idist)/r*(180.d0/PI)
 ...
 if(myrank.eq.0) close(113)
  ...
  if(.not.LOW_RESOLUTION) then
    ipoint=0
    do iface=1,num_faces_this_mediatype
      do igll = igll_start_inject,igll_end_inject
       ipoint=ipoint+1
...
!coupling and receiver-side coupling. If receiver-side coupling chose, only
...
!Thus, Wenbo chose to simply use one file 'CMTSOLUTION' for both scenarios. 
...
if(COUPLINT_TYPE.eq.2) then
  ...
  !For receiver-side coupling, the teleseismic staiton becomes as a virtual
  !single-force source.
