...
!  double precision,dimension(:),allocatable :: lat_trench,lon_trench
  ...
  use constants
  character (len=80) :: TOMO_2D_FILENAME = 'tomography_model_2D.xyz'
      ! model dimensions
...
!  enddo
  ...
  integer :: irecord,ier
  integer ::idepth,iy,irecord0
  character(len=256):: filename
  filename = IN_DATA_FILES(1:len_trim(IN_DATA_FILES))//trim(TOMO_FILENAME_2D)
  open(unit=27,file=trim(filename),status='old',action='read',iostat=ier)
  if( ier /= 0 ) call exit_MPI(myrank,'error reading tomography file')
  ...
  nrecord = NDEPTH_2D*NY_2D
  ! allocates model records
  allocate(vp_tomography_2D(1:nrecord), &
          vs_tomography_2D(1:nrecord), &
          rho_tomography_2D(1:nrecord),stat=ier)
  if(ier /= 0) call exit_MPI(myrank,'not enough memory to allocate arrays')

  ! reads in record sections
  do irecord = 1,nrecord
    read(27,*) depth_tmp,y_tmp,vp_tmp,vs_tmp,rho_tmp

    ! stores record values
    vp_tomography_2D(irecord) = vp_tmp
    vs_tomography_2D(irecord) = vs_tmp
    rho_tomography_2D(irecord) = rho_tmp
  enddo
  ...
  ! user output
  if( myrank == 0 ) then
    write(IMAIN,*) '     2D tomography model: ',trim(TOMO_FILENAME)
  endif
...
subroutine add_sediment(myrank,x_eval,y_eval,z_eval,flag_media,fluid_thispoint,vp_final, &
                        vs_final,rho_final)
use sediment
implicit none

  double precision, intent(in) :: x_eval,y_eval,z_eval
  integer,intent(in) ::myrank,flag_media,fluid_thispoint
  real(kind=CUSTOM_REAL), intent(out) ::vp_final,vs_final,rho_final



  real(kind=CUSTOM_REAL) ::Radi,theta_surf,phi_surf
  integer ::ilat,ilon
...
!the code. myrank are not used now, that occationally causes error 
...
!1000 meters layer 1. Layer 2 has linearly increased thickness, from 0 at
!distance 0.224dge to 3900 meters at distance 0.5396deg.
...
subroutine taper_to_dsm_model(taper,depth,taper_depth_start,taper_depth_end, &
...
! 0.0 (becomes dsm model) below 'taper_depth_end'; Between these two depths, 
!it a linearly decreased taper.
double precision,intent(in) ::taper_depth_start,taper_depth_end
...
!Convert the unit of rad to degree. 
...
!*******taper along depth
if(depth.lt.taper_depth_start) then
   taper_depth=1.0
else if(depth.ge.taper_depth_start .and. depth.le.taper_depth_end) then
   taper_depth=1.0-(depth - taper_depth_start)/(taper_depth_end - taper_depth_start)
else
   taper_depth=0.0
end if
...
!finally, take account of the taperings along depth and edges
...
!crust 1.0 for Macquarie region
!1.500 0.000 1.020 2.710
!  1.750 0.340 1.820 0.040
!  5.000 2.700 2.550 0.700
!  6.500 3.700 2.850 1.400
!  7.100 4.050 3.050 4.900
!  8.040 4.470 3.310 0
...
subroutine add_oceanic_lithosphere(x,y,z,x_cubedsph,y_cubedsph,z_cubedsph,&
                    vp,vs,rho,myrank)
use bathymetry_model
use SEMtoTele_par, only:ANGULAR_WIDTH_XI_IN_DEGREES,ANGULAR_WIDTH_ETA_IN_DEGREES
use constants, only:CUSTOM_REAL,R_EARTH_SURF,PI
implicit none
double precision, intent(in)::x,y,z,x_cubedsph,y_cubedsph,z_cubedsph
real(kind=CUSTOM_REAL) ::vp,vs,rho
integer, intent(in) ::myrank

!specify these two depths (meters), from which local/tomography model is grapdually transitioned to
!the dsm model.
...
double precision, parameter ::taper_depth_start = 40000.0 
double precision, parameter ::taper_depth_end = 50000.0
...
if(ix_find<1.or.ix_find>nx_bathymetry-1) then
   print *,'x_cubedsph to find=',x_cubedsph/degtorad," degrees"
   call exit_MPI(myrank,'error target x_cubedsphere is out of bathymetry model range')
end if

if(iy_find<1.or.iy_find>ny_bathymetry-1) then
   print *,'y_cubedsph to find=',y_cubedsph/degtorad," degrees"
   call exit_MPI(myrank,'error target y_cubedsphere is out of bathymetry model range')
end if
...
if(myrank.eq.87.and.depth<2000.0) print *,'x_cubedsph-depth',depth,x_cubedsph_shift/degtorad,&
        y_cubedsph_shift/degtorad,bathymetry_this_point,ix_find,iy_find

!Note that negative batheymtry means below sea level while depth is positive.
depth_from_ocean_bot=depth+bathymetry_this_point
!get the vp, vs and rho of oceanic lithosphere
call oceanic_lithosphere_structure(depth_from_ocean_bot,vp_ols,vs_ols,rho_ols)

!tapering it to the dsm model at the bottom and edges of simulated model.
call taper_to_dsm_model(taper,depth,taper_depth_start,taper_depth_end, &
                              x_cubedsph,y_cubedsph,taper_width_edges,dsm_width_edges)
w_ols=taper
w_dsm=1.0-w_ols
...
!set up sediment close to trench
!  real(kind=CUSTOM_REAL) ::dist_min,dist
!  real(kind=CUSTOM_REAL) ::azim,bazim
!  real(kind=CUSTOM_REAL) ::dist_Ocbot,dist_Ocbot_square,dist_Ocbot_cube
!  real(kind=CUSTOM_REAL) ::sedi_thick_thispoint
!  real(kind=CUSTOM_REAL) ::depth_slabtop
!  integer ::itrench,itrench_find
!  integer ::sediment_units,vp_grd_accreprism,&
!            taper_prism,vp_prism
...
if(fluid_thispoint.eq.1.and.Radi.gt.6360000.0) then
!water
   vs_final=0.d0
   vp_final=1500.0
   rho_final=1000.0
...
!                 vp_final,vs_final,rho_final,myrank)
  ...
  if(oceanic_lithosphere_incorporated) then
   call add_oceanic_lithosphere(x_eval,y_eval,z_eval,xeval_cubedsph,yeval_cubedsph,zeval_cubedsph,&
                    vp_final,vs_final,rho_final,myrank)
  end if
 
  if(tomography_2D) then
  end if
...
!***********************find the perturbation***********************
  ...
  double precision ::Radi,depth,Rearth
...
!***********************find the perturbation***********************
  subroutine get_tomography_2D(x_eval,y_eval,z_eval, &
                             rho_final,vp_final,vs_final)
    use tomography_2D

    Radi=dsqrt(x_eval**2+y_eval**2+z_eval**2)
    depth_this_pt=R_EARTH_SURF-Radi

    spac_depth = (depth_this_pt - ORIG_DEPTH_2D) / SPACING_DEPTH_2D
    spac_y = (y_eval_proj - ORIG_Y_2D) / SPACING_Y_2D
  ...
  if(abs_VpVsRho) then
     ...
     vp_final=vp_final*(1.0+vp_tomo)
     vs_final=vs_final*(1.0+vs_tomo)
     rho_final=rho_final*(1.0+rho_tomo)
  end if
