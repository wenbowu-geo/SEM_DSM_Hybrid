...
!            taper_prism,vp_prism
  ...
  logical, parameter ::tomography_2D = .true.
...
!***********************find the perturbation***********************
  subroutine get_tomography_2D(x,y,z,x_cubedsph,y_cubedsph, &
    ...
    !specify these two depths (meters), from which local/tomography model is
    !grapdually transitioned to the dsm model.
    !iasp91 has a 35 km thick crust, so let's set it below 35 km, where they have
    !closer structures in the continent and oceanic lithospheres.
    ...
    !specify the below two widths for making the local-dsm model transitions at the
    !four edges.
    !This width gives the gradual tapering (in unit of degree and 0.1 degree is
    !roughly 11.2 km at the surface)
    double precision, parameter ::taper_width_edges = 0.2
    !DSM model is used in this outermost width (in unit of degree and 0.1 degree is
    !roughly 11.2 km at the surface)
    ...
    logical ::abs_VpVsRho
    double precision ::Radi,depth_this_pt
    ...
    !add taper at the bottom and edges of simulated model to make it smoothly 
    !transit to DSM model
