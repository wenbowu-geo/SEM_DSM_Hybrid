...
!other parameters
  ...
  integer,allocatable,dimension(:) ::iface_bound
...
!  character(len=256) clean_LOCAL_PATH
  ...
  data id_depth_elas_file /'id_depth_elastic_innerbound'/
  data id_depth_acous_file /'id_depth_acoustic_innerbound'/

  character(len=29) :: dist_table_elas_file
  character(len=30) :: dist_table_acous_file
  data dist_table_elas_file /'dist_table_elastic_innerbound'/
  data dist_table_acous_file /'dist_table_acoustic_innerbound'/

  character(len=30) :: depth_table_elas_file
  character(len=31) :: depth_table_acous_file
  data depth_table_elas_file /'depth_table_elastic_innerbound'/
  data depth_table_acous_file /'depth_table_acoustic_innerbound'/
...
!    if(DEBUG_COUPLING) print *,'iface',iface,ispec_is_elastic(ispec)
  ...
  if(num_faces_this_mediatype.gt.0) then
     allocate(iface_bound(num_faces_this_mediatype))
  else 
  ...
  if(DEBUG_COUPLING) print *,'depth_list_inner begin',myrank,npoint_bound
...
!  end if
  ...
  ipoint=0
  do iface_this_media=1,num_faces_this_mediatype
     iface=iface_bound(iface_this_media)
     ispec = Bound_Info(iface)%ispec_bound
     ...
     do k=kmin,kmax,dk
      do j=jmin,jmax,dj
       do i=imin,imax,di
          if(LOW_RESOLUTION) then
               ipoint=iface_this_media
               ipoint_allmedia=iface_bound(iface_this_media)
          else
               ipoint_iface=ipoint_iface+1
               ipoint=(iface_this_media-1)*NGLLX*NGLLY+ipoint_iface
               ipoint_allmedia=(iface-1)*NGLLX*NGLLY+ipoint_iface
          end if
          write(117,*) iface_bound(iface_this_media),depth_id(ipoint),dist_id(ipoint),dist_bound(ipoint)
...
!save depth and distance id
  ...
  if(num_faces_this_mediatype.gt.0) deallocate(iface_bound)
