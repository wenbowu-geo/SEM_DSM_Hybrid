subroutine SEMtoTele() 
  ...
  if(it.eq.1) then
     !call read_EleBound()
     !if(DEBUG_COUPLING) print *,'read TelePara'
     !call read_parameter_SEMtoTele(myrank)
!    if(myrank.eq.0) call save_parameter_coupling()
...
! if(DEBUG_COUPLING) print *,'calculate_strain_ela start',myrank,it
  if(DECIMATE_COUPLING.eq.1.or.mod(it,DECIMATE_COUPLING).eq.1) then
     ...
     if(myrank.eq.40.and.DEBUG_COUPLING) print *,'calculate_strain_ela',myrank,it
     call compute_pres_disp_acoustic()
!  call calculate_strain_acoustic()
!  if(DEBUG_COUPLING) print *,'calculate_strain_acou',myrank,it
     !call compute_pres_disp_acoustic()
     if(it.eq.1.and.myrank.eq.195.and.DEBUG_COUPLING) print *,'coord_disp1',&
                      xstore(ibool(2,2,1,604)),ystore(ibool(2,2,1,604)),&
                      zstore(ibool(2,2,1,604))
     if(it.eq.1.and.myrank.eq.195.and.DEBUG_COUPLING) print *,'coord_disp2',&
                      xstore(ibool(3,3,1,604)),ystore(ibool(3,3,1,604)),&
                      zstore(ibool(3,3,1,604))

     if(myrank.eq.195.and.DEBUG_COUPLING) print *,'bound_disp1',displ(:,ibool(2,2,1,604))
     if(myrank.eq.195.and.DEBUG_COUPLING) print *,'bound_disp2',displ(:,ibool(3,3,1,604))
   else
     it_bound=-1
   end if
!  if(DEBUG_COUPLING) print *,'velocity',myrank,it
   if( (it.gt.1.and.mod(it_bound,NSTEP_BETWEEN_OUTPUTBOUND).eq.0) .or. &
       (it.eq.NSTEP .and. mod((it-1)/DECIMATE_COUPLING+1,NSTEP_BETWEEN_OUTPUTBOUND).gt.0) ) then
!WENBO 
     if(myrank.eq.40.and.DEBUG_COUPLING) print *,"save_bound",it
     if(myrank.eq.40) print *,"save_bound",it,it_bound
