...
!WENBO
  use impose_1DBCS,only:inject_npt_eachpack,ipack_impose,ipack_old, &
...
!get_Telecoupling_info is executed in xgene
    ...
    if(CUBED_SPHERE_PROJECTION.and.(COUPLING_TYPE.eq.2.or.COUPLING_TYPE.eq.3)) then
      ...
      if(it.eq.1) then
        ...
        print *,'end init',inject_dt
        ipack_old=-1
      end if
      if(myrank.eq.1) print *,"time",(it-1)*deltat,int( ((it-1)*deltat -t0_this_pack)/inject_dt ) + 1
      ...
      if(ipack_impose.gt.ipack_old) then
        ...
        print *,'INJECTED_WAVEFIELD_PATH_rank',myrank,INJECTED_WAVEFIELD_PATH
      ...
      if(it_impose.le.0.or.it_impose.ge.inject_npt_eachpack) then
        print *,'t0',t0_this_pack,inject_dt,ipack_impose,it_impose,inject_npt_eachpack
