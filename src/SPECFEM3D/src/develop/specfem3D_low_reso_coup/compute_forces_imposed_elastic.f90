...
! apply 1-D tractions on the bounaries
  ...
  use constants
  !only for debug
  use impose_1DBCS, only:iDepDist_point
...
! local parameters
  ...
  real(kind=CUSTOM_REAL) ::nx,ny,nz,tx,ty,tz,jacobianw,sum_jacobian


  real(kind=CUSTOM_REAL) ,dimension(3)::velo1D

  integer :: ispec,iglob,i,j,k,iface,igll
  integer ::it_this_package
  ...
  do iface = 1,num_abs_boundary_faces
    ...
    if (ispec_is_elastic(ispec)) then
      do igll = 1,NGLLSQUARE
         sum_jacobian=abs_boundary_jacobian2Dw(igll,iface)
      end do

      ! reference GLL points on boundary face
      do igll = (NGLLSQUARE+1)/2,(NGLLSQUARE+1)/2
...
!#endif
        ...
        ! stacey term of scattering waves: velocity vector component * vp * rho in normal direction + vs * rho component tangential to it
...
!          tx=0.0;ty=0.0;tz=0.0
        ...
        accel(1,iglob) = accel(1,iglob) + (traction_imposed(1)-tx)*sum_jacobian
        accel(2,iglob) = accel(2,iglob) + (traction_imposed(2)-ty)*sum_jacobian
        accel(3,iglob) = accel(3,iglob) + (traction_imposed(3)-tz)*sum_jacobian
