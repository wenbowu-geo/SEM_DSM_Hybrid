...
!WENBO
  use impose_1DBCS,only:in_npt_eachpack,ipack_impose,ipack_old, &
      it_impose,t0_this_pack,in_dt
...
!get_Telecoupling_info is executed in xgene
       ...
       print *,'end read SEMtoTele_par',myrank,it,SINGLE_FORCE_ENZ
    end if

    if(CUBED_SPHERE_PROJECTION.and.(COUPLING_TYPE.eq.2.or.COUPLING_TYPE.eq.3)) then
      if(NSOURCES.lt.1) then
         call exit_MPI(myrank,'Now, only one source is allowed for impose_BCS mode')
      end if
      if(it.eq.1) then
        call synchronize_all()
        print *,'start init'
        call init_impose_BCS()
        print *,'end init',in_dt
        ipack_old=-1
      end if
      if(myrank.eq.1) print *,"time",(it-1)*deltat,int( ((it-1)*deltat -t0_this_pack)/in_dt ) + 1
      ipack_impose=int((it-1)*deltat/(in_dt*(in_npt_eachpack-1)))+1
      if(ipack_impose.gt.ipack_old) then
        t0_this_pack=(ipack_impose-1)*in_dt*(in_npt_eachpack-1)
        if(ipack_impose.eq.2)  then
          print *,'read new package',ipack_impose,t0_this_pack,(it-1)*deltat,int( ((it-1)*deltat - t0_this_pack)/in_dt ) + 1
        end if
        call read_1Dboundary_values(ipack_impose,myrank,SINGLE_FORCE_ENZ)
        ipack_old=ipack_impose
      end if
      it_impose=int( ((it-1)*deltat - t0_this_pack)/in_dt ) + 1
      if(it_impose.le.0.or.it_impose.ge.in_npt_eachpack) then
        print *,'t0',t0_this_pack,in_dt,ipack_impose,it_impose,in_npt_eachpack
        call exit_MPI(myrank,'Error,it_impose>in_npt_eachpack-1 or <1')
      end if
    end if
!WENBO_E
