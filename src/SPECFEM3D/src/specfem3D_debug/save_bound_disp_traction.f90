...
!  endif
  ...
  do iface_bound=1,nele_tele_coupling

   ispec=coupling_ele_property(iface_Bound)%ispec_coupling
   imin=coupling_ele_property(iface_Bound)%istart;imax=coupling_ele_property(iface_Bound)%iend
   jmin=coupling_ele_property(iface_Bound)%jstart;jmax=coupling_ele_property(iface_Bound)%jend
   kmin=coupling_ele_property(iface_Bound)%kstart;kmax=coupling_ele_property(iface_Bound)%kend
...
!                   action='write',form='formatted',iostat=ier2)
                 open(unit=11,file=trim(disp_bound_file),status='replace',&
                   action='write',access='stream',form='unformatted',iostat=ier1)
                 open(unit=21,file=trim(traction_bound_file),status='replace',&
...
!---------------------------------------------------------------------
!save the strains for building Green's functions in the next step using the
!reciprocity.
subroutine save_strain_selected()
  ...
  integer ::ier1,it_tmp
  character(len=256) ::final_LOCAL_PATH,clean_LOCAL_PATH
  character(len=256) ::dir_this_proc,strain_file
...
!  endif
  ...
  if( ier1 /= 0 ) then
      ...
      call exit_mpi(myrank,'error opening Green_Strain file')
  endif
     ...
     do k=kmin,kmax
       do j=jmin,jmax
         do i=imin,imax
            ...
            !do it_tmp=1,NSTEP_BETWEEN_OUTPUTBOUND
                !write(11,*)
                !'six-comp',ipoint_saved,strain_saved(it_tmp,1,ipoint_saved),strain_saved(it_tmp,2,ipoint_saved)
            !end do
       end do
     end do
   end do
