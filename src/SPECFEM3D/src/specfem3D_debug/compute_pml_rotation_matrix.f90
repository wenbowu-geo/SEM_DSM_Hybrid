...
use constants, only:PI,CPML_Z_ONLY,CPML_X_ONLY,CPML_XZ_ONLY,NGLLX,&
        NGLLY,NGLLZ,TINYVAL,TINYVAL_SNGL,CUSTOM_REAL,NDIM,DEBUG_COUPLING
use specfem_par, only:ibool,xstore,ystore,zstore
use tele_coupling_par, only:CENTER_LONGITUDE_IN_DEGREES,&
...
if(myrank.eq.0.and.DEBUG_COUPLING) print *,"rot_matrix_ximin",pml_rotation_matrix_ximin(:,1),&
  pml_rotation_matrix_ximin(:,2),pml_rotation_matrix_ximin(:,3)
if(myrank.eq.0.and.DEBUG_COUPLING) print *,"rot_matrix_ximax",pml_rotation_matrix_ximax(:,1),&
        pml_rotation_matrix_ximax(:,2),pml_rotation_matrix_ximax(:,3)
if(myrank.eq.0.and.DEBUG_COUPLING) print *,"rot_matrix_etamin",pml_rotation_matrix_etamin(:,1),&
  pml_rotation_matrix_etamin(:,2),pml_rotation_matrix_etamin(:,3)
if(myrank.eq.0.and.DEBUG_COUPLING) print *,"rot_matrix_etamax",pml_rotation_matrix_etamax(:,1),&
...
!cubed sphere coordinates associated with this element itself
     ...
     if(myrank.eq.0.and.DEBUG_COUPLING) print *,"rot_matrix_Z",pml_rotation_matrix(:,:,ispec_pml)
!to be fixed
  else 
