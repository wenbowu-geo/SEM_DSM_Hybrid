...
subroutine pml_compute_memory_variables_elastic(ispec,ispec_CPML,tempx1,tempy1,tempz1,tempx2,tempy2,tempz2, &
  ...
  use specfem_par, only: deltat,xix,xiy,xiz,etax,etay,etaz,gammax,gammay,gammaz,jacobian, &
                         kappastore,mustore,CUBED_SPHERE_PROJECTION
  use pml_par, only: NSPEC_CPML,CPML_regions,k_store_x,k_store_y,k_store_z, &
                     ...
                     PML_duz_dxl_new, PML_duz_dyl_new, PML_duz_dzl_new,pml_rotation_matrix

!WENBO_S
!  use tele_coupling_par, only:rotation_matrix
!WENBO_E
...
!WENBO_S
  real(kind=CUSTOM_REAL), dimension(3,3) ::PML_strain,PML_strain_cubedsph,PML_strain_old,&
             PML_strain_cubedsph_old,PML_strain_new,PML_strain_cubedsph_new,array_tmp,sigma_array
  real(kind=CUSTOM_REAL),dimension(3,3) ::rotation_matrix
!WENBO_E

!WENBO_S
if(CUBED_SPHERE_PROJECTION) rotation_matrix(:,:)=pml_rotation_matrix(:,:,ispec_CPML)
!WENBO_E
...
!WENBO_S
       PML_strain(1,1)=PML_dux_dxl(i,j,k)
       PML_strain(1,2)=PML_dux_dyl(i,j,k)
       PML_strain(1,3)=PML_dux_dzl(i,j,k)
       PML_strain(2,1)=PML_duy_dxl(i,j,k)
       PML_strain(2,2)=PML_duy_dyl(i,j,k)
       PML_strain(2,3)=PML_duy_dzl(i,j,k)
       PML_strain(3,1)=PML_duz_dxl(i,j,k)
       PML_strain(3,2)=PML_duz_dyl(i,j,k)
       PML_strain(3,3)=PML_duz_dzl(i,j,k)
       PML_strain_old(1,1)=PML_dux_dxl_old(i,j,k)
       PML_strain_old(1,2)=PML_dux_dyl_old(i,j,k)
       PML_strain_old(1,3)=PML_dux_dzl_old(i,j,k)
       PML_strain_old(2,1)=PML_duy_dxl_old(i,j,k)
       PML_strain_old(2,2)=PML_duy_dyl_old(i,j,k)
       PML_strain_old(2,3)=PML_duy_dzl_old(i,j,k)
       PML_strain_old(3,1)=PML_duz_dxl_old(i,j,k)
       PML_strain_old(3,2)=PML_duz_dyl_old(i,j,k)
       PML_strain_old(3,3)=PML_duz_dzl_old(i,j,k)
       PML_strain_new(1,1)=PML_dux_dxl_new(i,j,k)
       PML_strain_new(1,2)=PML_dux_dyl_new(i,j,k)
       PML_strain_new(1,3)=PML_dux_dzl_new(i,j,k)
       PML_strain_new(2,1)=PML_duy_dxl_new(i,j,k)
       PML_strain_new(2,2)=PML_duy_dyl_new(i,j,k)
       PML_strain_new(2,3)=PML_duy_dzl_new(i,j,k)
       PML_strain_new(3,1)=PML_duz_dxl_new(i,j,k)
       PML_strain_new(3,2)=PML_duz_dyl_new(i,j,k)
       PML_strain_new(3,3)=PML_duz_dzl_new(i,j,k)

!!!!!!From global coordinates to cubed_sphere coordinates.
!The following results is different from
!array_tmp=matmul(rotation_matrix,matmul(PML_strain,transpose(rotation_matrix))),
!if sigma_array is not symmetrical matrix.
       array_tmp=matmul(transpose(rotation_matrix),matmul(PML_strain,rotation_matrix))
       PML_strain_cubedsph=array_tmp
       array_tmp=matmul(transpose(rotation_matrix),matmul(PML_strain_old,rotation_matrix))
       PML_strain_cubedsph_old=array_tmp
       array_tmp=matmul(transpose(rotation_matrix),matmul(PML_strain_new,rotation_matrix))
       PML_strain_cubedsph_new=array_tmp

!       print *,"check_du_dx",CPML_region_local,d_store_x(i,j,k,ispec_CPML),&
!                 d_store_y(i,j,k,ispec_CPML),d_store_z(i,j,k,ispec_CPML),&
!        PML_dux_dxl(i,j,k), PML_dux_dyl(i,j,k), PML_dux_dzl(i,j,k),&
!           PML_duy_dxl(i,j,k),PML_duy_dyl(i,j,k), PML_duy_dzl(i,j,k), &
!       PML_duz_dxl(i,j,k), PML_duz_dyl(i,j,k), PML_duz_dzl(i,j,k),'rotation',PML_strain_cubedsph(1,:),&
!           PML_strain_cubedsph(2,:),PML_strain_cubedsph(3,:)

       PML_dux_dxl(i,j,k)=PML_strain_cubedsph(1,1)
       PML_dux_dyl(i,j,k)=PML_strain_cubedsph(1,2)
       PML_dux_dzl(i,j,k)=PML_strain_cubedsph(1,3)
       PML_duy_dxl(i,j,k)=PML_strain_cubedsph(2,1)
       PML_duy_dyl(i,j,k)=PML_strain_cubedsph(2,2)
       PML_duy_dzl(i,j,k)=PML_strain_cubedsph(2,3)
       PML_duz_dxl(i,j,k)=PML_strain_cubedsph(3,1)
       PML_duz_dyl(i,j,k)=PML_strain_cubedsph(3,2)
       PML_duz_dzl(i,j,k)=PML_strain_cubedsph(3,3)
       PML_dux_dxl_old(i,j,k)=PML_strain_cubedsph_old(1,1)
       PML_dux_dyl_old(i,j,k)=PML_strain_cubedsph_old(1,2)
       PML_dux_dzl_old(i,j,k)=PML_strain_cubedsph_old(1,3)
       PML_duy_dxl_old(i,j,k)=PML_strain_cubedsph_old(2,1)
       PML_duy_dyl_old(i,j,k)=PML_strain_cubedsph_old(2,2)
       PML_duy_dzl_old(i,j,k)=PML_strain_cubedsph_old(2,3)
       PML_duz_dxl_old(i,j,k)=PML_strain_cubedsph_old(3,1)
       PML_duz_dyl_old(i,j,k)=PML_strain_cubedsph_old(3,2)
       PML_duz_dzl_old(i,j,k)=PML_strain_cubedsph_old(3,3)
       PML_dux_dxl_new(i,j,k)=PML_strain_cubedsph_new(1,1)
       PML_dux_dyl_new(i,j,k)=PML_strain_cubedsph_new(1,2)
       PML_dux_dzl_new(i,j,k)=PML_strain_cubedsph_new(1,3)
       PML_duy_dxl_new(i,j,k)=PML_strain_cubedsph_new(2,1)
       PML_duy_dyl_new(i,j,k)=PML_strain_cubedsph_new(2,2)
       PML_duy_dzl_new(i,j,k)=PML_strain_cubedsph_new(2,3)
       PML_duz_dxl_new(i,j,k)=PML_strain_cubedsph_new(3,1)
       PML_duz_dyl_new(i,j,k)=PML_strain_cubedsph_new(3,2)
       PML_duz_dzl_new(i,j,k)=PML_strain_cubedsph_new(3,3)
!WENBO_E
...
!WENBO_S
        sigma_array(1,1)=sigma_xx
        sigma_array(1,2)=sigma_xy
        sigma_array(1,3)=sigma_xz
        sigma_array(2,1)=sigma_yx
        sigma_array(2,2)=sigma_yy
        sigma_array(2,3)=sigma_yz
        sigma_array(3,1)=sigma_zx
        sigma_array(3,2)=sigma_zy
        sigma_array(3,3)=sigma_zz

!!!!!!From cubed_sphere coordinates to global coordinates.
!The following results is different from
!array_tmp=matmul(transpose(rotation_matrix),matmul(sigma_array,rotation_matrix)),
!if sigma_array is not symmetrical matrix.
        array_tmp=matmul(rotation_matrix,matmul(sigma_array,transpose(rotation_matrix)))
        sigma_xx=array_tmp(1,1)
        sigma_xy=array_tmp(1,2)
        sigma_xz=array_tmp(1,3)
        sigma_yx=array_tmp(2,1)
        sigma_yy=array_tmp(2,2)
        sigma_yz=array_tmp(2,3)
        sigma_zx=array_tmp(3,1)
        sigma_zy=array_tmp(3,2)
        sigma_zz=array_tmp(3,3)
!WENBO_E
