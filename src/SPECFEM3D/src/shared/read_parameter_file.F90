...
!=====================================================================
  ...
  !WENBO_S
    call read_value_logical(CUBED_SPHERE_PROJECTION, 'CUBED_SPHERE_PROJECTION',ier)
    if(ier /= 0)  stop 'Error reading Par_file parameter CUBED_SPHERE_PROJECTION'
    call read_value_integer(COUPLING_TYPE, 'COUPLING_TYPE',ier)
    if(ier /= 0)  stop 'Error reading Par_file parameter COUPLING_TYPE'
    if(CUBED_SPHERE_PROJECTION.and.COUPLING_TYPE.eq.4) then
       stop 'Error, cubed-sphere coordinates not implemented for COUPLING_TYPE=4'
    end if
    call read_value_integer(SINGLE_FORCE_ENZ, 'SINGLE_FORCE_ENZ',ier)
    if(ier /= 0)  stop 'Error reading Par_file parameter SINGLE_FORCE_ENZ'
    call read_value_string(INJECTED_WAVEFIELD_PATH, 'INJECTED_WAVEFIELD_PATH',ier)
    if(ier /= 0)  stop 'Error reading Par_file parameter PATH_INJECTED_WAVEFIELD'
  !WENBO_E
  ...
  !! \author MPBL
    ...
    !WENBO-S
       if(COUPLING_TYPE.lt.4) then
         if(IMODEL .ne. IMODEL_DSM1D .and. IMODEL.ne.IMODEL_DSM3D) &
           stop 'COUPLING_TYPE=1,2 or 3, MODEL has to be dsm1d or dsm3d'
...
!WENBO_S
    call bcast_all_singlel_world(CUBED_SPHERE_PROJECTION)
    call bcast_all_singlei_world(COUPLING_TYPE)
    call bcast_all_singlei_world(SINGLE_FORCE_ENZ)
    call bcast_all_string_world(INJECTED_WAVEFIELD_PATH)
!WENBO_E
