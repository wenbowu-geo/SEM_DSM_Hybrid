...
! other variables
   ...
   integer ::n_refine
   character(len=MAX_STRING_LEN) ::para_file,para_file_SEM_Par,GREEN_par_file
   ...
   npoints_max=npoints_per_pack_SEM*npack_per_proc
   ...
   ! Find the sampling interval 'deltat_refine' to do resampling of SEM wavefield (see the notes)
...
subroutine read_DSM_depths(dir_DSM_input,myrank)
   ...
   ! ddepth and ddep are used in DSM and not functional here.
  ...
  do idepth=1,ndep_acous_DSM
   ...
   open( unit=12, file=depth_file, status='old' )
   ! ddepth and ddep used in DSM and not functional here.
   read(12,*) ddep_tmp,depth_tole_tmp 
   read(12,*) ndep_tmp
...
subroutine read_SEM_depths(dir_SEM_input)
   ...
   ! ddep_tmp,depth_tole is used in DSM and not functional here.
...
subroutine read_package_id(dir_SEM_input,igroup)
   use coupling_SEM_DSM_par, only:package_list_file_SEM,npackage_SEM,&
           ...
           npoint_pack,ipack_start,myrank
   use constants, only: MAX_STRING_LEN
   implicit none
   ...
   npoints=0
   if(npack(igroup).gt.0) then
      do ipack=1,npack(igroup)
        if(ipack.eq.1) then
         ipoint_start(ipack)=1
        else 
         ipoint_start(ipack)=ipoint_start(ipack-1)+npoint_pack(ipack-1+ipack_start(igroup)-1)
        end if
      end do
      npoints=ipoint_start(npack(igroup))+npoint_pack(npack(igroup)+ipack_start(igroup)-1)-1
   end if
