...
subroutine read_DSMsrc_elastic(ifreq)
    use convolution_par
    use coupling_SEM_DSM_par, only:myrank,dir_DSMsrc_input,disp_elas_DSM_toSrc,&
          stress_DSM_toSrc
    use constants
    implicit none
...
! other variables
    character(len=MAX_STRING_LEN) ::green_disp_file,green_stress_file,file_tmp
    integer ::icomp,ifreq,ios


    write(file_tmp,"('/OUTPUT_FILES/disp_solid/freq_',i5.5)") ifreq-1
    green_disp_file=trim(dir_DSMsrc_input(1:len_trim(dir_DSMsrc_input)))// '/' // &
       trim(file_tmp(1:len_trim(file_tmp)))
!     open(unit=15,file=trim(green_disp_file),status='unknown',
...
!code-velo_stress_quadra
    do icomp=1,3
      read(15) disp_elas_DSM_toSrc(:,icomp)
    end do
    close(15)

    write(file_tmp,"('/OUTPUT_FILES/stress/freq_',i5.5)") ifreq-1
    green_stress_file=trim(dir_DSMsrc_input(1:len_trim(dir_DSMsrc_input)))// '/' // &
       trim(file_tmp(1:len_trim(file_tmp)))
...
!      open(unit=16,file=trim(green_stress_file),status='unknown',form='formatted',iostat=ios)
    ...
    if(ios /= 0) stop 'error reading GREENS_EPSILON'
    do icomp=1,ncomp_stress
       read(16)stress_DSM_toSrc(:,icomp)
    end do
    close(16)
end subroutine read_DSMsrc_elastic

subroutine read_DSMsrc_acoustic(ifreq)
    use convolution_par
    use coupling_SEM_DSM_par, only:myrank,dir_DSMsrc_input,&
...
! other variables
    ...
    write(file_tmp,"('/OUTPUT_FILES/disp_fluid/freq_',i5.5)") ifreq-1
    green_disp_file=trim(dir_DSMsrc_input(1:len_trim(dir_DSMsrc_input)))// '/' // &
         trim(file_tmp(1:len_trim(file_tmp)))
...
!code-velo_stress_quadra
    do icomp=1,3
      read(15) disp_acous_DSM_toSrc(:,icomp)
    end do
    close(15)

    write(file_tmp,"('/OUTPUT_FILES/pressure/freq_',i5.5)") ifreq-1
    green_pressure_file=trim(dir_DSMsrc_input(1:len_trim(dir_DSMsrc_input)))// '/' // &
       trim(file_tmp(1:len_trim(file_tmp)))

!     open(unit=16,file=trim(green_pressure_file),status='unknown',form='formatted',iostat=ios)
    open(unit=16,file=trim(green_pressure_file),status='unknown',form='unformatted',iostat=ios)
    if(ios /= 0) stop 'error reading GREENS_EPSILON'
    read(16)pressure_DSM_toSrc(:)
    close(16)

end subroutine read_DSMsrc_acoustic
