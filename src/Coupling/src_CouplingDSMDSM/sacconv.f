...
c variables for input/output
        ...
        implicit none
	integer n_frequency,nstation
	double precision time_series_length,omega_imag
	double precision source_depth,source_lat,source_lon
	double precision station_lat(nstation),station_lon(nstation)
	complex*16 station_displacement(3,n_frequency)
	real work_time(32*n_frequency)
	complex*16 work_spc(16*n_frequency)
	character*256 sac_file(nstation)
c other variables
	integer i_station,icomp,i_frequency,ismooth,n1,m1,nn,isign,i
ccccccc    integer id_seismo
	integer lnblnk
	double precision t
	character*4 cext(3)
...
c
	data ismooth / 8 /
	data cext / '.bhe','.bhn','.bhz' /
c
	  do 900 icomp=1,3
ccc WENBO- we modify the following lines. 
ccc	    work_spc(1) = dcmplx(0.d0)
	    do 200 i_frequency=1,n_frequency
	      work_spc(i_frequency)
     &	        = station_displacement(icomp,i_frequency)
  200	    continue
...
c --- km -> m
ccc	      work_time(i+nn) = work_time(i+nn) * 1.e3
  230	    continue
	   outfile
     &	    ="OUTPUT_FILES/"
     &          //sac_file(i_station)(1:lnblnk(sac_file(i_station)))
     &	        //cext(icomp)
c            print *,'output_station',outfile
...
c
	...
	dist = 0.0
        stla_tmp(1)=stla
        stlo_tmp(1)=stlo
	call distaz1(evla,evlo,stla_tmp,stlo_tmp,1,dist_tmp,
     &   az_tmp,baz_tmp,gcarc_tmp,ier)
        dist=dist_tmp(1)
        az=az_tmp(1)
        baz=baz_tmp(1)
        gcarc=gcarc_tmp(1)
...
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!This subroutine is stolen from dsmti-2.2.9 package by TAKEUCHI, Nozomu 
!
