!This 2d Lagrange interpolation subroutine is from the below website
...
!
!    Output, real ( kind = 8 ) YI, the value of the I-th Lagrange 1D
!    basis 
...
!*****************************************************************************80
!
!! LAGRANGE_INTERP_2D evaluates the Lagrange interpolant for a product
!grid.
!
!  Licensing:
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) MX, MY, the polynomial degree in X and
!    Y.
!
!    Input, real ( kind = 8 ) XD_1D(MX+1), YD_1D(MY+1), the 1D data
!    locations.
!
!    Input, real ( kind = 8 ) ZD((MX+1),(MY+1)), the 2D array of data
!    values.
!
!    Input, integer ( kind = 4 ) NI, the number of 2D interpolation
!    points.
!
!    Input, real ( kind = 8 ) XI(NI), YI(NI), the 2D interpolation
!    points.
!
