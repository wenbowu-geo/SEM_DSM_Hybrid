...
use coupling_SEM_DSM_par,only:nproc,myrank,npack,fft_disp_bound_new,fft_traction_bound_new,&
              ...
              disp_elas_DSM_toSrc,stress_DSM_toSrc,disp_acous_DSM_toSrc,pressure_DSM_toSrc,&
              depths_elas_DSM_toSrc,zones_elas_DSM_toSrc,ndep_elas_DSM_toSrc,depths_acous_DSM_toSrc,&
              zones_acous_DSM_toSrc,ndep_acous_DSM_toSrc,ndist_DSM_toSrc,min_dist_DSM_toSrc,ddist_DSM_toSrc
...
double precision::depth_this_point,dist_this_point,dist_this_point_toSrc
...
double precision,dimension(ncomp,ncomp) ::rot1,rot2,rot_src
...
complex(kind=8),dimension(ncomp) ::disp_sphe_SrcToBox
complex(kind=8),dimension(ncomp,ncomp,ncomp) ::stress_sphe
complex(kind=8),dimension(ncomp,ncomp) ::stress_sphe_SrcToBox
complex(kind=8),dimension(ncomp,ncomp) ::traction_Green_carte
complex(kind=8),dimension(ncomp) ::traction_SrcToBox_carte
complex(kind=8) ::term1,term2

!used for lagrange interpolation scheme
integer ::ifit

character(len=MAX_STRING_LEN) ::file_check
...
!get stress and displacement from the source to the SEM box
!way 1 - copy from SEM
   !my_disp_bound(:)=fft_disp_bound_new(ifreq,:,ipoint)
   !my_traction_bound(:)=fft_traction_bound_new(ifreq,:,ipoint)
!way 2 - interpolated from DSM. Note that this is only for benchmark.

   dist_this_point_toSrc=distance_toSrc(ipoint)
   if(ifreq.eq.1) then
        if(is_elastic(ipoint)) then
          zone_this_point=zones_solid_SEM(id_depth_SEM(ipoint))
          call get_locDSM_fit(depth_this_point,dist_this_point_toSrc,zone_this_point,nfit_dep,&
                    nfit_dist,min_dist_DSM_toSrc,ddist_DSM_toSrc,ndist_DSM_toSrc,depths_elas_DSM_toSrc,zones_elas_DSM_toSrc,&
                    ndep_elas_DSM_toSrc,idep0_fit_DSM,idist0_fit_DSM)
        else
          zone_this_point=zones_fluid_SEM(id_depth_SEM(ipoint))
          call get_locDSM_fit(depth_this_point,dist_this_point_toSrc,zone_this_point,nfit_dep,&
                    nfit_dist,min_dist_DSM_toSrc,ddist_DSM_toSrc,ndist_DSM_toSrc,depths_acous_DSM_toSrc,zones_acous_DSM_toSrc,&
                    ndep_acous_DSM_toSrc,idep0_fit_DSM,idist0_fit_DSM)
        end if
        first_idep_fit_DSM_toSrc(ipoint)=idep0_fit_DSM
        first_idist_fit_DSM_toSrc(ipoint)=idist0_fit_DSM
   end if

   do ifit_dep=1,nfit_dep
     if(is_elastic(ipoint)) then
       depths_fit_in(ifit_dep)=depths_elas_DSM_toSrc(first_idep_fit_DSM_toSrc(ipoint)+ifit_dep-1)
     else
       depths_fit_in(ifit_dep)=depths_acous_DSM_toSrc(first_idep_fit_DSM_toSrc(ipoint)+ifit_dep-1)
     end if
   end do

   do ifit_dist=1,nfit_dist
     dists_fit_in(ifit_dist)=min_dist_DSM_toSrc+(first_idist_fit_DSM_toSrc(ipoint)-1 +ifit_dist-1)*ddist_DSM_toSrc
   end do

   do icomp_disp=1,ncomp
      do ifit_dep=1,nfit_dep
         do ifit_dist=1,nfit_dist
            idGreen=(first_idep_fit_DSM_toSrc(ipoint)-1 + ifit_dep-1)*ndist_DSM_toSrc + &
                     first_idist_fit_DSM_toSrc(ipoint)+ifit_dist-1
            if(is_elastic(ipoint)) then
              imag_z_in(ifit_dep,ifit_dist)=aimag(disp_elas_DSM_toSrc(idGreen,icomp_disp))
              real_z_in(ifit_dep,ifit_dist)=real(disp_elas_DSM_toSrc(idGreen,icomp_disp))
            else
              imag_z_in(ifit_dep,ifit_dist)=aimag(disp_acous_DSM_toSrc(idGreen,icomp_disp))
              real_z_in(ifit_dep,ifit_dist)=real(disp_acous_DSM_toSrc(idGreen,icomp_disp))
            end if
         end do
      end do
      call lagrange_interp_2d(nfit_dep-1,nfit_dist-1,depths_fit_in,dists_fit_in,imag_z_in,1,&
                                 depth_this_point,dist_this_point_toSrc,imag_z_out)
      call lagrange_interp_2d(nfit_dep-1,nfit_dist-1,depths_fit_in,dists_fit_in,real_z_in,1,&
                                 depth_this_point,dist_this_point_toSrc,real_z_out)
      disp_sphe_SrcToBox(icomp_disp)=cmplx(real_z_out,imag_z_out)
      if(myrank.eq.6.and.ipoint.eq.1) then
!             print
!             *,'disp_check',disp_Green_sphe(icomp_disp,icomp_force),icomp_disp,icomp_force,ifreq
!            print *,'disp_DSMsrc_sphe',disp_Green_sphe(icomp_disp,icomp_force),&
!                first_idep_fit_DSM(ipoint,ista),first_idist_fit_DSM(ipoint,ista),min_dist_DSM,&
!                depth_this_point,icomp_disp,icomp_force,dists_fit_in,dist_this_point,'imag',imag_z_in(2,:),&
!                'real',real_z_in(2,:)
      end if
             ...
             do icomp2_stress=1,ncomp
                do ifit_dep=1,nfit_dep
                   do ifit_dist=1,nfit_dist
                      ...
                      imag_z_in(ifit_dep,ifit_dist)=aimag(stress_DSM_toSrc(idGreen,&
                                             icomp_sigma_table(icomp1_stress,icomp2_stress)))
                      real_z_in(ifit_dep,ifit_dist)=real(stress_DSM_toSrc(idGreen, &
                                             icomp_sigma_table(icomp1_stress,icomp2_stress)))
                   end do
                end do
                call lagrange_interp_2d(nfit_dep-1,nfit_dist-1,depths_fit_in,dists_fit_in,imag_z_in,1,&
                ...
                call lagrange_interp_2d(nfit_dep-1,nfit_dist-1,depths_fit_in,dists_fit_in,real_z_in,1,&
                                    depth_this_point,dist_this_point_toSrc,real_z_out)
                stress_sphe_SrcToBox(icomp1_stress,icomp2_stress)=cmplx(real_z_out,imag_z_out)
                if(myrank.eq.6.and.ipoint.eq.1.and.icomp1_stress.eq.1.and.icomp2_stress.eq.1) then
!                   print
!                   *,'stress',stress_sphe(icomp1_stress,icomp2_stress,icomp_force),icomp_force,ifreq
!                  print
!                  *,'stress',stress_sphe(icomp1_stress,icomp2_stress,icomp_force),icomp1_stress,icomp2_stress,&
!                      icomp_force
                end if
          ...
          do ifit_dep=1,nfit_dep
             do ifit_dist=1,nfit_dist
                ...
                imag_z_in(ifit_dep,ifit_dist)=aimag(pressure_DSM_toSrc(idGreen))
                real_z_in(ifit_dep,ifit_dist)=real(pressure_DSM_toSrc(idGreen))
              end do
           end do
           call lagrange_interp_2d(nfit_dep-1,nfit_dist-1,depths_fit_in,dists_fit_in,imag_z_in,1,&
                               depth_this_point,dist_this_point_toSrc,imag_z_out)
           call lagrange_interp_2d(nfit_dep-1,nfit_dist-1,depths_fit_in,dists_fit_in,real_z_in,1,&
                               depth_this_point,dist_this_point_toSrc,real_z_out)
           stress_sphe_SrcToBox(:,:)= cmplx(0.d0)
           stress_sphe_SrcToBox(1,1)= cmplx(real_z_out,imag_z_out)
           stress_sphe_SrcToBox(2,2)=stress_sphe_SrcToBox(1,1)
           stress_sphe_SrcToBox(3,3)=stress_sphe_SrcToBox(1,1)
        end if !is_elastic(ipoint)


   rot_src(:,:)=rot_matrix_bound_toSrc(:,:,ipoint)
   my_disp_bound(:)=matmul(transpose(rot_src),disp_sphe_SrcToBox)
   call calcu_traction_SrcToBox(traction_SrcToBox_carte,rot_src, &
               normal_vector(1,ipoint),stress_sphe_SrcToBox(1,1),ncomp)
   my_traction_bound(:)=traction_SrcToBox_carte(:)
   if(myrank.eq.40.and.ipoint.eq.3) then
     write(file_check,"('OUTPUT_FILES/freq_',i5.5)")ifreq
     open(unit=40,file=file_check,action='write',form="formatted",status="unknown")
     write(40,*)depth_this_point,dist_this_point_toSrc,is_elastic(ipoint)
     write(40,*)my_disp_bound(1),fft_disp_bound_new(ifreq,1,ipoint)
     write(40,*)my_disp_bound(2),fft_disp_bound_new(ifreq,2,ipoint)
     write(40,*)my_disp_bound(3),fft_disp_bound_new(ifreq,3,ipoint)
     write(40,*)my_traction_bound(1),fft_traction_bound_new(ifreq,1,ipoint),stress_sphe_SrcToBox(1,1)
     write(40,*)my_traction_bound(2),fft_traction_bound_new(ifreq,2,ipoint),stress_sphe_SrcToBox(2,2)
     write(40,*)my_traction_bound(3),fft_traction_bound_new(ifreq,3,ipoint),stress_sphe_SrcToBox(3,3)
     close(40)
   end if
...
!to fix
!     if(ipoint.eq.1.and.ista.eq.1) dist_this_point=145.160001 

     !get the interpolating DSM depths and distances
     !only need to do it one time when ifreq=1
     if(ifreq.eq.1) then
        if(is_elastic(ipoint)) then
          zone_this_point=zones_solid_SEM(id_depth_SEM(ipoint))
     ...
     !to fix
!     if(myrank.eq.0.and.ipoint.eq.1.and.ista.eq.1.and.ifreq.eq.10) &
!        print *,"check_idep_idist",first_idep_fit_DSM(ipoint,ista),first_idist_fit_DSM(ipoint,ista),&
!           distance(ipoint,ista),id_depth_SEM(ipoint)
...
!     if(myrank.eq.34) print *,'idGreen_iproc41',idGreen,ista,normal_vector(:,ipoint)
     do icomp_force=1,ncomp
        ...
        do icomp_disp=1,ncomp
          ...
          if(myrank.eq.6.and.ipoint.eq.1.and.ista.eq.1.and.icomp_disp.eq.1.and.icomp_force.eq.1) then
!             print *,'disp_check',disp_Green_sphe(icomp_disp,icomp_force),icomp_disp,icomp_force,ifreq
!            print *,'disp_Green_sphe',disp_Green_sphe(icomp_disp,icomp_force),&
!                first_idep_fit_DSM(ipoint,ista),first_idist_fit_DSM(ipoint,ista),min_dist_DSM,&
!                depth_this_point,icomp_disp,icomp_force,dists_fit_in,dist_this_point,'imag',imag_z_in(2,:),&
!                'real',real_z_in(2,:)
          end if
        ...
        if(is_elastic(ipoint)) then
          ...
          do icomp1_stress=1,ncomp
             do icomp2_stress=1,ncomp
                ...
                if(myrank.eq.6.and.ipoint.eq.1.and.ista.eq.1.and.icomp1_stress.eq.1.and.icomp2_stress.eq.1) then
!                   print *,'stress',stress_sphe(icomp1_stress,icomp2_stress,icomp_force),icomp_force,ifreq
!                  print *,'stress',stress_sphe(icomp1_stress,icomp2_stress,icomp_force),icomp1_stress,icomp2_stress,&
!                      icomp_force
                end if
                !stress_sphe(icomp1,icomp2,icomp_force)= cmplx(0.d0)
                !if(idGreen<=ntheta) stress_sphe(icomp1,icomp2,icomp_force)=cmplx(0.d0)
             end do !icomp2_stress
          end do !icomp1_stress
     ...
     do icomp_force=1,ncomp
       do icomp1=1,ncomp
          ...
          !term1=cmplx(0.d0,0.d0)
          !term2=cmplx(0.d0,0.d0)
!Bottom
!          if(normal_vector(3,ipoint)>0.75) then
!Top
!          if(normal_vector(3,ipoint)<-0.75) then
!edge1Left
!          if(normal_vector(1,ipoint)<-0.85) then
!edge1Right
!          if(normal_vector(1,ipoint)>0.80) then
!edge2Left
!          if(normal_vector(2,ipoint)<-0.65) then
!edge2Right
!          if(normal_vector(2,ipoint)>0.65) then

!660km Top
!          if(normal_vector(1,ipoint)<-0.95) then
!660km Bot
!          if(normal_vector(1,ipoint)>0.95) then
...
!     if(ipoint.eq.1.and.ifreq.eq.100) print *,'final_disp',disp(1,ifreq,1),ifreq,myrank
...
!    if(myrank.eq.6.and.depth_this_point>813.0) print *,'depth_zone',depth_this_point,&
!       idep_end_this_zone,idep_start_this_zone
    
!    if(ndepths_this_zone.lt.nfit_dep) print *,'nzone_DSM',ndepths_this_zone,idep_start_this_zone,&
!        nfit_dep,zone_this_point,zones_DSM(1),ndepths_DSM
...
subroutine calcu_traction_SrcToBox(traction_SrcToBox_carte,rot_src, &
               normal_vector,stress_sphe_SrcToBox,ncomp)
   implicit none

!input/output
   integer ::ncomp
   complex(kind=8),dimension(ncomp),intent(out) ::traction_SrcToBox_carte
   double precision,dimension(ncomp,ncomp),intent(in) ::rot_src
   complex(kind=8),dimension(ncomp,ncomp),intent(in) ::stress_sphe_SrcToBox
   double precision, dimension(ncomp),intent(in) ::normal_vector
!other variables
   integer ::i,j,n
   complex(kind=8),dimension(ncomp,ncomp) ::temp_sphe,temp_carte
   complex(kind=8),dimension(ncomp,ncomp) ::stress_cartesian
   complex(kind=8),dimension(ncomp) ::traction_temp


   temp_sphe(:,:)=stress_sphe_SrcToBox(:,:)
   temp_carte=matmul(transpose(rot_src),matmul(temp_sphe,rot_src))
   stress_cartesian(:,:)=temp_carte(:,:)
   traction_temp(:)=cmplx(0.d0)
   do i=1,ncomp
      do j=1,ncomp
         traction_temp(i)=traction_temp(i)+normal_vector(j)*&
                            stress_cartesian(i,j)
      end do
   end do

   traction_SrcToBox_carte=traction_temp
