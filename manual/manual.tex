...
\code{configure}. The integrated topography converter inherits this configured
toolchain. Edit compiler variables directly only for components that do not use
\code{configure}.
...
The default SPECFEM3D target builds \code{xdecompose_mesh},
\code{xcubedsphere_topo}, \code{xmeshfem3D},
\code{xgenerate_databases}, and \code{xspecfem3D}. An MPI-enabled
configuration also builds \code{xdecompose_mesh_mpi}. Step~0 later installs
the selected case's model sources and rebuilds the canonical
\code{xgenerate_databases}. The converter is therefore available to
topographic cases without a separate demo-specific compilation step.

The converter is built locally with the compiler selected by
\code{configure}; do not reuse a precompiled converter from a different Linux
system. Its program entry point is
\code{src/SPECFEM3D/src/cubedsphere_topo/program_cubedsphere_topo.f90}; the
implementation is split across
\code{src/SPECFEM3D/src/cubedsphere_topo/cubedsphere_topo.f90} and companion
Fortran sources.
There is intentionally no single source file named
\code{xcubedsphere_topo.f90}. The Makefile in that source directory is a
compatibility wrapper around the top-level SPECFEM3D build.

Ordinary SPECFEM3D \code{make clean} removes the converter with the other
default targets. To force only the converter to be rebuilt after changing
compilers or system libraries, run from \code{src/SPECFEM3D}:
\begin{lstlisting}
make CLEAN=cubedsphere_topo clean
...
wavefield files. \\
\code{xcubedsphere_topo} & Converts geographic interface topography to the
cubed-sphere grid used by topographic SEM meshes. \\
...
test -x src/SPECFEM3D/bin/xcubedsphere_topo
...
For interface-topography cases, the applicable \code{DSM3D} path in Step~1
invokes \code{xcubedsphere_topo}; flat or \code{DSM1D} modes skip that
conversion. In cases that generate injected waves locally, Step~1 only records
...
with \code{xcubedsphere_topo}. Build it from the repository root with
\code{make -C src/SPECFEM3D xcubedsphere_topo}.
...
\code{xcubedsphere_topo} is missing or reports a GLIBC version error &
Configure SPECFEM3D for the local toolchain, then run
\code{make -C src/SPECFEM3D CLEAN=cubedsphere_topo clean} followed by
\code{make -C src/SPECFEM3D xcubedsphere_topo}. \\
