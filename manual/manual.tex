...
The main user-facing workflow is controlled by a case directory containing
\code{DATA/Par_file_SEM_DSM}, \code{DATA/CMTSOLUTION},
\code{DATA/dsm_model_base}, \code{DATA/tele_station.txt}, auxiliary Python
scripts, and shell scripts named \code{step*.sh}.
...
The scripts write generated products under \code{WORK/}. Large completed
...
for the example submission scripts. The default Makefiles use commands such as
\code{gfortran}, \code{gcc}, \code{mpif77}, \code{mpif90}, and \code{mpicc}.
If your cluster uses Intel, NVHPC, or module-provided compiler wrappers, edit
the compiler variables in the relevant Makefiles before compiling.
...
Some examples use model-specific database generators, for example
\code{xgenerate_databases_DSM1D}, \code{xgenerate_databases_ICB_Topo},
\code{xgenerate_databases_PKIKP3D}, \code{xgenerate_databases_ULVZ}, or
\code{xgenerate_databases_CMBtopo_CMBhetero}. Compile the generator needed by
the selected example and make sure it is executable in \code{src/SPECFEM3D/bin}.
...
After compilation, verify that the scripts can find the executables:
\begin{lstlisting}
...
\begin{itemize}
  ...
  numerical mismatch.
  \item \code{DSM3D}: apply the case-specific 3-D model implementation in the
  modified \specfem{} database generator.
\end{itemize}

For a new 3-D model, edit the model implementation used by the selected
database generator, rebuild \specfem{}, and rerun the database generation
steps.
...
\code{DSM3D}. \code{step3} writes and submits the Slurm jobs for
\code{xmeshfem3D} and \code{xgenerate_databases*}.
...
rotation experiment, switch to \code{DSM3D}, recompile the relevant
\specfem{} database generator after modifying the model implementation, and
rerun the database, solver, coupling, and assembly steps.
...
with \code{xcubedsphere_topo}. If the converter is missing, build or copy it
into \code{src/SPECFEM3D/bin}.
