...
# Reference runtime from a completed 256-task source-to-box DSM run.
# Queue wait time is not included; actual runtime can vary with filesystem and cluster load.
#   dsm_src_to_box: ntasks=256, runs=1, runtime=00:50:33.
...
if grep -Eq '^[[:space:]]*REUSE_DSM_DATABASE[[:space:]]*=[[:space:]]*\.([Tt][Rr][Uu][Ee])' "${PARAM_FILE}"; then
    echo "Step 4 skipped: reusing the configured shared DSM source-to-box database."
    exit 0
fi

# Keep the historical WORK-tree entry point available for validation and
# for cases configured to share this source-to-box database.
cat > "${WORK_DIR}/run.sh" <<'RUNNER_EOF'
#!/usr/bin/env bash
set -euo pipefail
CASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
exec "${CASE_DIR}/step4_dsm_src_to_box.sh" "$@"
RUNNER_EOF
...
echo "Configured NPROC=${NPROC}; the per-job task count will be capped by NFREQUENCY/4."
...
case "${SOURCE_MODE}" in
  ...
  *)
    ...
    echo "Supported values: -1 explosion, 0 moment-tensor Green functions, 1 North, 2 East, 3 Vertical."
    exit 1
    ;;
esac
