...
#   injected_waves: ntasks=128, runs=1, runtime=00:08:13.
...
if grep -Eq '^[[:space:]]*REUSE_INJECTED_WAVES[[:space:]]*=[[:space:]]*\.([Tt][Rr][Uu][Ee])' "${PARAM_FILE}"; then
    echo "Step 7 skipped: reusing the configured shared InjectedWaves output."
    exit 0
fi

# Preserve the historical WORK-tree entry point while keeping the
# parameter-driven implementation in the case root.
cat > "${WORK_DIR}/run.sh" <<'RUNNER_EOF'
#!/usr/bin/env bash
set -euo pipefail
CASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
exec "${CASE_DIR}/step7_extract_injected_waves.sh" "$@"
RUNNER_EOF
...
# Mantle-only boxes can have no acoustic depths. Create the matching empty
# distance table once here instead of letting every MPI rank race to create it.
ACOUSTIC_DEPTH_COUNT=$(depth_count_from_table "${SEM_DATABASE_DIR}/depth_table_acoustic")
if (( ACOUSTIC_DEPTH_COUNT == 0 )) && [[ ! -e "${SEM_DATABASE_DIR}/dist_table_acoustic" ]]; then
    : > "${SEM_DATABASE_DIR}/dist_table_acoustic"
fi
...
validate_dsm_solid_depth_order() {
  local table=$1
  if [[ ! -f "${table}" ]]; then
    echo "Error: DSM solid-depth table not found: ${table}" >&2
    return 1
  fi
  awk '
    NR == 2 {
    ...
    NR > 2 {
      ...
      zone_count[zone]++
      if (count > 1) {
        if (depth < previous_depth - 1.0e-4) {
          printf("Error: DSM depths are not shallow-to-deep at rows %d-%d: %.6f/z%d -> %.6f/z%d\n", NR-1, NR, previous_depth, previous_zone, depth, zone) > "/dev/stderr"
          bad = 1
        }
        if (zone != previous_zone) {
          if (depth - previous_depth > 1.0e-4 || previous_depth - depth > 1.0e-4) {
            printf("Error: DSM zone transition is not a duplicated discontinuity at rows %d-%d: %.6f/z%d -> %.6f/z%d\n", NR-1, NR, previous_depth, previous_zone, depth, zone) > "/dev/stderr"
            bad = 1
          }
          closed[previous_zone] = 1
          if (zone in closed) {
            printf("Error: DSM zone %d appears in multiple disjoint blocks in %s\n", zone, FILENAME) > "/dev/stderr"
            bad = 1
          }
        }
      }
    ...
    END {
      if (count != expected) {
        printf("Error: DSM depth count mismatch in %s: header=%d rows=%d\n", FILENAME, expected, count) > "/dev/stderr"
        bad = 1
      }
      for (zone in zone_count) {
        if (zone_count[zone] < 4) {
          printf("Error: DSM zone %d has only %d solid depths; at least 4 are required\n", zone, zone_count[zone]) > "/dev/stderr"
          bad = 1
        }
      }
      exit bad
...
for this_source in "${SOURCES[@]}"; do
    if [[ ! -d "${DSM_SRC_TO_BOX_DIR}/${this_source}" ]]; then
        ...
        echo "Run step4_dsm_src_to_box.sh for this source type and wait for the DSM job to finish first."
        exit 1
    fi
    if ! validate_dsm_solid_depth_order "${DSM_SRC_TO_BOX_DIR}/${this_source}/DATA/depth_solid_list"; then
        echo "Rerun step4_dsm_src_to_box.sh after correcting the DSM depth ordering." >&2
        exit 1
    fi
done
