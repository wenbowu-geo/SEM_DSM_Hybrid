...
cp "${BIN_DIR}/../src/generate_databases/model_tomography.f90" "${WORK_DIR}/model/model_tomography.f90"
cp "${BIN_DIR}/../src/generate_databases/get_model.F90" "${WORK_DIR}/model/get_model.F90"
...
if [[ ! -f "${DSM_MODEL_BASE}" && -f "${ROOT_DIR}/DATA/dsm_model_input" ]]; then
  DSM_MODEL_BASE="${ROOT_DIR}/DATA/dsm_model_input"
fi
...
ICB_RADIUS=$(find_solid_fluid_interface "${DSM_MODEL}" "${BOTTOM_RADIUS_KM}" "${TOP_RADIUS_KM}" "${R_EARTH}" || true)
if [[ -z "${ICB_RADIUS}" ]]; then
  # Slab boxes contain solid material only; there is no ICB/interface layer.
  ICB_RADIUS=$(awk -v b="${BOTTOM_RADIUS_KM}" -v t="${TOP_RADIUS_KM}" 'BEGIN { printf "%.6f", (b+t)/2.0 }')
  LOWER_LAYER_SPEED_KM_S=$(min_wave_speed_in_box "${DSM_MODEL}" "${BOTTOM_RADIUS_KM}" "${TOP_RADIUS_KM}" "${R_EARTH}")
  UPPER_LAYER_SPEED_KM_S="${LOWER_LAYER_SPEED_KM_S}"
  LOWER_MATERIAL_ID=2
  UPPER_MATERIAL_ID=2
else
...
if [[ "${DSM1D_OR_3D}" == "DSM3D" ]]; then
  # Slab cases use the model_tomography.f90 implementation directly; no ICB
  # topography or stochastic heterogeneity tiles are required.
  if [[ -f "${ICB_TOPO_SOURCE}" ]]; then
...
# number of interfaces
 2
# Slab_PP solid interfaces: 410-km boundary and SEM-box top.
...
constant_topography_grid "${Z_ICB}" 300 300 "${MESHFEM_DIR}/topo_410km.dat"
