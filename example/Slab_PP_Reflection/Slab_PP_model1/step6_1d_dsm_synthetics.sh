...
# Reference runtimes from a completed EquatorialPath Step 6 workflow.
# Queue wait time is not included; actual runtime can vary with filesystem and cluster load.
#   dsm_1d_freq2sac: ntasks=1, runs=1, runtime=00:00:04.
#   dsm_1d_green: ntasks=512, runs=1, runtime=00:13:51.
#   dsm_1d_synthesis: ntasks=1, runs=1, runtime=00:00:00.
# End reference runtimes.
...
if grep -Eq '^[[:space:]]*REUSE_DSM_DATABASE[[:space:]]*=[[:space:]]*\.([Tt][Rr][Uu][Ee])' "${PARAM_FILE}"; then
    echo "Step 6 skipped: reusing the configured shared 1-D DSM database."
    exit 0
fi
...
echo "Configured 1-D DSM rank limit: ${NPROC}."
...
# Preserve the historical WORK-tree entry point while the implementation lives
# in the case-root step script.
cat > "${WORK_DIR}/run.sh" <<'RUNNER_EOF'
#!/usr/bin/env bash
set -euo pipefail
CASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
exec "${CASE_DIR}/step6_1d_dsm_synthetics.sh" "$@"
RUNNER_EOF
