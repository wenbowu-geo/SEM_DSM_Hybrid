...
SAC_FLOAT_COUNT = 70
SAC_INT_COUNT = 40
SAC_CHAR_OFFSET = 4 * (SAC_FLOAT_COUNT + SAC_INT_COUNT)
...
def set_sac_string(header: bytearray, index: int, value: str) -> None:
    start = SAC_CHAR_OFFSET + 8 * index
    encoded = value.upper().encode("ascii", errors="replace")[:8].ljust(8, b" ")
    header[start : start + 8] = encoded


def angular_distance_deg(src_lat: float, src_lon: float, sta_lat: float, sta_lon: float) -> float:
    lat1 = math.radians(src_lat)
    lat2 = math.radians(sta_lat)
    dlon = math.radians(sta_lon - src_lon)
    cosine = math.sin(lat1) * math.sin(lat2) + math.cos(lat1) * math.cos(lat2) * math.cos(dlon)
    return math.degrees(math.acos(max(-1.0, min(1.0, cosine))))


def patch_sac_metadata(
    header: bytearray,
    cmt: dict[str, float],
    station: dict[str, float | str],
    component: str,
) -> None:
    evla = cmt["latitude"]
    evlo = cmt["longitude"]
    stla = float(station["lat"])
    stlo = float(station["lon"])
    az = azimuth_deg(evla, evlo, stla, stlo)
    baz = azimuth_deg(stla, stlo, evla, evlo)
    gcarc = angular_distance_deg(evla, evlo, stla, stlo)
    radial_az = (baz + 180.0) % 360.0
    component = component.lower().lstrip(".")
    orientations = {
        "bhz": (0.0, 0.0),
        "bhn": (0.0, 90.0),
        "bhe": (90.0, 90.0),
        "bhr": (radial_az, 90.0),
        "bht": ((radial_az + 90.0) % 360.0, 90.0),
    }
    if component not in orientations:
        raise RuntimeError(f"unsupported SAC component: {component}")

    set_sac_float(header, 31, stla)
    set_sac_float(header, 32, stlo)
    set_sac_float(header, 33, float(station["elev"]))
    set_sac_float(header, 35, evla)
    set_sac_float(header, 36, evlo)
    set_sac_float(header, 38, cmt["depth"])
    set_sac_float(header, 50, math.radians(gcarc) * 6371.0)
    set_sac_float(header, 51, az)
    set_sac_float(header, 52, baz)
    set_sac_float(header, 53, gcarc)
    set_sac_float(header, 57, orientations[component][0])
    set_sac_float(header, 58, orientations[component][1])
    set_sac_int(header, 38, 1)
    set_sac_string(header, 0, str(station["sta"]))
    set_sac_string(header, 20, component)
    set_sac_string(header, 21, str(station["net"]))
...
def main() -> None:
    ...
    parser.add_argument("--injected-par", default="WORK/InjectedWaves/OUTPUT_FILES/InjectedWaves_Par_file")
    ...
    metadata = read_injected_metadata(metadata_path)
    scatter_time_shift = metadata["TIME_START_CUT"]
    scatter_shift_source = f"TIME_START_CUT from {metadata_path}"
    injected_par = (root / args.injected_par).resolve()
    if injected_par.is_file():
        injected_values = read_key_values(injected_par, "=")
        if "t_begin_Green" in injected_values:
            scatter_time_shift = float(injected_values["t_begin_Green"])
            scatter_shift_source = f"t_begin_Green from {injected_par}"
    ...
    for station in stations:
        ...
        for ext, header, trace in (
            ("bhz", header_z, trace_z),
            ("bhr", header_r, trace_r),
            ("bht", header_t, trace_t),
        ):
            output_header = header.copy()
            patch_sac_metadata(output_header, cmt, station, ext)
            write_sac(out_1d / f"{name}.{ext}", output_header, trace)
        delta_scatter = None
        ...
        baz = azimuth_deg(float(station["lat"]), float(station["lon"]), cmt["latitude"], cmt["longitude"])
        az = math.radians((baz + 180.0) % 360.0)
