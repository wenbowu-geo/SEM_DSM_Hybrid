...
!**********add reflector
subroutine add_large_scatter(myrank,x_eval,y_eval,z_eval,xeval_cubedsph,yeval_cubedsph, &
...
DOUBLE PRECISION :: depth_eval, radius0
DOUBLE PRECISION :: dx, dy, dz
DOUBLE PRECISION :: strike_rad, dip_rad
DOUBLE PRECISION :: sx, sy
DOUBLE PRECISION :: dhx, dhy
DOUBLE PRECISION :: nx, ny, nz
DOUBLE PRECISION :: coord_strike, coord_dip, coord_normal
DOUBLE PRECISION :: half_width_strike, half_width_dip, half_thick
DOUBLE PRECISION :: taper_x, taper_y, taper_z, taper_total
DOUBLE PRECISION :: taper_len_x, taper_len_y, taper_len_z

! Model parameters
DOUBLE PRECISION, PARAMETER :: large_scatter_thickness = 7000.0d0   ! half thickness, meter
DOUBLE PRECISION, PARAMETER :: depth_scatter_center = 360000.0d0    ! meter

! Widths in degrees, converted below to meters
DOUBLE PRECISION, PARAMETER :: large_scatter_width_x = 3.8d0
DOUBLE PRECISION, PARAMETER :: large_scatter_width_y = 3.8d0

! Center in cubed-sphere angular coordinates, degrees
DOUBLE PRECISION, PARAMETER :: large_scatter_x0 = 0.d0
DOUBLE PRECISION, PARAMETER :: large_scatter_y0 = 0.d0

! Reflector orientation
! strike: clockwise from +y direction, following usual geographic convention
! dip:    positive downward, in degrees
! Dip direction follows the right-hand rule:
! the reflector dips toward strike + 90 degrees (clockwise; just project the downward dip
! vector to the free surface).

!DOUBLE PRECISION, PARAMETER :: strike_deg = 260.46
!DOUBLE PRECISION, PARAMETER :: dip_deg    = 38.19
DOUBLE PRECISION, PARAMETER :: strike_deg = 260.77
DOUBLE PRECISION, PARAMETER :: dip_deg    = 34.96

DOUBLE PRECISION, PARAMETER :: degtorad = PI/180.0d0

DOUBLE PRECISION, PARAMETER :: dvp_vp   = 0.05d0
DOUBLE PRECISION, PARAMETER :: dvs_vs   = 0.020d0
DOUBLE PRECISION, PARAMETER :: drho_rho =  0.020d0

radius = DSQRT(x_eval**2 + y_eval**2 + z_eval**2)

if (vs_final .le. TINYVAL) return

! Depth positive downward
depth_eval = 6371000.0d0 - radius
radius0 = 6371000.0d0 - depth_scatter_center

! Convert cubed-sphere angular offsets to local horizontal distances
! dx: local +x direction
! dy: local +y direction
dx = radius0 * (xeval_cubedsph - large_scatter_x0*degtorad)
dy = radius0 * (yeval_cubedsph - large_scatter_y0*degtorad)
dz = depth_eval - depth_scatter_center

! Convert widths from degrees to meters
half_width_strike = 0.5d0 * large_scatter_width_x * degtorad * radius0
half_width_dip    = 0.5d0 * large_scatter_width_y * degtorad * radius0
half_thick        = large_scatter_thickness

strike_rad = strike_deg * degtorad
dip_rad    = dip_deg    * degtorad

! Along-strike unit vector in local horizontal coordinates
sx = DSIN(strike_rad)
sy = DCOS(strike_rad)

! Horizontal down-dip direction: strike + 90 degrees
dhx = DCOS(strike_rad)
dhy = -DSIN(strike_rad)

! Unit normal to dipping reflector plane
! coord_normal = 0 defines the reflector mid-plane
nx = -DSIN(dip_rad) * dhx
ny = -DSIN(dip_rad) * dhy
nz =  DCOS(dip_rad)

coord_strike = dx*sx + dy*sy
coord_dip    = dx*DCOS(dip_rad)*dhx + dy*DCOS(dip_rad)*dhy + dz*DSIN(dip_rad)
coord_normal = dx*nx + dy*ny + dz*nz

if (DABS(coord_strike) .lt. half_width_strike .and. &
    DABS(coord_dip)    .lt. half_width_dip    .and. &
    DABS(coord_normal) .lt. half_thick) then

  ! Taper length near side boundaries
  taper_len_x = 0.20d0 * half_width_strike
  taper_len_y = 0.20d0 * half_width_dip
  taper_len_z = 0.50d0 * half_thick

  taper_x = 1.0d0
  taper_y = 1.0d0
  taper_z = 1.0d0

  if (half_width_strike - DABS(coord_strike) .lt. taper_len_x) then
    taper_x = 0.5d0 * (1.0d0 - DCOS(PI * (half_width_strike - DABS(coord_strike)) / taper_len_x))
  endif

  if (half_width_dip - DABS(coord_dip) .lt. taper_len_y) then
    taper_y = 0.5d0 * (1.0d0 - DCOS(PI * (half_width_dip - DABS(coord_dip)) / taper_len_y))
  endif

  if (half_thick - DABS(coord_normal) .lt. taper_len_z) then
    taper_z = 0.5d0 * (1.0d0 - DCOS(PI * (half_thick - DABS(coord_normal)) / taper_len_z))
  endif

  taper_total = taper_x * taper_y * taper_z

  vp_final  = vp_final  * (1.0d0 + taper_total*dvp_vp)
  vs_final  = vs_final  * (1.0d0 + taper_total*dvs_vs)
  rho_final = rho_final * (1.0d0 + taper_total*drho_rho)

endif

END SUBROUTINE add_large_scatter
...
! invalid number
  ...
  logical, parameter ::large_scatter_incorporated=.true.
...
!********************************************************************
  ...
  if(large_scatter_incorporated) then
   call add_large_scatter(myrank,x_eval,y_eval,z_eval,xeval_cubedsph,yeval_cubedsph, &
                 vp_final,vs_final,rho_final)
  end if
