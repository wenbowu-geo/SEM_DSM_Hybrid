...
#   coupling: ntasks=256, runs=4, range=00:07:20-00:21:39, avg=00:12:03.
...
NPROC_MAX=$(param COUPLING_NPROC_MAX 512)
TARGET_PACKAGES_PER_RANK=$(param COUPLING_TARGET_PACKAGES_PER_RANK 10)
...
if [[ -f "${COUPLING_PAR}" ]]; then
  ...
  if [[ "${NPROC}" =~ ^[Aa][Uu][Tt][Oo]$ ]]; then
    if [[ -z "${NPACKAGE_SEM}" || "${NPACKAGE_SEM}" -le 0 ]]; then
      echo "Error: COUPLING_NPROC=AUTO requires npackage_SEM in ${COUPLING_PAR}." >&2
      exit 1
    fi
    if ! is_positive_int "${NPROC_MAX}"; then
      echo "Error: COUPLING_NPROC_MAX must be a positive integer, got '${NPROC_MAX}'." >&2
      exit 1
    fi
    if ! is_positive_int "${TARGET_PACKAGES_PER_RANK}"; then
      echo "Error: COUPLING_TARGET_PACKAGES_PER_RANK must be a positive integer, got '${TARGET_PACKAGES_PER_RANK}'." >&2
      exit 1
    fi
    NPROC=$(( (NPACKAGE_SEM + TARGET_PACKAGES_PER_RANK - 1) / TARGET_PACKAGES_PER_RANK ))
    if [[ "${NPROC}" -gt "${NPROC_MAX}" ]]; then
      NPROC=${NPROC_MAX}
    fi
    if [[ "${NPROC}" -gt "${NPACKAGE_SEM}" ]]; then
      NPROC=${NPACKAGE_SEM}
    fi
    echo "COUPLING_NPROC=AUTO selected NPROC=${NPROC} from npackage_SEM=${NPACKAGE_SEM}, target packages/rank=${TARGET_PACKAGES_PER_RANK}, max=${NPROC_MAX}."
  elif ! is_positive_int "${NPROC}"; then
    echo "Error: COUPLING_NPROC must be AUTO or a positive integer, got '${NPROC}'." >&2
    exit 1
  elif [[ -n "${NPACKAGE_SEM}" && "${NPACKAGE_SEM}" -gt 0 && "${NPROC}" -gt "${NPACKAGE_SEM}" ]]; then
    echo "Warning: COUPLING_NPROC=${NPROC} is larger than npackage_SEM=${NPACKAGE_SEM}; using ${NPACKAGE_SEM}."
    NPROC=${NPACKAGE_SEM}
  fi
elif [[ "${NPROC}" =~ ^[Aa][Uu][Tt][Oo]$ ]]; then
  echo "Error: COUPLING_NPROC=AUTO requires ${COUPLING_PAR}." >&2
  exit 1
elif ! is_positive_int "${NPROC}"; then
  echo "Error: COUPLING_NPROC must be AUTO or a positive integer, got '${NPROC}'." >&2
  exit 1
