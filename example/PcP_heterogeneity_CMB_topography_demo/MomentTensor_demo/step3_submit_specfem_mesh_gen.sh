...
#   generate_databases: ntasks=512, runs=3, range=00:00:11-00:01:11, avg=00:00:34.
#   mesh: ntasks=512, runs=3, range=00:00:04-00:41:47, avg=00:13:59.
...
# Read processor grid from Mesh_Par_file; xmeshfem3D requires ntasks=NPROC_XI*NPROC_ETA.
...
NPROC_XI=$(read_mesh_key NPROC_XI) || { echo "Could not read NPROC_XI from ${MESH_PAR_FILE}" >&2; exit 1; }
NPROC_ETA=$(read_mesh_key NPROC_ETA) || { echo "Could not read NPROC_ETA from ${MESH_PAR_FILE}" >&2; exit 1; }
...
NPROC_PAR=$(awk -F= '
  ...
  END { if (!found) exit 1 }
' "${PAR_FILE}" 2>/dev/null || true)
if [[ "${NPROC_PAR}" != "${NPROC}" ]]; then
  echo "Warning: ${PAR_FILE} has NPROC=${NPROC_PAR:-unset}, but Mesh_Par_file requires ${NPROC}. Updating Par_file."
  awk -v value="${NPROC}" '
    BEGIN { done=0 }
    $0 ~ /^[[:space:]]*NPROC[[:space:]]*=/ { printf "%-32s= %s\n", "NPROC", value; done=1; next }
    { print }
    END { if (!done) printf "%-32s= %s\n", "NPROC", value }
  ' "${PAR_FILE}" > "${PAR_FILE}.tmp"
  mv "${PAR_FILE}.tmp" "${PAR_FILE}"
fi
...
if [[ -z "${DATABASE_BIN}" ]]; then
  ...
  elif [[ -x "${BIN_DIR}/xgenerate_databases_CMBtopo_CMBhetero" ]]; then
    DATABASE_BIN="${BIN_DIR}/xgenerate_databases_CMBtopo_CMBhetero"
  else
