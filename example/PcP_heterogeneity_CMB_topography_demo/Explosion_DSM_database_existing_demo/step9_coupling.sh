...
#   coupling: ntasks=256, runs=1, runtime=00:13:52.
...
NPROC_MAX=$(param COUPLING_NPROC_MAX 256)
...
if [[ -f "${COUPLING_PAR}" ]]; then
  NPACKAGE_SEM=$(awk -F= '$1 ~ /^[[:space:]]*npackage_SEM[[:space:]]*$/ { gsub(/^[[:space:]]+|[[:space:]]+$/, "", $2); print $2; exit }' "${COUPLING_PAR}")
fi

if [[ "${NPROC}" =~ ^[Aa][Uu][Tt][Oo]$ ]]; then
  if ! is_positive_integer "${NPACKAGE_SEM}"; then
    echo "Error: COUPLING_NPROC=AUTO requires a positive npackage_SEM in ${COUPLING_PAR}." >&2
    exit 1
  fi
  if ! is_positive_integer "${NPROC_MAX}"; then
    echo "Error: COUPLING_NPROC_MAX must be a positive integer; got '${NPROC_MAX}'." >&2
    exit 1
  fi
  if ! is_positive_integer "${TARGET_PACKAGES_PER_RANK}"; then
...
elif ! is_positive_integer "${NPROC}"; then
  echo "Error: COUPLING_NPROC must be a positive integer or AUTO; got '${NPROC}'." >&2
  exit 1
elif is_positive_integer "${NPACKAGE_SEM}" && [[ "${NPROC}" -gt "${NPACKAGE_SEM}" ]]; then
