...
def read_key_values(path: Path, separator: str) -> dict[str, str]:
    ...
    for raw in path.read_text().splitlines():
        ...
        value = value.strip()
        values[key.strip()] = value.split()[0] if value else ""
    return values
...
def main() -> None:
    ...
    parser.add_argument("--one-d-dir")
    parser.add_argument("--one-d-dsm-model")
    ...
    root = Path.cwd()
    params_path = root / "DATA" / "Par_file_SEM_DSM"
    params = read_key_values(params_path, "=") if params_path.is_file() else {}
    dsm_database_dir = params.get("DSM_DATABASE_DIR", "")

    if args.one_d_dir:
        one_d_dir = resolve_case_path(root, args.one_d_dir)
    elif dsm_database_dir:
        one_d_dir = resolve_case_path(
            root,
            f"{dsm_database_dir}/1D_tele_syn/explosion/OUTPUT_FILES/disp_solid_time_sac",
        )
    else:
        one_d_dir = resolve_case_path(root, "WORK/DSM/1D_tele_syn/explosion/OUTPUT_FILES/disp_solid_time_sac")

    if args.one_d_dsm_model:
        one_d_dsm_model = resolve_case_path(root, args.one_d_dsm_model)
    elif dsm_database_dir:
        one_d_dsm_model = resolve_case_path(root, f"{dsm_database_dir}/1D_tele_syn/explosion/DATA/dsm_model")
    else:
        one_d_dsm_model = resolve_case_path(root, "WORK/DSM/1D_tele_syn/explosion/DATA/dsm_model")

    if args.metadata:
        metadata_path = resolve_case_path(root, args.metadata)
    elif params.get("INJECTED_WAVES_METADATA", ""):
        metadata_path = resolve_case_path(root, params["INJECTED_WAVES_METADATA"])
    else:
        metadata_path = resolve_case_path(root, "WORK/InjectedWaves/DATA/injected_waves_metadata.env")

    coupling_dir = resolve_case_path(root, args.coupling_dir)
    output_root = resolve_case_path(root, args.output_root)
