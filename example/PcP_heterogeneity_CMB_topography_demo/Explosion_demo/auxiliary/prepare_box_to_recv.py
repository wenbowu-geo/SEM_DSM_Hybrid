#!/usr/bin/env python3
"""Prepare DSM box-to-receiver (reciprocity) inputs for the PKP precursor ULVZ demo."""
...
def write_depth_file(path: Path, entries: list[tuple[float, int]], ddepth: float, tolerance: float) -> None:
    entries = sorted(entries, key=lambda item: (round(item[0], 8), -item[1]))
...
def main() -> None:
    ...
    phase_arrival_time_length = math.ceil(arrival_time + 500.0)
    arrival_time_metadata = f"{arrival_time:.3f}"
    arrival_time_label = f"{arrival_time:.3f} s"

    if time_length_override is not None:
        if time_length_override <= phase_arrival_time_length:
            die(
                "TIME_LENGTH_DSM_BOX_TO_RECEIVER must be longer than the phase-arrival-based "
                f"length ({phase_arrival_time_length:.1f} s); got {time_length_override:.1f} s"
            )
        ...
        time_length = phase_arrival_time_length
    ...
    (DATA_DIR / f"box_to_recv_metadata_{force_comp}.env").write_text(
        "\n".join(
            [
                ...
                f"PHASE_ARRIVAL_BASED_TIME_LENGTH={phase_arrival_time_length:.1f}",
