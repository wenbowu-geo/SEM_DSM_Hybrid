...
!WENBO_S
  ...
  case (IMODEL_DSM3D)
     ...
     call model_DSM1D(imaterial_id,xmesh,ymesh,zmesh,r_middle,&
                                  rho,vp,vs,qkappa_atten,qmu_atten,myrank)
     !Add perturbations using the 3-D tomography model
     !Peruterbations only in solid media (the ocean and outer core has no
     !heterogeneities) and imaterial_id=2 for solid media
     if (imaterial_id.eq.2) call model_DSM3D(xmesh,ymesh,zmesh,xmesh_cubedsph,&
                                  ymesh_cubedsph,zmesh_cubedsph, &
                                  rho,vp,vs,qkappa_atten,qmu_atten,myrank)
     has_tomo_value=.true.
