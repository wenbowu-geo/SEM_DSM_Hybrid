#!/usr/bin/env bash
#
# Step 0: prepare ICB topography, install this case's SPECFEM3D model sources,
# and build the database generator.
...
# Generate the case-specific ICB topography after the model setup succeeds.
# The numerical generator remains in DATA/topo_files/generate_bump.py.
if [[ "${DSM_MODEL_MODE}" == "DSM3D" ]]; then
  ...
  TOPO_PYTHON="${PYTHON_BIN:-}"

  if [[ ! -f "${TOPO_GENERATOR}" ]]; then
    echo "Error: ICB topography generator is missing: ${TOPO_GENERATOR}" >&2
    exit 1
  fi
  if [[ -z "${TOPO_PYTHON}" ]]; then
    if [[ -x "${DEFAULT_PYTHON}" ]]; then
      TOPO_PYTHON="${DEFAULT_PYTHON}"
    else
      TOPO_PYTHON=python3
    fi
  fi
  if ! command -v "${TOPO_PYTHON}" >/dev/null 2>&1; then
    echo "Error: Python interpreter is not available: ${TOPO_PYTHON}" >&2
    exit 1
  fi

  echo "Generating ICB topography with:"
  echo "  ${TOPO_GENERATOR}"
  (cd "$(dirname "${TOPO_GENERATOR}")" && \
    "${TOPO_PYTHON}" "$(basename "${TOPO_GENERATOR}")")

  if [[ ! -s "${TOPO_FILE}" ]]; then
    echo "Error: topography generator did not create: ${TOPO_FILE}" >&2
    exit 1
  fi
