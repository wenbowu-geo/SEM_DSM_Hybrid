...
echo "computing the 1-D 410-km triplication example : `date`"
...
setup_and_run_job() {
    ...
    awk -v time_length="$time_length" -v source_depth_km="$source_depth_km" -v moment_or_force="$moment_or_force" -v force="$force_vector" '{
        if (NR == 1) {
            print time_length " 4096  #time_series_length n_freqnency"
        }else if (NR == 2) {
	    print "1.0d-3         #omega_imag";
...
setup_and_run_job "410km_triplication" "25 1.0 1.0 1.0 0.0 0.0 0.0" "DSM_410km_tele" ${NPROC} "23:59:00"
