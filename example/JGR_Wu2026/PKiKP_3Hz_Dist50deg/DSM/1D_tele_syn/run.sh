...
setup_and_run_job() {
    ...
    awk -v time_length="$time_length" -v source_depth_km="$source_depth_km" -v moment_or_force="$moment_or_force" -v force="$force_vector" '{
        if (NR == 1) {
            print time_length " 8192  #time_series_length n_freqnency"
        }else if (NR == 2) {
	    print "0.6d-3         #omega_imag";
...
setup_and_run_job "PKiKP" "25 1.0 1.0 1.0 0.0 0.0 0.0" "DSM_PKiKP" ${NPROC} "23:59:00"
