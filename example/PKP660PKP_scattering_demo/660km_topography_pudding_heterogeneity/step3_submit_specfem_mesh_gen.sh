...
#   generate_databases: ntasks=500, runs=1, runtime=00:03:32.
#   mesh: ntasks=500, runs=1, runtime=00:00:07.
...
# Read NPROC from Par_file and keep it consistent with Mesh_Par_file.
NPROC=$(param_file_key "${PAR_FILE}" NPROC) || { echo "Could not read NPROC from ${PAR_FILE}" >&2; exit 1; }
MESH_PAR_FILE="${DATA_DIR}/meshfem3D_files/Mesh_Par_file"
if [[ -f "${MESH_PAR_FILE}" ]]; then
  NPROC_XI=$(param_file_key "${MESH_PAR_FILE}" NPROC_XI) || { echo "Could not read NPROC_XI from ${MESH_PAR_FILE}" >&2; exit 1; }
  NPROC_ETA=$(param_file_key "${MESH_PAR_FILE}" NPROC_ETA) || { echo "Could not read NPROC_ETA from ${MESH_PAR_FILE}" >&2; exit 1; }
  MESH_NPROC=$((NPROC_XI * NPROC_ETA))
  if [[ "${NPROC}" -ne "${MESH_NPROC}" ]]; then
    echo "Error: ${PAR_FILE} has NPROC=${NPROC}, but ${MESH_PAR_FILE} requires NPROC_XI*NPROC_ETA=${MESH_NPROC} (${NPROC_XI}*${NPROC_ETA})." >&2
    echo "Run Step 1 again, or make NPROC consistent with NPROC_XI/NPROC_ETA before submitting Step 3." >&2
    exit 1
  fi
fi

DATABASE_BIN_OVERRIDE=$(param_sem_dsm SPECFEM3D_DATABASE_BIN "")
if [[ -n "${DATABASE_BIN_OVERRIDE}" ]]; then
  DATABASE_BIN="${DATABASE_BIN_OVERRIDE}"
elif [[ "${DSM1D_OR_3D}" == "DSM1D" ]]; then
  DATABASE_BIN="${BIN_DIR}/xgenerate_databases_DSM1D"
else
  DATABASE_BIN="${BIN_DIR}/xgenerate_databases_660km_hete_puding"
fi
