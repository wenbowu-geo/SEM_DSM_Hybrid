...
def box_to_receiver_distance_floor(params: dict[str, str], spacing: float) -> float:
    ...
    floor = 2.0 * spacing
    print(f"  Using box-to-receiver DSM minimum distance: {floor:.6f} degrees")
    return floor
...
def write_fluid_distance_file(path: Path, fluid_depth_count: int) -> None:
    if fluid_depth_count > 0:
        path.write_text((DATA_DIR / "dist_solid_list").read_text())
    else:
        path.write_text("0\n")
...
def main() -> None:
    ...
    write_fluid_distance_file(DATA_DIR / "dist_fluid_list", len(fluid))
