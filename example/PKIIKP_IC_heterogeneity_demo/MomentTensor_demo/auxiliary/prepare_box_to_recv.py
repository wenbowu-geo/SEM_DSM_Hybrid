#!/usr/bin/env python3
"""Prepare DSM box-to-receiver (reciprocity) inputs for the PKIIKP inner-core heterogeneity explosion demo."""
...
def box_to_receiver_distance_floor(params: dict[str, str], spacing: float) -> float:
    ...
    floor = 2.0 * spacing
    print(f"  Using DSM minimum distance: {floor:.6f} degrees")
    return floor
...
def write_distance_files(stations: list[dict], params: dict[str, str], zones: list[dict], earth_radius: float, freq: float) -> tuple[float, float, float, int, float, float, float, float, int]:
    ...
    min_distance = box_to_receiver_distance_floor(params, spacing)
