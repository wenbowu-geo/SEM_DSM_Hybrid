...
DSM_SRC_TO_BOX_DIR = ROOT_DIR / "WORK/DSM/src_to_box"
...
def dsm_source_to_box_distance_limits() -> tuple[float | None, float | None]:
    metadata_paths = [DSM_SRC_TO_BOX_DIR / "DATA/src_to_box_metadata.env"]
    metadata_paths.extend(sorted(DSM_SRC_TO_BOX_DIR.glob("*/DATA/src_to_box_metadata.env")))
    spacing: float | None = None
    for path in metadata_paths:
        if not path.is_file():
            continue
        value = optional_float(read_params(path), "DISTANCE_SPACING_DEG")
        if value is not None and value > 0.0:
            spacing = value
            break

    distance_paths = [
        DSM_SRC_TO_BOX_DIR / "DATA/dist_solid_list",
        DSM_SRC_TO_BOX_DIR / "DATA/dist_fluid_list",
    ]
    distance_paths.extend(sorted(DSM_SRC_TO_BOX_DIR.glob("*/DATA/dist_solid_list")))
    distance_paths.extend(sorted(DSM_SRC_TO_BOX_DIR.glob("*/DATA/dist_fluid_list")))
    for path in distance_paths:
        values = counted_distance_values(path)
        if len(values) < 2:
            continue
        unique_values = sorted(set(values))
        if len(unique_values) < 2:
            continue
        max_distance = unique_values[-1]
        last_spacing = unique_values[-1] - unique_values[-2]
        if spacing is None or spacing <= 0.0:
            spacing = last_spacing
        return max_distance, spacing
...
def sem_table_arrivals(
    ...
    source_depth: float,
    top_depth: float,
    max_dsm_distance: float | None,
    max_dsm_distance_tolerance: float,
    ...
    for domain in ("elastic", "acoustic"):
        ...
        if not distances or not radii:
            ...
            continue
        if max_dsm_distance is not None:
            offenders = [dist for dist in distances if dist > max_dsm_distance + max_dsm_distance_tolerance]
            if offenders:
                die(
                    f"SEM {domain} distance table reaches {max(offenders):.6f} deg, "
                    f"but the DSM distance limit used for checking is {max_dsm_distance:.6f} deg "
                    f"with one-sided tolerance {max_dsm_distance_tolerance:.6f} deg. "
                    "Move the SEM box/source geometry away from the exact antipode, reduce the SEM angular extent, or regenerate the DSM distance table with a larger safe maximum."
                )
        depths = radii
        for dist_label, distance in extrema(distances):
            for depth_label, receiver_depth in extrema(depths):
                ...
                try:
                    phase_name, arrival_time = arrival_time_for_distance_depth(
                        zones, earth_radius, source_depth, top_depth, receiver_depth, distance
                    )
                except SystemExit:
                    skipped.append(name)
                    continue
...
def main() -> None:
    ...
    for corner in box_corners(params):
        ...
        try:
            phase_name, arrival_time = arrival_time_for_distance_depth(
                zones, earth_radius, cmt["depth"], top_depth, receiver_depth, distance
            )
        except SystemExit:
            skipped.append(str(corner["name"]))
            continue
        arrivals.append(
            {
    ...
    if not arrivals:
        die(f"Could not compute source-to-box P/K/I arrivals for any SEM-box corner using {model_path}")

    max_dsm_distance = optional_float(params, "DSM_MAX_DISTANCE_DEGREES")
    if max_dsm_distance is not None:
        max_dsm_distance = min(180.0, max_dsm_distance)
    dsm_table_max_distance, dsm_distance_spacing = dsm_source_to_box_distance_limits()
    if dsm_table_max_distance is not None:
        max_dsm_distance = dsm_table_max_distance
    max_dsm_distance_tolerance = 2.0 * dsm_distance_spacing if dsm_distance_spacing is not None else 0.0
    table_arrivals, table_skipped = sem_table_arrivals(
        zones,
        earth_radius,
        cmt["depth"],
        top_depth,
        max_dsm_distance,
        max_dsm_distance_tolerance,
    )
    ...
    lines = [
        "CUT_WINDOW_SOURCE=DSM_MODEL_DIRECT_PKI_RAY",
        f"PHASE_SOURCE_TO_BOX={phase}",
        "TRAVEL_TIME_METHOD=DSM_MODEL_DIRECT_PKI_RAY",
