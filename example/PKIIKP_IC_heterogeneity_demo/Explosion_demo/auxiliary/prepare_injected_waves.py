        ...
        if max_dsm_distance is not None:
            ...
            if offenders:
                die(
                    ...
                    f"but DSM_MAX_DISTANCE_DEGREES is {max_dsm_distance:.6f} "
                    f"with tolerance {max_dsm_distance_tolerance:.6f}. "
                    "Move the SEM box/source geometry away from the exact antipode or use a smaller SEM angular extent so injected-wave distance interpolation remains two-sided."
                )
        depths = radii
...
def main() -> None:
    ...
    src_to_box_metadata = ROOT_DIR / "WORK/DSM/src_to_box/explosion/DATA/src_to_box_metadata.env"
    src_to_box_params = read_params(src_to_box_metadata) if src_to_box_metadata.is_file() else {}
    max_dsm_distance = optional_float(params, "DSM_MAX_DISTANCE_DEGREES")
    if max_dsm_distance is None:
        max_dsm_distance = optional_float(src_to_box_params, "DISTANCE_MAX_DEG")
    if max_dsm_distance is not None:
        max_dsm_distance = min(180.0, max_dsm_distance)
    dsm_distance_spacing = optional_float(src_to_box_params, "DISTANCE_SPACING_DEG")
    max_dsm_distance_tolerance = 4.0 * dsm_distance_spacing if dsm_distance_spacing is not None else 0.0
