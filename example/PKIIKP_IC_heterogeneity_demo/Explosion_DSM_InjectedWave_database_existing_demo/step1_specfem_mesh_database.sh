...
convert_interface_topography() {
  ...
  local margin
  margin=$(to_float "$(param ICB_TOPOGRAPHY_TRUNCATION_MARGIN_M 40000.0)")
...
ICB_RADIUS=$(find_solid_fluid_interface "${DSM_MODEL}" "${BOTTOM_RADIUS_KM}" "${TOP_RADIUS_KM}" "${R_EARTH}") || {
  echo "Could not find a solid/fluid interface inside the SEM box from ${DSM_MODEL}" >&2
  exit 1
}

LOWER_LAYER_SPEED_KM_S=$(min_wave_speed_in_box "${DSM_MODEL}" "${BOTTOM_RADIUS_KM}" "${ICB_RADIUS}" "${R_EARTH}")
UPPER_LAYER_SPEED_KM_S=$(min_wave_speed_in_box "${DSM_MODEL}" "${ICB_RADIUS}" "${TOP_RADIUS_KM}" "${R_EARTH}")
LOWER_MATERIAL_ID=$(layer_material_id "${DSM_MODEL}" "${BOTTOM_RADIUS_KM}" "${ICB_RADIUS}" "${R_EARTH}")
UPPER_MATERIAL_ID=$(layer_material_id "${DSM_MODEL}" "${ICB_RADIUS}" "${TOP_RADIUS_KM}" "${R_EARTH}")
...
NZ_LOWER_DEFAULT=$(nz_from_wavelength "${BOTTOM_RADIUS_KM}" "${ICB_RADIUS}" "${LOWER_LAYER_SPEED_KM_S}" "${FREQ_RESOLVED}" "${LOWER_EPW}")
NZ_UPPER_DEFAULT=$(nz_from_wavelength "${ICB_RADIUS}" "${TOP_RADIUS_KM}" "${UPPER_LAYER_SPEED_KM_S}" "${FREQ_RESOLVED}" "${UPPER_EPW}")
...
ICB_AUX_ELEMENTS_FROM_COUPLING=$(param ICB_AUX_ELEMENTS_FROM_COUPLING 1)
ICB_AUX_ELEMENTS_FROM_COUPLING=$(bounded_count "${ICB_AUX_ELEMENTS_FROM_COUPLING}" 1 1000000)

# Keep only the requested buffer next to coupling faces. All remaining elements
# stay on the ICB side of the auxiliary interfaces.
NZ_BELOW_ICB_AUX=$(bounded_count "$((COUPLING_IR_BOTTOM + ICB_AUX_ELEMENTS_FROM_COUPLING))" 1 "$((NZ_LOWER - 1))")
NZ_ABOVE_ICB_AUX_TOP=$(bounded_count "$((NZ_TOTAL - COUPLING_IR_TOP + ICB_AUX_ELEMENTS_FROM_COUPLING))" 1 "$((NZ_UPPER - 1))")
NZ_AUX_TO_ICB=$((NZ_LOWER - NZ_BELOW_ICB_AUX))
NZ_ICB_TO_AUX=$((NZ_UPPER - NZ_ABOVE_ICB_AUX_TOP))

Z_BELOW_ICB_AUX=$(awk -v bottom="${BOTTOM_RADIUS_KM}" -v icb="${ICB_RADIUS}" -v n="${NZ_BELOW_ICB_AUX}" -v nt="${NZ_LOWER}" -v top="${TOP_RADIUS_KM}" 'BEGIN { r=bottom + (icb-bottom)*n/nt; printf "%.6f", (r-top)*1000.0 }')
Z_ICB=$(awk -v r="${ICB_RADIUS}" -v top="${TOP_RADIUS_KM}" 'BEGIN { printf "%.6f", (r-top)*1000.0 }')
Z_ABOVE_ICB_AUX=$(awk -v icb="${ICB_RADIUS}" -v top="${TOP_RADIUS_KM}" -v n="${NZ_ICB_TO_AUX}" -v nt="${NZ_UPPER}" 'BEGIN { r=icb + (top-icb)*n/nt; printf "%.6f", (r-top)*1000.0 }')
...
BELOW_ICB_AUX_OFFSET_KM=$(awk -v z="${Z_BELOW_ICB_AUX}" -v zi="${Z_ICB}" 'BEGIN { printf "%.6f", (zi-z)/1000.0 }')
ABOVE_ICB_AUX_OFFSET_KM=$(awk -v z="${Z_ABOVE_ICB_AUX}" -v zi="${Z_ICB}" 'BEGIN { printf "%.6f", (z-zi)/1000.0 }')
TOPO_BELOW_ICB_FILE="topo_$(format_depth_label "${BELOW_ICB_AUX_OFFSET_KM}")kmBelowICB.dat"
TOPO_ABOVE_ICB_FILE="topo_$(format_depth_label "${ABOVE_ICB_AUX_OFFSET_KM}")kmAboveICB.dat"

ICB_TOPO_SOURCE=$(param ICB_TOPOGRAPHY_FILE "DATA/topo_files/latlon_ICB_topo.txt")
if [[ "${ICB_TOPO_SOURCE}" != /* ]]; then
  ICB_TOPO_SOURCE="${ROOT_DIR}/${ICB_TOPO_SOURCE}"
fi
...
INJECTED_WAVEFIELD_PATH_RAW=$(param INJECTED_WAVEFIELD_PATH "${ROOT_DIR}/WORK/InjectedWaves/OUTPUT_FILES")
if [[ "${INJECTED_WAVEFIELD_PATH_RAW}" != /* ]]; then
  INJECTED_WAVEFIELD_PATH="${ROOT_DIR}/${INJECTED_WAVEFIELD_PATH_RAW}"
else
  INJECTED_WAVEFIELD_PATH="${INJECTED_WAVEFIELD_PATH_RAW}"
fi
...
if [[ "${DSM1D_OR_3D}" == "DSM1D" ]]; then
  ...
  ICB_DXI=$(awk -v w="${ANGULAR_WIDTH_XI}" -v n="${ICB_NXI}" 'BEGIN { printf "%.10fd0", w/(n-1) }')
  ICB_DETA=$(awk -v w="${ANGULAR_WIDTH_ETA}" -v n="${ICB_NETA}" 'BEGIN { printf "%.10fd0", w/(n-1) }')
fi
...
# Interface 4: auxiliary interface below the ICB, placed above the lower coupling boundary.
# SUPPRESS_UTM_PROJECTION  NXI  NETA  XI_MIN   ETA_MIN    SPACING_XI SPACING_ETA
 .true.                    ${AUX_NXI} ${AUX_NETA} 0.d0       0.d0      ${AUX_DXI}    ${AUX_DETA}
 ${TOPO_BELOW_ICB_FILE}
# Interface 3: ICB topography.
 .true.                    ${ICB_NXI} ${ICB_NETA} 0.d0       0.d0      ${ICB_DXI}    ${ICB_DETA}
 topo_ICB.dat
# Interface 2: auxiliary interface above the ICB, placed below the upper coupling boundary.
 .true.                    ${AUX_NXI} ${AUX_NETA} 0.d0       0.d0      ${AUX_DXI}    ${AUX_DETA}
 ${TOPO_ABOVE_ICB_FILE}
...
${NZ_BELOW_ICB_AUX}
# Number of elements between the lower auxiliary interface and the ICB
${NZ_AUX_TO_ICB}
# Number of elements between the ICB and the upper auxiliary interface
${NZ_ICB_TO_AUX}
# Number of elements in the top layer
${NZ_ABOVE_ICB_AUX_TOP}
EOF_INTERFACES
...
constant_topography "${Z_BELOW_ICB_AUX}" "${MESHFEM_DIR}/topo_top.dat" "${MESHFEM_DIR}/${TOPO_BELOW_ICB_FILE}"
if [[ "${DSM1D_OR_3D}" == "DSM3D" ]]; then
  convert_interface_topography 2 "DATA/meshfem3D_files/latlon_interface_topo.txt" "${Z_ICB}" "${MESHFEM_DIR}/topo_ICB.dat"
else
  echo "DSM1D_OR_3D=DSM1D: using flat ICB interface; skipping xcubedsphere_topo conversion."
  constant_topography_grid "${Z_ICB}" "${ICB_NXI}" "${ICB_NETA}" "${MESHFEM_DIR}/topo_ICB.dat"
fi
constant_topography "${Z_ABOVE_ICB_AUX}" "${MESHFEM_DIR}/topo_top.dat" "${MESHFEM_DIR}/${TOPO_ABOVE_ICB_FILE}"
