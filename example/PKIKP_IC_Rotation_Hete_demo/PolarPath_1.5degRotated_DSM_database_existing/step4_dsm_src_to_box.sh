...
DSM_SRC_TO_BOX_DIR=$(resolve_case_path "$(param DSM_SRC_TO_BOX_DIR "../EquatorialPath/WORK/DSM/src_to_box")")

echo "----------------------------------------------------------------------"
echo "Step 4: Reusing DSM source-to-box database"
echo "DSM source-to-box directory: ${DSM_SRC_TO_BOX_DIR}"
echo "----------------------------------------------------------------------"

check_dsm_model_compatibility

if [[ ! -d "${DSM_SRC_TO_BOX_DIR}" ]]; then
  echo "Error: reused DSM source-to-box directory not found: ${DSM_SRC_TO_BOX_DIR}" >&2
  exit 1
fi
if [[ ! -d "${DSM_SRC_TO_BOX_DIR}/explosion" ]]; then
  echo "Error: expected explosion source database is missing: ${DSM_SRC_TO_BOX_DIR}/explosion" >&2
  exit 1
fi

echo "Step 4 complete: no DSM job submitted for this rotated case."
